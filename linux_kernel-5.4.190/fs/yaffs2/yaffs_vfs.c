...
static unsigned int yaffs_uid;
static unsigned int yaffs_gid;
/* Module Parameters */
#if (LINUX_VERSION_CODE > KERNEL_VERSION(2, 5, 0))
...
module_param(yaffs_uid, uint, 0644);
module_param(yaffs_gid, uint, 0644);
#else
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 0))
		if(inode->i_uid == 0) inode->i_uid = yaffs_uid;
		if(inode->i_gid == 0) inode->i_gid = yaffs_gid;
#else
		if(inode->i_uid.val == 0) inode->i_uid.val = yaffs_uid;
		if(inode->i_gid.val == 0) inode->i_gid.val = yaffs_gid;
#endif
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 19))
