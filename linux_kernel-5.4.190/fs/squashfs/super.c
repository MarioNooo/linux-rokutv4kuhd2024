...
static int squashfs_fill_super(struct super_block *sb, struct fs_context *fc)
{
	...
	for (offset = 0; offset <= 512; offset += 512) {
		/*
		 * msblk->bytes_used is checked in squashfs_read_table to ensure
		 * reads are not beyond filesystem end.  But as we're using
		 * squashfs_read_table here to read the superblock (including
		 * the value of bytes_used) we need to set it to an initial 
		 * sensible dummy value
		 */
		msblk->bytes_used = sizeof(*sblk) + offset;
		sblk = squashfs_read_table(sb,
				SQUASHFS_START + offset, sizeof(*sblk));
		if (IS_ERR(sblk)) {
	...
	/* Check the filesystem does not extend beyond the end of the
	   block device */
#ifdef CONFIG_SQUASHFS_ROKU
	msblk->bytes_used = le64_to_cpu(sblk->bytes_used) + msblk->sb_offset;
#else
...
#ifdef CONFIG_SQUASHFS_ROKU
		next_table = le64_to_cpu(sblk->bytes_used);
#else
...
static const struct fs_parameter_spec squashfs_param_specs[] = {
	fsparam_u32    ("uid",     MOUNT_PARAM_UID),
	fsparam_u32    ("gid",     MOUNT_PARAM_GID),
	{}
};

const struct fs_parameter_description squashfs_fs_parameters = {
	.name		= "squashfs",
	.specs		= squashfs_param_specs,
};

int squashfs_parse_param(struct fs_context *fc, struct fs_parameter *param)
{
	struct fs_parse_result result;
	struct squashfs_sb_info *sbi =  fc->s_fs_info;
	int opt;
	...
	switch (opt) {
	case MOUNT_PARAM_UID:
		sbi->uid = result.uint_32;
		break;
	case MOUNT_PARAM_GID:
		sbi->gid = result.uint_32;
		break;
	default:
		return -EINVAL;
	}
...
#ifdef CONFIG_CRAMFS_ROKU
extern int cramfs_get_authdata(struct file *file, unsigned int wtype,
