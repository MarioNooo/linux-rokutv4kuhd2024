...
#define CRAMFS_FLAG_ROKU	0x00001000	/* for roku */
#define CRAMFS_ROKU_SUPPORTED_FLAGS	\
	(CRAMFS_SUPPORTED_FLAGS | CRAMFS_FLAG_ROKU)
	...
	const struct cramfs_inode *cramfs_inode, unsigned int offset)
{
	...
	if (CRAMFS_SB(sb)->flags & (MS_NODEV | MS_NOSUID))
		inode->i_mode |= S_ISDIR(cramfs_inode->mode) ? 0050 : 0040;
		/*(S_IRGRP | S_IXGRP) : S_IRGRP */
	/* Unless the file is a special file, it should not writable. */
	if (!S_ISBLK(inode->i_mode) && !S_ISCHR(inode->i_mode) &&
	    !S_ISFIFO(inode->i_mode) && !S_ISSOCK(inode->i_mode))
		inode->i_mode &= ~(0222); /* S_IWUGO */
	inode->i_uid.val = CRAMFS_SB(sb)->uid == 0 ?
		cramfs_inode->uid : CRAMFS_SB(sb)->uid;
	inode->i_gid.val = CRAMFS_SB(sb)->gid == 0 ?
		cramfs_inode->gid : CRAMFS_SB(sb)->gid;
#else
			     ...
			     struct cramfs_super *super)
{
	...
	if (super->flags & ~CRAMFS_ROKU_SUPPORTED_FLAGS) {
#else
...
static const struct fs_parameter_spec cramfs_param_specs[] = {
	fsparam_u32    ("uid",     MOUNT_PARAM_UID),
	fsparam_u32    ("gid",     MOUNT_PARAM_GID),
	{}
};

const struct fs_parameter_description cramfs_fs_parameters = {
	.name		= "cramfs",
	.specs		= cramfs_param_specs,
};

int cramfs_parse_param(struct fs_context *fc, struct fs_parameter *param)
{
	struct fs_parse_result result;
	struct cramfs_sb_info *sbi =  fc->s_fs_info;
	int opt;

	opt = fs_parse(fc, &cramfs_fs_parameters, param, &result);
	if (opt < 0)
		return opt;
...
#ifdef CONFIG_CRAMFS_ROKU
	.parse_param = cramfs_parse_param,
#endif
};
