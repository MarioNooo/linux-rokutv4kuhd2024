...
unsigned long find_section(char *sec_name, struct elfhdr *ehdr, struct file *file)
{
	...
	if (!sec_tbl)
		return 0;
	pos = ehdr->e_shoff;
	retval = kernel_read(file, (char *)sec_tbl, sec_tbl_size, &pos);
	...
	pos = sec_tbl[ehdr->e_shstrndx].sh_offset;
	retval = kernel_read(file, str_tbl, str_tbl_size, &pos);
...
static void *zlib_deflate_buffer;
// If we're going to use gzipped cores, we don't need nearly as much ltcore memory, so trim it down
#define LTCORE_MAX_PAGES 224
#else
#define LTCORE_MAX_PAGES 768
#endif

unsigned int __section(.bss_noinit) core_dumped;
unsigned int __section(.bss_noinit) ltcore_dump_cnt;
char __section(.bss_noinit) ltcore_dump_file[LTCORE_MAX_PAGES * PAGE_SIZE];
...
const struct file_operations ltcore_fops = {
	.read   = read_ltcore,
	.open   = open_ltcore,
...
static struct vm_area_struct *next_vma(struct vm_area_struct *this_vma,
					struct vm_area_struct *gate_vma);
static u8 *ltcore_idx[32];

static int ltcore_set_alloc(void)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(ltcore_idx); i++) {
		ltcore_idx[i] = (u8 *)get_zeroed_page(GFP_ATOMIC);
		if (!ltcore_idx[i]) {
			pr_info("%s: get_zeroed_page failed!\n", __func__);
				return 0;
		}
	}
	return 1;
...
static void ltcore_free_pages(void)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(ltcore_idx); i++)
		free_page((unsigned long)ltcore_idx[i]);

	memset(ltcore_idx, 0, sizeof(ltcore_idx));
}
...
#define GET_USER_VAR(ret_, local_, uaddr_) do { \
	ret_ = 0; if (__copy_from_user_inatomic(&(local_), \
	(const char __user *)(uaddr_), sizeof(local_))) ret_ = 1; \
} while (0)
	...
	GET_USER_VAR(ret, r_map, r_debug + 4);
	if (ret)
		return;
	while (r_map) {
		...
		GET_USER_VAR(ret, l_name, r_map + 4);
		if (ret)
			return;
		GET_USER_VAR(ret, l_next, r_map + 12);
		if (ret)
			return;
		if (l_name)
			ltcore_add_range(l_name, l_name + l_name_size);
		r_map = l_next;
	}
}
...
static int ltcore_get_seg_count(struct vm_area_struct *gate_vma, long signr)
{
	...
	for (vma = first_vma(current, gate_vma); vma != NULL;
		...
		/* vma_dump_size(vma, mm_flags, signr) in grsecure */
...
static int fill_note_info(struct elfhdr *elf, int phdrs,
			  ...
			  const kernel_siginfo_t *siginfo, struct pt_regs *regs, bool ltcore_mode)
#else
static int fill_note_info(struct elfhdr *elf, int phdrs,
			  struct elf_note_info *info,
...
static int elf_core_dump(struct coredump_params *cprm)
{
	...
	if (cprm->ltcore_mode) {
		...
		/*
		 * Collect all the non-memory information about the
		 * process for the notes.  This also sets up the file
		 * header.
		 */
		if (!fill_note_info(elf, 0, &info, cprm->siginfo, cprm->regs, true))
			goto cleanup;

		gate_vma = get_gate_vma(current->mm);
		segs = ltcore_get_seg_count(gate_vma, cprm->siginfo->si_signo);
	...
	if (cprm->ltcore_mode) {
		...
		if (!fill_note_info(elf, e_phnum, &info, cprm->siginfo, cprm->regs, false))
			goto cleanup;
	}
#else
	...
	if (!cprm->ltcore_mode) {
		...
		for (i = 0, vma = first_vma(current, gate_vma); vma != NULL;
			...
			/* vma_dump_size(vma, mm_flags, signr) in grsecure patch */
...
vma_again:
			...
			for (addr = vm_start; addr < vm_end; addr += PAGE_SIZE) {
				...
				if (__copy_from_user_inatomic(buf,
					(const char __user *)addr, PAGE_SIZE))
...
#ifdef CONFIG_ROKU_LTCORE
void ltcore_dump(struct coredump_params *cprm)
{
	unsigned int total_in = 0;

	pr_notice("starting ltcore dump\n");

	ltcore_dump_cnt = 0;

	core_dumped = 0x00dead00; /* unfinished */

	cprm->limit = sizeof(ltcore_dump_file);
#ifdef	LTCORE_USE_ZLIB
	{
		int rv;
		...
		ltcore_dump_zstr.avail_out = sizeof(ltcore_dump_file);
		/*
		 * ZLIB will stop us from overrunning the buffer, make
		 * the raw limit greater than the expected compression
		 * ratio.
		 */
		cprm->limit *= 20;
		elf_core_dump(cprm);
	...
	__cpuc_flush_dcache_area(ltcore_dump_file, sizeof(ltcore_dump_file));
	__cpuc_flush_dcache_area(&ltcore_dump_cnt, sizeof(ltcore_dump_cnt));
	__cpuc_flush_dcache_area(&core_dumped, sizeof(core_dumped));

	pr_notice("core_dumped: %x %u -> %u\n", core_dumped, total_in, ltcore_dump_cnt);
}
#endif
