...
static u8 die_magic[] = {0x6b, 0x73, 0x74, 0x6b, 0xf2, 0xdb, 0x7e, 0xcd};
static u8 die_stack[THREAD_SIZE + sizeof(struct pt_regs) + sizeof(die_magic)] __section(.bss_noinit);

ssize_t read_kstack(struct file *file, char __user *buf,
			size_t count, loff_t *ppos)
{
	if (memcmp(die_stack + THREAD_SIZE + sizeof(struct pt_regs), die_magic, sizeof(die_magic)))
		return 0;
	...
	*ppos += count;
	if (*ppos >= sizeof(die_stack))
		memset(die_stack, 0, sizeof(die_stack));
	return count;
}
...
static int __die(const char *str, int err, struct pt_regs *regs)
{
	...
	if (die_counter == 0) {
		...
		memcpy(die_stack + THREAD_SIZE + sizeof(struct pt_regs), die_magic, sizeof(die_magic));
		__cpuc_flush_dcache_area(die_stack, sizeof(die_stack));
	}
	if (die_counter > 2)
		while (true)
		; /* That's enough. Spin forever. */
