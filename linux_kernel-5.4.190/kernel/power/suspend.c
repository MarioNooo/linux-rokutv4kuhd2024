...
static void platform_resume_noirq(suspend_state_t state)
{
    ...
    if (!iomem_kpi) {
        printk(KERN_WARNING "cannot fetch kernel start time\n");
    } else {
        memcpy_fromio((void *)&kpi, (void *)iomem_kpi, (unsigned int)sizeof(__roku_boot_resume_kpi));
        iounmap(iomem_kpi);
        kernel_start = kpi.kernel_start;
    }
    ktime_t kernel_time_stamp_current = ktime_get();
    resume_start_time = ktime_to_ms(kernel_time_stamp_current) - (s64)kernel_start;
#endif
}
