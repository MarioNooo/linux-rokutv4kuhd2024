...
static char __last_log_buf_initdata[__LOG_BUF_LEN] __initdata;
static char *__last_log_buf;
...
#ifdef CONFIG_RTLOG
static int null_vprintk_emit(int facility, int level, const char *dict, size_t dictlen, const char *fmt, va_list args)
{
    return 0;
}

static int (*g_vprintk_cb)(int facility, int level, const char *dict, size_t dictlen, const char *fmt, va_list args) = null_vprintk_emit;

void set_vprintk_callback(int (*cb)(int facility, int level, const char *dict, size_t dictlen, const char *fmt, va_list args))
{
    if ( NULL == cb ) {
        g_vprintk_cb = null_vprintk_emit;
    } else {
        g_vprintk_cb = cb;
    }
}
EXPORT_SYMBOL(set_vprintk_callback);
#endif
...
#ifdef CONFIG_RTLOG
    if ( (*g_vprintk_cb)(facility,level,dict,dictlen,text,args) )
    {
        return(0);
    }
#endif
 ...
 */
#ifdef CONFIG_RTLOG_STEAL_PRINTK
asmlinkage __visible int _non_rtlog_printk(const char *fmt, ...)
#else
...
#ifdef CONFIG_RTLOG_STEAL_PRINTK
EXPORT_SYMBOL(_non_rtlog_printk);
#else
...
void printk_init(void)
{
	...
	if ((__log_buf_signature == __LOG_BUF_SIGNATURE) &&
		...
		memcpy(__last_log_buf_initdata, log_buf, __LOG_BUF_LEN);
...
void printk_copybuffer(void)
{
	__last_log_buf = kmalloc(__LOG_BUF_LEN, GFP_KERNEL);
	memcpy(__last_log_buf, __last_log_buf_initdata, __LOG_BUF_LEN);
}
...
static u32 last_log_next(u32 idx)
{
	...
	if (idx + msg->len > __LOG_BUF_LEN) {
		pr_err("%s tries to go beyond end of buffer idx=%u, msg->len=%u, len=%u\n",
			__func__, idx, msg->len, __LOG_BUF_LEN);
		pr_err("last_log_first_seq=%llu, last_log_first_idx=%u\n",
