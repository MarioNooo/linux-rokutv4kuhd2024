...
static u8 u8mt5873_slpprot[SLPPROT_MT5873_GALS][SLPPROT_MAX_CONTENT] = {
	{0x14,  0,   1,   2,   3, 0x84,  8,   9, SLPPROT_ENABLE},//pm
};
...
#define SMI_COMMON_BUS_SEL_OFFSET 0x220
#define SMI_COMMON_BUS_SEL_BIT	0x3
#define SMI_COMMON_MAX_N	8
static const u16 emi_only_port[] = {0, 1, 7};
struct smi_common_info {
	const char *name;
	void __iomem *base;
	u32 output_r_sig;/* right path signature */
	u32 output_l_sig;/* left path signature */
} smi_common_dev[SMI_COMMON_MAX_N] = {0};

/*
 * print help info of cid
 * smi_common num will be returned
 */
static u16 get_and_print_hit_cid_ext_info(u64 cid)
{
	int i;
	u16 value, mask = SMI_COMMON_BUS_SEL_BIT;
	u16 emi_num, smi_port_num;
	u16 hit_emi_ssc_sig, path_sig;
	u16 ret = USHRT_MAX;

	emi_num = (u16)abs_value_w_mask(cid, CID_2_EMI_MASK);

	for (i = 0; i < sizeof(emi_only_port)/sizeof(u16); i++) {
		if (emi_only_port[i] == emi_num)
			return ret;
	}
	/* [15:11] */
	hit_emi_ssc_sig = (u16)abs_value_w_mask(cid, (CID_2_EMI_MASK | CID_2_SSC_MASK));
	/* [10:8] */
	smi_port_num = (u16)abs_value_w_mask(cid, SMI_PORT_NUM_MASK);
	/* right,left setting mask */
	mask = mask << (smi_port_num * hweight_long(SMI_COMMON_BUS_SEL_BIT));

	for (i = 0; i < SMI_COMMON_MAX_N; i++) {
		if (smi_common_dev[i].base) {
			value = readw_relaxed(smi_common_dev[i].base + SMI_COMMON_BUS_SEL_OFFSET);
			if (abs_value_w_mask(value, mask) == 0)
				path_sig = smi_common_dev[i].output_l_sig;
			else
				path_sig = smi_common_dev[i].output_r_sig;

		} else
			path_sig = smi_common_dev[i].output_r_sig;

		if (path_sig == 0)
			pr_err("[MIUP][HIT]unable to parse %d sig\n", i);

		if (path_sig == hit_emi_ssc_sig) {
			pr_emerg("CID:0x%llX_smi:%d_port:%d:0x%x\n", cid,
				i, smi_port_num,
				(u16)(cid & CID_HW_ONLY_MASK)
			);
			ret = i;
			break;
		}
...
/*
 * Find "smi-common*" node in dtb and fetch info
 */
#define smi_commonX_str_size	12
#define MAX_OUT_PORT		2
#define SIG_INFO_SIZE		2
static void prepare_smi_common_info(void)
{
	struct device_node *node;
	int ret = 0;
	int i, output;
	char smi_common_name[smi_commonX_str_size] = {0};
	void __iomem *base;
	u32 tmp[SIG_INFO_SIZE] = {0};

	for (i = 0; i < SMI_COMMON_MAX_N; i++) {
		ret = snprintf(smi_common_name, sizeof(smi_common_name), "smi_common%d", i);
		if (ret != (smi_commonX_str_size - 1))
			continue;

		node = of_find_node_by_name(NULL, smi_common_name);
		if (!node)
			continue;
		...
		if (of_property_read_u32(node, "output-port-nr", &output)
			|| output < MAX_OUT_PORT) {
			/* for case output-port-nr < 2 , check output-path property: */
			if (of_property_read_variable_u32_array(node, "output-path",
				&tmp[0], 0, SIG_INFO_SIZE) == SIG_INFO_SIZE) {
				smi_common_dev[i].output_r_sig = emi_ssc_sig_cal(tmp[0], tmp[1]);
				smi_common_dev[i].output_l_sig = smi_common_dev[i].output_r_sig;
			}
		...
		if (!base) {
			of_node_put(node);
			pr_crit("[MIUP][HIT]%s 2 port but no reg base\n", node->name);
			continue;
		}

		smi_common_dev[i].base = base;
		smi_common_dev[i].name = node->name;

		if (of_property_read_variable_u32_array(node, "output-r-path",
			&tmp[0], 0, SIG_INFO_SIZE) == SIG_INFO_SIZE)
			smi_common_dev[i].output_r_sig = emi_ssc_sig_cal(tmp[0], tmp[1]);

		if (of_property_read_variable_u32_array(node, "output-l-path",
			&tmp[0], 0, SIG_INFO_SIZE) == SIG_INFO_SIZE)
			smi_common_dev[i].output_l_sig = emi_ssc_sig_cal(tmp[0], tmp[1]);
...
static void mtk_gals_slpprot(struct device *dev, unsigned int flag)
{
	...
	case CHIP_MT5879:
		...
		break;
	case CHIP_MT5873:
		gals_cnt = SLPPROT_MT5873_GALS;
		slpprot = *u8mt5873_slpprot;
		break;
	default:
		goto RET_FUN;
	}
				      ...
				      struct mtk_miup *pdata)
{
	...
	for (idx = 0; idx < ob_black_client_cnt; idx++) {
		...
		pr_warn("[MIUP_OB][%d][0x%X]\n", idx, ob_black_client_list[idx]);
	}
...
static irqreturn_t mtk_miup_irq(int irq, void *id)
{
	...
	for (i = 0; i < null_mem_cnt; i++) {
		...
		if (hit_log.hit_blk >= pdata->max_prot_blk &&
		   (hit_log.hit_addr >= nullmem_staddr &&
			...
			pr_warn_ratelimited(
			"hit null mem[CID:0x%llX %s address: 0x%llX - 0x%llX block:%d\n",
			hit_log.cid, (hit_log.attr == 0) ? "read" : "write",
			hit_log.hit_addr, (hit_log.hit_addr + 0x1F), hit_log.hit_blk);
			return IRQ_NONE;
		}
	...
	pr_emerg_ratelimited("[MIUP][HIT][CID:0x%llX %s address: 0x%llX - 0x%llX block:%d\n",
	hit_log.cid, (hit_log.attr == 0) ? "read" : "write",
	hit_log.hit_addr, (hit_log.hit_addr + 0x1F), hit_log.hit_blk);

	smi_common = get_and_print_hit_cid_ext_info(hit_log.cid);

	if (pdata->hit_panic)
		panic("MIUP hit panic[CID:0x%llX_%d %s address: 0x%llX - 0x%llX block:%d\n",
		hit_log.cid, smi_common, (hit_log.attr == 0) ? "read" : "write",
		hit_log.hit_addr, (hit_log.hit_addr + 0x1F), hit_log.hit_blk);
...
static void mtk_miup_init_nullmem(struct device *dev, struct mtk_miup *pdata)
{
	...
	for (i = 0; i < null_mem_cnt; i++) {
		...
		/* Only caring null mem in positive value */
		start_addr = tmp[START_ADDR_H];
		start_addr = (start_addr << SHIFT_32BIT) | tmp[START_ADDR_L];
		if (start_addr < cpu_base)
			continue;

		null_mem[i].start_addr = (start_addr - cpu_base);
...
static int mtk_miup_probe(struct platform_device *pdev)
{
	...
	ret = get_memory_info(&prot_ba_base);
	if (ret)
		goto RET_FUN;
