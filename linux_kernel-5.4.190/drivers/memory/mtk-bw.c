...
#include "mtk-bw.h"
#include "mtk-dramc.h"
#include "dt-bindings/memory/mtk-bw-client.h"

/* DRAM config */
static u32 bus_width;
static u32 dram_clock;
static struct dramc_info _dramc_info = {0};
...
static struct fifo_status *flist;
static u32 emi_cnt;
...
/* simple mon */
static u32 simple_mon;

static const u32 mt5896_effi_table_idx2freq[MT5896_EFFI_CNT] = {CLK_4266};
static const u32 mt5896_lpddr4_effi_table[EFFI_BURST_SET_CNT * MT5896_EFFI_CNT] = {
	0x200a, 0x402d, 0x5446, 0x5654, 0x5656, 0x5656, 0x5656, 0x5656,
};
struct effi_hw_info mtk5896_effi_info = {
	.table_size = MT5896_EFFI_CNT,
	.table_idx2freq = (u32 *)mt5896_effi_table_idx2freq,
	.table = mt5896_lpddr4_effi_table,
};

static const u32 mt5876_effi_table_idx2freq[MT5876_EFFI_CNT] = {CLK_2666, CLK_3200, CLK_2133};
...
struct effi_hw_info mtk5876_effi_info = {
	.table_size = MT5876_EFFI_CNT,
	.table_idx2freq = (u32 *)mt5876_effi_table_idx2freq,
	.table = mt5876_effi_table,
};

static const u32 mt5879_effi_table_idx2freq[MT5879_EFFI_CNT] = {CLK_2666, CLK_3200};
...
struct effi_hw_info mtk5879_effi_info = {
	.table_size = MT5879_EFFI_CNT,
	.table_idx2freq = (u32 *)mt5879_effi_table_idx2freq,
	.table = mt5879_effi_table,
};

static const u32 mt5873_effi_table_idx2freq[MT5873_EFFI_CNT] = {CLK_3200};
static const u32 mt5873_effi_table[EFFI_BURST_SET_CNT * MT5873_EFFI_CNT] = {
	0x1E0F, 0x382A, 0x4B3E, 0x544A, 0x564D, 0x5750, 0x5851, 0x5A52, //A32 //3200
};
struct effi_hw_info mtk5873_effi_info = {
	.table_size = MT5873_EFFI_CNT,
	.table_idx2freq = (u32 *)mt5873_effi_table_idx2freq,
	.table = mt5873_effi_table,
};

static const struct of_device_id mtk_effi_of_ids[] = {
	{
		.compatible = "mediatek,mt5896-effi-mon",
		.data = &mtk5896_effi_info,
	},
	{
		.compatible = "mediatek,mt5876-effi-mon",
		.data = &mtk5876_effi_info,
	},
	{
		.compatible = "mediatek,mt5879-effi-mon",
		.data = &mtk5879_effi_info,
	},
	{
		.compatible = "mediatek,mt5873-effi-mon",
		.data = &mtk5873_effi_info,
	},
	{}
};
...
static void put_buf_group_bw(struct bw_cfg *cfg,
			 struct bw_res *res[],
			 char *buf,
			 unsigned int *len)
{
	...
	putbuf(buf, len, "VDEC      : %6llu MB/s\033[K\n", g_bw.group_bw_vdec);
	putbuf(buf, len, "DISPLAY   : %6llu MB/s\033[K\n", g_bw.group_bw_display);
	putbuf(buf, len, "MEMC      : %6llu MB/s\033[K\n", g_bw.group_bw_memc);
	putbuf(buf, len, "CPU       : %6llu MB/s\033[K\n", g_bw.group_bw_cpu);
	putbuf(buf, len, "GRAPHIC   : %6llu MB/s\033[K\n", g_bw.group_bw_graphic);
	putbuf(buf, len, "AUD       : %6llu MB/s\033[K\n", g_bw.group_bw_aud);
	putbuf(buf, len, "OTHERS    : %6llu MB/s\033[K\n", g_bw.group_bw_others);
	putbuf(buf, len, "\033[K\n");
...
static void put_buf_simple_all_module(struct bw_cfg *cfg,
				      struct bw_res *res[],
				      char *buf,
				      unsigned int *len)
{
...
static void put_buf_fifo_status(char *buf, unsigned int *len)
{
	u32 idx;

	putbuf(buf, len, "Starting Monitor......\033[K\n\033[K\n");
	putbuf(buf, len, "\033[7m%4s%10s%10s%10s%10s%10s%10s%10s%10s%16s",
			"ID",
			"MODULE",
			"IP_NAME",
			"TH_PULTRA",
	...
	for (idx = 0; idx < fifo_cnt; idx++) {
		putbuf(buf, len, "%4d%10s%10s%10u%10d%10u%10d%10u%10d%16u",
				idx,
				flist[idx].module_name,
				flist[idx].ip_name,
				flist[idx].th_preultra,
				flist[idx].mask_preultra,
				flist[idx].th_ultra,
				flist[idx].mask_ultra,
				flist[idx].th_urgent,
				flist[idx].mask_urgent,
				flist[idx].not_done_cnt);
		putbuf(buf, len, "\033[K\n");
	}
	putbuf(buf, len, "\033[K\n");
}
...
static void larb_bw_measure(struct bw_cfg *cfg, struct larb_res *res)
{
	...
	for (i = 0; i < port_num; i++) {
		...
		/* get bw result */
		res->r_bw[i] = READ_REG(SMI_LARB_MON_BEAT_CNT)*BEAT_TO_BYTE/cfg->t_ms;
	...
	for (i = 0; i < port_num; i++) {
		...
		/* get bw result */
		res->w_bw[i] = READ_REG(SMI_LARB_MON_BEAT_CNT)*BEAT_TO_BYTE/cfg->t_ms;
...
static void comm_bw_measure(struct bw_cfg *cfg, struct bw_res *res)
{
	...
	for (i = 0; i < m_common.pcount; i++) {
		...
		/* get read bw result */
		temp_cport_data_bw = READ_REG(SMI_MON_AXI_BEA_CNT)*BEAT_TO_BYTE/cfg->t_ms;
...
static void comm_cp_dp_bw_measure(struct bw_cfg *cfg, struct scomm_res *res)
{
	...
	for (i = 0; i < port_num; i++) {
		...
		/* get bw result */
		res->r_bw[i] = READ_REG(SMI_MON_AXI_BEA_CNT)*BEAT_TO_BYTE/cfg->t_ms;
	...
	for (i = 0; i < port_num; i++) {
		...
		/* get bw result */
		res->w_bw[i] = READ_REG(SMI_MON_AXI_BEA_CNT)*BEAT_TO_BYTE/cfg->t_ms;
...
static void emi_bw_measure_init_monitor(struct bw_cfg *cfg,
					void __iomem *base,
					unsigned int offset,
					unsigned int offset_2ND)
{
	...
	/* monitor read or write transaction */
	if (cfg->rw_type == EMI_READ_ONLY) {
		bw_setbit(DBW0_RCMD_GNT_SEL, true, base + offset);
	} else if (cfg->rw_type == EMI_WRITE_ONLY) {
...
static void emi_bw_measure_init(struct bw_cfg *cfg)
{
	...
	WRITE_REG(emi_cycle, EMI_BSTP);
	emi_bw_measure_init_monitor(cfg, base, EMI_DBWA, EMI_DBWA_2ND);
	emi_bw_measure_init_monitor(cfg, base, EMI_DBWB, EMI_DBWB_2ND);
	emi_bw_measure_init_monitor(cfg, base, EMI_DBWC, EMI_DBWC_2ND);
	emi_bw_measure_init_monitor(cfg, base, EMI_DBWD, EMI_DBWD_2ND);
	emi_bw_measure_init_monitor(cfg, base, EMI_DBWE, EMI_DBWE_2ND);
	emi_bw_measure_init_monitor(cfg, base, EMI_DBWF, EMI_DBWF_2ND);

	/* monitor read and write transaction for latency */
	if (cfg->rw_type == EMI_READ_ONLY) {
		WRITE_REG(EMI_MON_RO, EMI_BMRW0);
		bw_setbit(BUS_MON_R, true, base + EMI_BMEN);
	} else if (cfg->rw_type == EMI_WRITE_ONLY) {
		WRITE_REG(EMI_MON_WO, EMI_BMRW0);
		bw_setbit(BUS_MON_W, true, base + EMI_BMEN);
	} else {
...
static int group_module_bw(struct bw_cfg *cfg, struct bw_res *res[])
{
	u32 m_idx;

	g_bw = (struct group_bw) {
		.group_bw_vdec = 0,
		.group_bw_display = 0,
		.group_bw_memc = 0,
		.group_bw_cpu = 0,
	...
	for (m_idx = 0; m_idx < module_cnt; m_idx++) {
		if (mlist[m_idx].group_id == GROUP_VDEC)
			g_bw.group_bw_vdec += res[m_idx]->bus_bw;
		else if (mlist[m_idx].group_id == GROUP_DISPLAY)
			g_bw.group_bw_display += res[m_idx]->bus_bw;
		else if (mlist[m_idx].group_id == GROUP_MEMC)
			g_bw.group_bw_memc += res[m_idx]->bus_bw;
		else if (mlist[m_idx].group_id == GROUP_CPU)
			g_bw.group_bw_cpu += res[m_idx]->bus_bw;
		else if (mlist[m_idx].group_id == GROUP_GRAPHIC)
			g_bw.group_bw_graphic += res[m_idx]->bus_bw;
		else if (mlist[m_idx].group_id == GROUP_AUD)
			g_bw.group_bw_aud += res[m_idx]->bus_bw;
		else if (mlist[m_idx].group_id == GROUP_OTHERS)
			g_bw.group_bw_others += res[m_idx]->bus_bw;
		else
			return -1;
	}
...
static ssize_t bw_group_show(struct device_driver *drv, char *buf)
{
	struct bw_cfg *cfg;
	struct bw_res **res;
	u32 m_idx;
	...
	/* group module bw */
	group_module_bw(cfg, res);

	/* print all module */
	put_buf_group_bw(cfg, res, buf, &len);
...
static ssize_t bw_total_show(struct device_driver *drv, char *buf)
{
	...
	/* monitor all module */
	if (simple_mon)
		put_buf_simple_all_module(cfg, res, buf, &len);
	else
...
static ssize_t bw_emi_show(struct device_driver *drv, char *buf)
{
	...
	/* measure emi bw */
	cfg->t_ms = emi_mon_cfg.t_ms;
	cfg->rw_type = emi_mon_cfg.rw_type;
	if (emi_measure_wrap(cfg, emi_res) < 0) {
		len = -EPERM;
		goto RET_FUNC;
	}
...
static ssize_t larb_module_list_show(struct device_driver *drv, char *buf)
{
	u32 len = 0;
	u32 mid = 0;
	u32 cnt = 0;
	...
	for (mid = 0; mid < module_cnt; mid++) {
		if (mlist[mid].larb_num != 0) {
			cnt++;
			putbuf(buf, &len, "%-2d: %-13s", mid, mlist[mid].name);
			if (cnt == LARB_MODULE_LIST_PRINT) {
				putbuf(buf, &len, "\n");
				cnt = 0;
			}
		}
...
static ssize_t fifo_status_show(struct device_driver *drv, char *buf)
{
	u32 len = 0;
	void __iomem *base;
	int i;
	...
	for (i = 0; i < fifo_cnt; i++) {
		base = flist[i].cpu_base;
		if (READ_REG(REG_ACTIVE) != 0x0) {
			if (READ_REG(REG_STATUS) & BIT(NOT_DONE))
				flist[i].not_done_cnt++;
		}
		flist[i].th_preultra = READ_REG(REG_TH_DYM_ULTRA) & FILED_TH_PREULTRA;
		flist[i].mask_preultra = READ_REG(REG_TH_DYM_ULTRA) & MASK_PREULTRA;
		flist[i].th_ultra = RSHIFT_8BIT(READ_REG(REG_TH_DYM_ULTRA) & FILED_TH_ULTRA);
		flist[i].mask_ultra = READ_REG(REG_TH_DYM_ULTRA) & MASK_ULTRA;
		flist[i].th_urgent = READ_REG(REG_TH_URGENT) & FILED_TH_URGENT;
		flist[i].mask_urgent = READ_REG(REG_TH_URGENT) & MASK_URGENT;
	}

	put_buf_fifo_status(buf, &len);

	return len;
}

static ssize_t fifo_status_store(struct device_driver *drv,
				 const char *buf,
				 size_t count)
{
	bool enable;
	...
	for (i = 0; i < fifo_cnt; i++) {
		bw_setbit(FORCE_CLK, 1, flist[i].cpu_base + REG_FORCE_CLOCK);
		wmb(); /* make sure settings are written */
		bw_setbit(FORCE_CLK, 0, flist[i].cpu_base + REG_FORCE_CLOCK);
		wmb(); /* make sure settings are written */
		bw_setbit(RB_ENABLE, enable, flist[i].cpu_base + REG_READBACK);
		if (!enable)
			flist[i].not_done_cnt = 0;
	}

	return count;
}

static ssize_t fifo_status_clear_store(struct device_driver *drv,
				       const char *buf,
				       size_t count)
{
	bool reset;
	...
	if (reset) {
		for (i = 0; i < fifo_cnt; i++) {
			bw_setbit(RB_ENABLE, 0, flist[i].cpu_base + REG_READBACK);
			wmb(); /* make sure settings are written */
			bw_setbit(RB_ENABLE, 1, flist[i].cpu_base + REG_READBACK);
			wmb(); /* make sure settings are written */
		}
	}
	return count;
...
static ssize_t dram_info_show(struct device_driver *drv, char *buf)
{
	int i;
	u32 len = 0;

	if (!_dramc_info.chn_freq || !_dramc_info.chn_size) {
		putbuf(buf, &len, "No dramc info\n");
		return len;
	}

	for (i = 0; i < _dramc_info.chn_num; i++)
		putbuf(buf, &len, " chn-%d_freq: %u\n", i, _dramc_info.chn_freq[i]);

	for (i = 0; i < _dramc_info.chn_num; i++)
		putbuf(buf, &len, " chn-%d_size: %uGb\n", i, _dramc_info.chn_size[i]);

	putbuf(buf, &len, " type: %u\n", _dramc_info.ddr_type);
	putbuf(buf, &len, " asymsize: %uGb\n", _dramc_info.ddr_asymsize);
	putbuf(buf, &len, " pkg: %u\n", _dramc_info.ddrpkg);
...
static int fake_engine_loop(u32 id, u32 *loop)
{
	...
	if (*loop) {
		if (loop_mode_gen1) {
			BW_DBG("[MTK-BW] loop_mode_gen1\n");
			WRITE_REG((READ_REG(TEST_MODE_OLD) | BIT(FAKE_ENG_LOOP)), TEST_MODE_OLD);
		} else {
			BW_DBG("[MTK-BW] loop_mode_gen2\n");
			    ...
			    size_t count)
{
	...
	if (sscanf(buf, "%u %u", &t_ms, &rw_flag) != EMI_BW_PARA_CNT) {
		pr_info("[MTK_BW]Wrong Argument!!\n");
		return count;
	} else if (!t_ms) {
	...
	pr_info("[MTK_BW]Set Monitor Time= %d\n", emi_mon_cfg.t_ms);
	emi_mon_cfg.rw_type = rw_flag;
	pr_info("[MTK_BW]Set rw_tpye = %d\n", emi_mon_cfg.rw_type);
...
static ssize_t simple_mon_store(struct device_driver *drv,
				const char *buf,
				size_t count)
{
	if (kstrtou32(buf, 0, &simple_mon) != 0)
		pr_err("[MTK_BW]Wrong Argument!!\n");
...
static ssize_t emi_setting_show(struct device_driver *drv, char *buf)
{
	...
	base = find_device_addr(EMI_CEN_DRV, EMI_0);
	for (i = 0; i < emi_port_cnt; i++) {
		arb_value = READ_REG(EMI_ARB(i));
		putbuf(buf, &len, "-->EMI-Port(%d) bw_limit: 0x%lx \033[K\n",
						i, EMI_GET_BW_LMT(arb_value));
	}
...
static DRIVER_ATTR_RO(bw_group);
static DRIVER_ATTR_WO(simple_mon);
static DRIVER_ATTR_RO(emi_setting);
static DRIVER_ATTR_RW(larb_ostdl);
static DRIVER_ATTR_WO(larb_config);
static DRIVER_ATTR_RO(larb_module_list);
static DRIVER_ATTR_RO(ip_config);
static DRIVER_ATTR_RW(fifo_status);
static DRIVER_ATTR_WO(fifo_status_clear);
...
static DRIVER_ATTR_RO(dram_info);
...
static int set_fifo_list(void)
{
	u32 idx = 0;
	struct device_node *np = NULL;

	fifo_cnt = 0;
	for_each_node_by_type(np, "fifo")
		fifo_cnt++;

	pr_err("[MTK_BW] fifo_cnt: %u\n", fifo_cnt);
	flist = kcalloc(fifo_cnt, sizeof(struct fifo_status), GFP_KERNEL);

	/* parsing all module */
	for_each_node_by_type(np, "fifo") {
		if (of_property_read_string(np, "module-name", &flist[idx].module_name) < 0)
			return -1;
		if (of_property_read_string(np, "ip-name", &flist[idx].ip_name) < 0)
			return -1;
		if (of_property_read_u32(np, "riu-base", &flist[idx].riu_base) < 0)
			return -1;

		pr_err("[MTK_BW] riu base: %x\n", flist[idx].riu_base);
		pr_err("[MTK_BW] cpu base: %x\n", RIU2CPU_ADDR(flist[idx].riu_base));
		flist[idx].cpu_base = ioremap(RIU2CPU_ADDR(flist[idx].riu_base), REG_OP_SIZE);
		if (!flist[idx].cpu_base)
			return -1;
		flist[idx].not_done_cnt = 0;
...
static int set_module_list(void)
{
	...
	for_each_node_by_type(np, "bw") {
		if (of_property_read_string(np, "module-name", &mlist[mod_idx].name) < 0) {
			pr_err("[MTK_BW] can not get module-name from dts\n");
			return -1;
		}
		if (of_property_read_string(np, "domain", &mlist[mod_idx].domain) < 0) {
			pr_err("[MTK_BW] can not get domain from dts\n");
			return -1;
		}
		if (of_property_read_u32(np, "module-id", &mlist[mod_idx].module_id) < 0) {
			pr_err("[MTK_BW] can not get module-id from dts\n");
			return -1;
		}
		if (of_property_read_u32(np, "group-id", &mlist[mod_idx].group_id) < 0) {
			pr_err("[MTK_BW] can not get group-id from dts\n");
			return -1;
		}
		...
		if (strncmp(mlist[mod_idx].name, "BIST", sizeof("BIST")) == 0) {
			...
			if (of_property_read_u32(np, "alignment", &fake_eng_st_align) < 0)
				return -1;
			loop_mode_gen1 = of_property_read_bool(np, "loop-mode-gen1");
...
static int create_ip_related_node(struct device_driver *drv)
{
	int err = 0;

	err = driver_create_file(drv, &driver_attr_fifo_status);
	if (err) {
		pr_err("[MTK_BW]fifo_status create attribute error\n");
		return err;
	}
	err = driver_create_file(drv, &driver_attr_fifo_status_clear);
	if (err) {
		pr_err("[MTK_BW]fifo_status_clear create attribute error\n");
		return err;
	}
...
static int create_bw_config_related_node(struct device_driver *drv)
{
	int err = 0;
	...
	err = driver_create_file(drv, &driver_attr_larb_module_list);
	if (err) {
		pr_err("[MTK_BW]larb_module_list create attribute error\n");
		return err;
	}
	err = driver_create_file(drv, &driver_attr_simple_mon);
	if (err) {
		pr_notice("[MTK_BW]simple_mon create attribute error\n");
		return err;
	}
	...
	err = driver_create_file(drv, &driver_attr_dram_info);
	if (err) {
		pr_notice("[MTK_BW]dram_info create attribute error\n");
		return err;
	}
...
static int create_bw_monitor_related_node(struct device_driver *drv)
{
	int err = 0;
	...
	err = driver_create_file(drv, &driver_attr_bw_group);
	if (err) {
		pr_err("[MTK_BW]bw_group create attribute error\n");
		return err;
	}
...
static int create_sysfs_node(void)
{
	...
	err = create_bw_monitor_related_node(drv);
	if (err) {
		pr_err("[MTK_BW]create_bw_monitor_related_node fail\n");
		return err;
	}
	err = create_bw_config_related_node(drv);
	if (err) {
		pr_err("[MTK_BW]create_bw_config_related_node fail\n");
		return err;
	}
	err = create_ip_related_node(drv);
	if (err) {
		pr_err("[MTK_BW]create_ip_related_node fail\n");
		return err;
	}
...
/*
 * higher chn freq lead to worse effi
 * 0 is returned if no chn freq info available
 */
static u32 get_dram_effi_worse_clk(void)
{
	u32 ret = 0;
	int channel_N, i;

	if (!_dramc_info.chn_freq)
		return 0;

	channel_N = _dramc_info.chn_num;

	for (i = 0; i < channel_N; i++)
		ret = ret > _dramc_info.chn_freq[i] ? ret : _dramc_info.chn_freq[i];

	return ret;
}

/*
 * We use minimal chn freq for bwtop ref clock
 * 0 is returned if no chn freq info available
 */
static u32 update_dram_bw_ref_clk(void)
{
	u32 ret = U32_MAX;
	int channel_N, i;
...
static int get_effi_table_index(const struct effi_hw_info *info)
{
	int ret = 0, i;
	u32 _dram_clock;
	const u32 *idx2freq;

	_dram_clock = get_dram_effi_worse_clk();

	/* if chn clock info not available, use info in dtsi */
	if (_dram_clock == 0)
		_dram_clock = dram_clock;

	idx2freq = info->table_idx2freq;
	/* if no table info available , use 0 as default */
	if (!idx2freq)
		return 0;

	for (i = 0; i < info->table_size; i++) {
		if (_dram_clock == idx2freq[i]) {
			ret = i;
			break;
		}
	}
...
static int dramc_info_setup(void)
{
	u32 channel_N, i;

	channel_N = mtk_dramc_get_support_channel_num();
	if (channel_N <= 0)
		return -ENODEV;

	/* reset info */
	kfree(_dramc_info.chn_freq);
	kfree(_dramc_info.chn_size);

	_dramc_info.chn_freq = kcalloc(channel_N, sizeof(*_dramc_info.chn_freq), GFP_KERNEL);
	if (!_dramc_info.chn_freq)
		return -ENOMEM;

	_dramc_info.chn_size = kcalloc(channel_N, sizeof(*_dramc_info.chn_size), GFP_KERNEL);
	if (!_dramc_info.chn_size)
		return -ENOMEM;

	for (i = 0; i < channel_N; i++) {
		_dramc_info.chn_freq[i] = mtk_dramc_get_freq(i);
		_dramc_info.chn_size[i] = mtk_dramc_get_channel_size(i);
	}

	_dramc_info.chn_num = channel_N;

	_dramc_info.ddr_type = mtk_dramc_get_ddr_type();
	_dramc_info.ddr_asymsize = mtk_dramc_get_ddr_asymsize();
	_dramc_info.ddrpkg = mtk_dramc_get_ddrpkg();
...
static int mtk_bw_setup_dram_config(void)
{
	...
	clock_tmp = update_dram_bw_ref_clk();
	if (clock_tmp)
		dram_clock = clock_tmp;
	pr_info("chip dram clock:[%u]\n", dram_clock);
...
static int mtk_bw_measure_init(void)
{
	...
	err = set_fifo_list();
	if (err) {
		pr_err("[MTK_BW] set_fifo_list error\n");
		return err;
	}
...
static int mtk_effi_probe(struct platform_device *pdev)
{
	...
	const u32 *array_base;
	struct resource *res;
	struct device *dev = &pdev->dev;
	struct mtk_effi *effi_mon;
	const struct effi_hw_info *effi_hw_info;
	...
	effi_hw_info = of_device_get_match_data(dev);
	if (!effi_hw_info)
		return -EINVAL;

	array_base = effi_hw_info->table;
	if (!array_base)
		return -EINVAL;

	effi_table_idx = get_effi_table_index(effi_hw_info);

	pr_err("effi table idx: [%u]\n", effi_table_idx);

	effi_mon->data = array_base + effi_table_idx * EFFI_BURST_SET_CNT;
...
SIMPLE_DEV_PM_OPS(effi_pm_ops, mtk_effi_suspend, mtk_effi_resume);
...
static int __init mtk_bw_init(void)
{
	...
	ret = dramc_info_setup();
	if (ret)
		pr_err("[MTK_BW]Failed to fetch dramc info\n");
