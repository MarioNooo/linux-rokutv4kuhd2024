...
#define DRAMC_TA_TEST_FLOW 1
#if defined(CONFIG_MTK_DRAMC_CA)
...
struct mtk_dramc_data {
	struct device *dev;
	struct iio_channel *iio;
	unsigned long dref_en, dram_pkg;
	int iHigh, dHigh, iLow, dLow;
	unsigned long chipid;
};
#define MTK_DRAMC_CONF_NUM 3
#define MTK_TEMP_THRESHOLD_NUM 4
struct mtk_dramc_data *dramc;

static u16 u16DramcChipID;
static int iChipIDindex;
// Chip ID ...
#define CHIP_ID_MT5896   0x0101
#define CHIP_ID_MT5897   0x0109
#define CHIP_ID_MT5876   0x010C
#define CHIP_ID_MT5879   0x010D
#define CHIP_ID_MT5873   0x0111
...
static struct task_struct *stIdleCntThread;
...
#define DDR_REF_RATE_7900 0x7900
#define DDR_REF_RATE_3900 0x3900
#define DDR_REF_RATE_1950 0x1950
...
#define DDR_REF_RATE_1950_SET 0x005d002e
#define DDR_REF_RATE_3900_SET 0x00bb005d
#define DDR_REF_RATE_7900_SET 0x017600bb
#define CH_OFFSET 0x2000

u16 u16Pre_interval = DDR_REF_RATE_1950;
static int refresh_temp_pre;
static u32 u32DREFLog;
static u32 u32DREFEn;
...
#define REG_PM_T_SENSOR            (0x4F << 2)
#define TSENSOR_BANK_UNIT           0x10000
#define MT5873_TS_TA_CODE_DEFAULT     275
#define MT5873_TS_VBE_CODE_FT_DEFAULT 400
...
#define MT5879_TSENSOR_EQUATION(VBE_FT, TA, VBE)   ((VBE_FT == 400) ?\
	((1270*(VBE_FT - VBE) + ((TA*100)))/1000):((1270*(VBE_FT - VBE) + ((TA*100)+1500))/1000))
#define MT5873_TSENSOR_EQUATION(VBE_FT, TA, VBE)   ((VBE_FT == 400) ?\
	((1250*(VBE_FT - VBE) + ((TA*100)))/1000):((1250*(VBE_FT - VBE) + ((TA*100)+300))/1000))

//5879 Equation = 1.27*(VBE_CODE_FT - VBE_CODE )+(TA_CODE/10+1.5)
//5873 Equation  = 1.25*(VBE_CODE_FT - VBE_CODE )+(TA_CODE/10+0.3)
...
#define DMMonitor       0x0024
#define DRAMC_REG_DRAMC_IDLE_COUNTER  0x308
#define DM_MONPAUSE_SW  0x04 //bit2
#define DM_BUSMONEN_SW  0x08 //bit3
#define MG_NUMBER_400  400
#define MG_NUMBER_1024 1024
#define DEFAULT_IDLE_THRESHOLD 20
u16 u16IdleCntThreshold = DEFAULT_IDLE_THRESHOLD;
u32 idelrate, idlecnt;

bool mtk_dramc_is_ddr_busy(void)
{
	bool bIsBusy = 0;

	if (idelrate > u16IdleCntThreshold)
		bIsBusy = 0;
	else {
		pr_crit("DDR busy idle rate:%d(x10)\n", idelrate);
		bIsBusy = 1;
	}
	return bIsBusy;
}
EXPORT_SYMBOL(mtk_dramc_is_ddr_busy);

void mtk_dramc_set_idlecnt_busy_threshold(u16 threshold)
{
	pr_crit("Change idle threshold %d > %d\n", u16IdleCntThreshold, threshold);
	u16IdleCntThreshold = threshold;
}
EXPORT_SYMBOL(mtk_dramc_set_idlecnt_busy_threshold);

static void mtk_dramc_idlecnt_monitor(void)
{

		//1. RG_BUSMONEN_SW = 0 (reset counter)
		//2. RG_MONPAUSE_SW = 0
		//3. RG_BUSMONEN_SW = 1 (start to counter)
		//wait 1000ms
		//4. RG_MONPAUSE_SW =  1 (pause counter)
		//5. read counter value

	DRAMC_NAO_WRITE_REG(DRAMC_NAO_READ_REG(DMMonitor)&~DM_BUSMONEN_SW, DMMonitor);
	DRAMC_NAO_WRITE_REG(DRAMC_NAO_READ_REG(DMMonitor)&~DM_MONPAUSE_SW, DMMonitor);
	DRAMC_NAO_WRITE_REG(DRAMC_NAO_READ_REG(DMMonitor)|DM_BUSMONEN_SW, DMMonitor);

	msleep(MG_NUMBER_1000);
	DRAMC_NAO_WRITE_REG(DRAMC_NAO_READ_REG(DMMonitor)|DM_MONPAUSE_SW, DMMonitor);

	idlecnt = DRAMC_NAO_READ_REG(DRAMC_REG_DRAMC_IDLE_COUNTER);
	idelrate = ((idlecnt)/(MG_NUMBER_400*MG_NUMBER_1024));
}

static int mtk_dramc_idlecnt_task(__maybe_unused void *punused)
{
	if (pDramcRes != NULL) {
		while (pDramcRes->bIdleCntTaskProcFlag)
			mtk_dramc_idlecnt_monitor();
	}
	return 0;
}

static bool Dramc_Idle_Cnt_Task_Init(void)
{
	bool bCreateSuccess = 1;

	if (pDramcRes->sIdleCntPollingTaskID < 0) {
		stIdleCntThread = kthread_create(mtk_dramc_idlecnt_task
			, pDramcRes, "DDR IdleCnt task");

			if (IS_ERR(stIdleCntThread)) {
				bCreateSuccess = 0;
				stIdleCntThread = NULL;
				pr_err("Create IdleCnt Thread Failed\r\n");
			} else {
				wake_up_process(stIdleCntThread);
				pDramcRes->sIdleCntPollingTaskID = stIdleCntThread->tgid;
				pDramcRes->bIdleCntTaskProcFlag = 1;
				pr_crit("Create IdleCnt Thread OK\r\n");
			}
		}
...
int Dramc_Thermal_CPU_Tsensor(void)
{
	int u6Thermal = 0;

	if (u16DramcChipID == CHIP_ID_MT5879) {
		u6Thermal = MT5879_TSENSOR_EQUATION(g_u16VBE_Code_FT,
				g_u16TA_Code,
				T_SENSOR_READ_REG(REG_CPU_T_SENSOR));
	} else if (u16DramcChipID == CHIP_ID_MT5873) {
		u6Thermal = MT5873_TSENSOR_EQUATION(g_u16VBE_Code_FT,
				g_u16TA_Code,
				T_SENSOR_READ_REG(REG_CPU_T_SENSOR));
	} else
		u6Thermal = 0;
...
int Dramc_Thermal_XC_Tsensor(void)
{
	int u6Thermal = 0;
...
int Dramc_Thermal_HDMI_Tsensor(void)
{
	int u6Thermal = 0;
...
int Dramc_Thermal_PM_Tsensor(void)
{
	int u6Thermal = 0;

	if (u16DramcChipID == CHIP_ID_MT5873) {
		u6Thermal = MT5873_TSENSOR_EQUATION(g_u16VBE_Code_FT,
				g_u16TA_Code,
				T_SENSOR_READ_REG(REG_PM_T_SENSOR));
	} else
		u6Thermal = 0;
...
static int Dramc_Cal_IC_Temperature_Avg(void)
{
	...
	if (u16DramcChipID == CHIP_ID_MT5876) {
	...
	} else if (u16DramcChipID == CHIP_ID_MT5879) {
		t = (Dramc_Thermal_CPU_Tsensor()
			+ Dramc_Thermal_XC_Tsensor()
			+ Dramc_Thermal_HDMI_Tsensor())/3;

		if (u32DREFLog == 1) {
			pr_crit("[Dramc_DRef] avg:%d cpu:%d xc:%d hdmi:%d"
			, t
...
int Dramc_Ref_Rate_Setting_Compare(int interval, int Ch_Cnt)
{
		switch (interval) {
		case DDR_REF_RATE_1950:
		{
			if (DRAMC_READ_REG((Ch_Cnt*CH_OFFSET)
							+REG_DDR_REF_RATE) == DDR_REF_RATE_1950_SET)
				return 1;
		}
		break;

		case DDR_REF_RATE_3900:
		{
			if (DRAMC_READ_REG((Ch_Cnt*CH_OFFSET)
							+REG_DDR_REF_RATE) == DDR_REF_RATE_3900_SET)
				return 1;
		}
		break;

		case DDR_REF_RATE_7900:
		{
			if (DRAMC_READ_REG((Ch_Cnt*CH_OFFSET)
							+REG_DDR_REF_RATE) == DDR_REF_RATE_7900_SET)
				return 1;
		}
		break;
...
static int Dramc_Is_MCP(int Ch_Cnt)
{
	int IsMCP = 0;

	if ((u16DramcChipID == CHIP_ID_MT5876)
		|| (u16DramcChipID == CHIP_ID_MT5879)) {
		if (mtk_dramc_get_freq(Ch_Cnt) < 3200)
			IsMCP = 1;
		else
			IsMCP = 0;

	} else if (u16DramcChipID == CHIP_ID_MT5873) {
		if (mtk_dramc_get_ddrpkg() == 0)
			IsMCP = 1;
		else
			IsMCP = 0;
	} else
...
static void Dramc_Set_Ddr_Refresh_Rate(int interval, int Ch_Cnt)
{
	...
	case DDR_REF_RATE_1950:
	{
		...
		if (Dramc_Is_MCP(Ch_Cnt) == 1) {//1.95us is MPC only
			DRAMC_WRITE_REG(DDR_REF_RATE_1950_SET,
			((Ch_Cnt*CH_OFFSET)+REG_DDR_REF_RATE));
		} else {
			DRAMC_WRITE_REG(DDR_REF_RATE_3900_SET,
			((Ch_Cnt*CH_OFFSET)+REG_DDR_REF_RATE));
		}
	}
	break;
	...
	case DDR_REF_RATE_3900:
	{
		...
		DRAMC_WRITE_REG(DDR_REF_RATE_3900_SET, ((Ch_Cnt*CH_OFFSET)+REG_DDR_REF_RATE));
	}
	break;
	...
	case DDR_REF_RATE_7900:
	{
		...
		DRAMC_WRITE_REG(DDR_REF_RATE_7900_SET, ((Ch_Cnt*CH_OFFSET)+REG_DDR_REF_RATE));
	...
	if (u32DREFEn == 1) {
		if (temp <= dramc->dLow) {
			...
			for (Ch_Cnt = 0; Ch_Cnt < DramcChannelMax; Ch_Cnt++) {
				if (!Dramc_Ref_Rate_Setting_Compare(DDR_REF_RATE_7900, Ch_Cnt))
					Dramc_Set_Ddr_Refresh_Rate(DDR_REF_RATE_7900, Ch_Cnt);
			}
		} else if (temp >= dramc->iHigh) {
			//set 1.95us
			for (Ch_Cnt = 0; Ch_Cnt < DramcChannelMax; Ch_Cnt++) {
				if (!Dramc_Ref_Rate_Setting_Compare(DDR_REF_RATE_1950, Ch_Cnt))
					Dramc_Set_Ddr_Refresh_Rate(DDR_REF_RATE_1950, Ch_Cnt);
			}
		} else if ((refresh_temp_pre > dramc->dHigh)
			&& (temp <= dramc->dHigh)) {
			//set 3.90
			for (Ch_Cnt = 0; Ch_Cnt < DramcChannelMax; Ch_Cnt++) {
				if (!Dramc_Ref_Rate_Setting_Compare(DDR_REF_RATE_3900, Ch_Cnt))
					Dramc_Set_Ddr_Refresh_Rate(DDR_REF_RATE_3900, Ch_Cnt);
			}
		} else if ((refresh_temp_pre < dramc->iLow)
			&& (temp >= dramc->iLow)) {
			//set 3.90
			for (Ch_Cnt = 0; Ch_Cnt < DramcChannelMax; Ch_Cnt++) {
				if (!Dramc_Ref_Rate_Setting_Compare(DDR_REF_RATE_3900, Ch_Cnt))
...
static int Is_Dynamic_Refresh_Rate_Support(void)
{
	if ((u16DramcChipID == CHIP_ID_MT5876)
		|| (u16DramcChipID == CHIP_ID_MT5879)
		|| (u16DramcChipID == CHIP_ID_MT5873)) {
		return 1;
	} else {
		return 0;
	}
...
static int Dynamic_Refresh_Rate_Task(__maybe_unused void *punused)
{
	if (pDramcRes != NULL) {
		while (pDramcRes->bDRAMCTaskProcFlag) {
			if (Is_Dynamic_Refresh_Rate_Support())
...
void dramc_dynamic_ref_debug(void)
{
	...
	for (Ch_Cnt = 0; Ch_Cnt < DramcChannelMax; Ch_Cnt++) {
		if (Dramc_Ref_Rate_Setting_Compare(DDR_REF_RATE_7900, Ch_Cnt))
			pr_crit("ch%d:7800ns\n", Ch_Cnt);
		else if (Dramc_Ref_Rate_Setting_Compare(DDR_REF_RATE_3900, Ch_Cnt))
			pr_crit("ch%d:3900ns\n", Ch_Cnt);
		else if (Dramc_Ref_Rate_Setting_Compare(DDR_REF_RATE_1950, Ch_Cnt))
			pr_crit("ch%d:1950ns\n", Ch_Cnt);
		else
			pr_crit("\n");
	}
...
static ssize_t dramc_dynamic_ref_show(struct device_driver *drv, char *buf)
{
	...
	pr_crit("\n[Dramc DRef] enable: %d reg_id:%x dts_id:%lu\n",
		u32DREFEn, pm_top_get_chip_id(), dramc->chipid);
	dramc_dynamic_ref_debug();
...
{
	int channel_num = 0, ddr_total_size = 0;
...
#define SHIFT_L_B_RISE   6 //decrease rx phase DQ perbit for Rising
#define SHIFT_R_B_RISE   7 //increase rx phase DQ perbit for Rising
#define SHIFT_L_B_FALL   8 //decrease rx phase DQ perbit for Falling
#define SHIFT_R_B_FALL   9 //increase rx phase DQ perbit for Falling
#define SHIFT_L_B_RFSM   10 //decrease rx phase DQ perbit for RF Same
#define SHIFT_R_B_RFSM   11 //increase rx phase DQ perbit for RF Same
...
//################################################
//M6
#define DRAMC_REG_DRAMCTRL_Ofst   0x024C
//M6L
#define DRAMC_REG_DRAMCTRL_Ofst_M6L   0x0004
#define ALE_BLKD BIT(14)
...
#define DDRPHY_REG_RK_B0_RXDVS2_Ofst_M6L   0x0608 //DDRPHY_R0_B0_RXDVS2_Ofst
	//[31:30] = 2'b01 : Run Time Shmoo ®É(SW Mode)
	//[31:30] = 2'b10: HW tracking®É(HW mode)
	//[31:30] = 2'b00: Direct using RG mode
	//[29]=1: Falling delay mode (set rising and falling separately)
...
{
	return (u16DramcChipID == CHIP_ID_MT5896) ? true : false;
}

static bool IsManks(void)
{
	return (u16DramcChipID == CHIP_ID_MT5897) ? true : false;
}
...
static bool IsShuffleRG1(void)
{
	...
	if (IsMachli() == true) {
		...
		u32val_shuLvl = SHM_DDRPHY_RD(u32addr_shuLvl);
...
static u32 MapAddr_dramc_chip(u32 u32addr, u8 u8channel, u8 u8rank, u8 u8byte)
{
	...
	case DRAMC_REG_TEST2_A4_Ofst:
		...
		break;
	case DRAMC_REG_DRAMCTRL_Ofst:
		if (IsManks() == true)
			u32addr_rslt_chip = DRAMC_REG_DRAMCTRL_Ofst_M6L;
		break;
...
static u32 u32RxWinBit = CRB_MAX;
...
#define RX_PHASE_DQS_MSK_R   ((IsMachli() == true) ? 0x000001ff : 0x007f0000)
#define RX_PHASE_DQS_MSK_F   ((IsMachli() == true) ? 0x01ff0000 : 0x7f000000)
#define RX_PHASE_DQS_MAX   ((IsMachli() == true) ? 0x000001ff : 0x0000007f)
#define RX_PHASE_DQS_SFT_R   ((IsMachli() == true) ? 0 : 16)
#define RX_PHASE_DQS_SFT_F   ((IsMachli() == true) ? 16 : 24)
#define RX_DVS0_SW_UP   BIT(8)
#define RX_DVS2_MOD_MSK   (BIT(31) | BIT(30))
#define RX_DVS2_MOD_FDLY   BIT(29)
#define RX_DVS2_MOD_SW   BIT(30)
#define RX_DVS2_MOD_HW   BIT(31)
#define RX_DVS2_MOD_RG   0x00000000
#define RX_RF_RGP_NUM   2
#define RX_RF_RGA_NUM   4
#define RX_RF_RGA_0   0
#define RX_RF_RGA_1   1
#define RX_RF_RGA_2   2
#define RX_RF_RGA_3   3
...
static u8 dramc_rx_phase_write_DQS(u8 u8channel, u8 u8rank, u8 u8byte, u32 u32phase_target)
{
	...
	//################################################
	// pre-setting rising or falling or same for dq perbit
	//################################################
	// get rxdvs0 setting
	u32addr_rxdvs0 = TransAddr_ddrphy(
		DDRPHY_REG_B0_RXDVS0_Ofst, u8channel, u8rank, u8byte);
	u32val_rxdvs0 = SHM_DDRPHY_RD(u32addr_rxdvs0);
	...
	u32val_rxdvs2 = SHM_DDRPHY_RD(u32addr_rxdvs2);
	pr_err("[%s] DDRPHY_READ[0x%08x] = 0x%08x\n",
		__func__, u32addr_rxdvs2, u32val_rxdvs2);

	// set rxdvs2 sw mode
	u32val_rxdvs2 = (u32val_rxdvs2 & (~RX_DVS2_MOD_MSK)) | RX_DVS2_MOD_SW;
	SHM_DDRPHY_WR(u32val_rxdvs2, u32addr_rxdvs2);
	pr_err("[%s] DDRPHY_WRITE[0x%08x] = 0x%08x\n",
		__func__, u32addr_rxdvs2, u32val_rxdvs2);
	...
	// write rg value
	// set rxdqs_dly
	u32val_rxdqsdly &= (~(RX_PHASE_DQS_MSK_R | RX_PHASE_DQS_MSK_F));
	u32val_rxdqsdly |= u32phase_target;
	SHM_DDRPHY_WR(u32val_rxdqsdly, u32addr_rxdqsdly);
	pr_crit("[%s] DDRPHY_WRITE[0x%08x] = 0x%08x\n",
		__func__, u32addr_rxdqsdly, u32val_rxdqsdly);

	//################################################
	// post-setting rising or falling or same for dq perbit
	//################################################
	// toggle rxdvs0 to really write phase to circuit
	u32val_rxdvs0 &= (~RX_DVS0_SW_UP); //bit8=0
	SHM_DDRPHY_WR(u32val_rxdvs0, u32addr_rxdvs0);
	pr_err("[%s] DDRPHY_WRITE[0x%08x] = 0x%08x (Toggle 0)\n",
		__func__, u32addr_rxdvs0, u32val_rxdvs0);
	u32val_rxdvs0 |= RX_DVS0_SW_UP; //bit8=1
	SHM_DDRPHY_WR(u32val_rxdvs0, u32addr_rxdvs0);
	pr_err("[%s] DDRPHY_WRITE[0x%08x] = 0x%08x (Toggle 1)\n",
		__func__, u32addr_rxdvs0, u32val_rxdvs0);
	...
	u8 u8dir, u8 u8changedelta, u32 u32ta2looptimes)
{
	...
	pr_crit("[%s][RX][ORIGINAL_VALUE][VREF] 0x%08x\n", __func__, u32val_rxdqvref_target);
	...
	case SHIFT_D: //decrease vref
		...
		pr_crit("[%s][RX][CHANGE_VALUE][VREF] 0x%08x\n", __func__, u32val_rxdqvref_target);
	...
	u8 u8dir, u8 u8changedelta, u32 u32ta2looptimes)
{
	...
	pr_crit("\n\n########################\n");
	pr_crit("Run dramc_rxwincenter for Phase DQ\n");
	pr_crit("########################\n\n");
	...
	if ((u8dir == SHIFT_L) || (u8dir == SHIFT_R)) {
		...
		for (u8dqrgIdx = 0; u8dqrgIdx < RX_PHASE_DQ_GRP; u8dqrgIdx++) {
			...
			pr_crit("[%s] DDRPHY_READ[0x%08x] = 0x%08x\n",
				__func__, u32addr_rxdqdly, u32val_rxdqdly);
			if (u8dqrgIdx == 0)
				pr_crit("[%s][RX][ORIGINAL_VALUE][DQ] 0x%08x\n",
				__func__, (u32)(u32val_rxdqdly_target[0] & RX_PHASE_DQ_MSK));
		}
	}
	...
	case SHIFT_L: //decrease phase
		for (u8dqrgIdx = 0; u8dqrgIdx < RX_PHASE_DQ_GRP; u8dqrgIdx++) {
			for (i = 0 ; i < RX_PHASE_DQ_GRP; i++) {
				...
				if ((u8dqrgIdx == 0) && (i == 0))
					pr_crit("[%s][RX][CHANGE_VALUE][DQ] 0x%08x\n",
						__func__, (u32)u8val_dqtgt[0]);
			}
...
static u8 dramc_rxwincenter_run_phase_DQS(u8 u8channel, u8 u8rank, u8 u8byte,
	u8 u8dir, u8 u8changedelta, u32 u32ta2looptimes)
{
	//phase dqs
	u32 u32addr_rxdqsdly = 0, u32val_rxdqsdly = 0;
	...
	pr_crit("\n\n########################\n");
	pr_crit("Run dramc_rxwincenter for Phase DQS\n");
	pr_crit("########################\n\n");
	...
	if ((u8dir == SHIFT_L_RX_DQS) || (u8dir == SHIFT_R_RX_DQS)) {
		u32addr_rxdqsdly = TransAddr_ddrphy(
			DDRPHY_REG_SHU_R0_B0_RXDLY5_Ofst, u8channel, u8rank, u8byte);
		u32val_rxdqsdly = SHM_DDRPHY_RD(u32addr_rxdqsdly);
		u32val_rxdqsdly_target =
			(u32val_rxdqsdly & RX_PHASE_DQS_MSK_R) >> RX_PHASE_DQS_SFT_R;
		pr_crit("[%s] DDRPHY_READ[0x%08x] = 0x%08x\n",
			__func__, u32addr_rxdqsdly, u32val_rxdqsdly);
		pr_crit("[%s][RX][ORIGINAL_VALUE][DQS] 0x%08x\n", __func__, u32val_rxdqsdly_target);
	}
	...
	case SHIFT_L_RX_DQS: //decrease phase DQS
		...
		pr_crit("[%s][RX][CHANGE_VALUE][DQS] 0x%08x\n", __func__, u32val_rxdqsdly_target);
		u32val_rxdqsdly_target = (u32val_rxdqsdly_target << RX_PHASE_DQS_SFT_R)
			| (u32val_rxdqsdly_target << RX_PHASE_DQS_SFT_F);
...
static void dramc_rxwincenter_run_phase_perbit_preset(u8 u8channel,
	u8 u8rank, u8 u8byte, u8 u8dir)
{
	//phase rxdvs2
	u32 u32addr_rxdvs2 = 0, u32val_rxdvs2 = 0;

	//(1) get rxdvs2 setting
	u32addr_rxdvs2 = TransAddr_ddrphy(
		DDRPHY_REG_RK_B0_RXDVS2_Ofst, u8channel, u8rank, u8byte);
	u32val_rxdvs2 = SHM_DDRPHY_RD(u32addr_rxdvs2);
	pr_err("[%s] DDRPHY_READ[0x%08x] = 0x%08x\n",
	...
	//(2) reset rxdvs2 to direct RG mode
	if ((u8dir >= SHIFT_L_B_RISE) && (u8dir <= SHIFT_R_B_FALL)) {
		u32val_rxdvs2 &= (~RX_DVS2_MOD_MSK);
		SHM_DDRPHY_WR(u32val_rxdvs2, u32addr_rxdvs2);
		pr_err("[%s] DDRPHY_WRITE[0x%08x] = 0x%08x\n",
			__func__, u32addr_rxdvs2, u32val_rxdvs2);
		u32val_rxdvs2 |= RX_DVS2_MOD_FDLY;
		SHM_DDRPHY_WR(u32val_rxdvs2, u32addr_rxdvs2);
		pr_err("[%s] DDRPHY_WRITE[0x%08x] = 0x%08x\n",
			__func__, u32addr_rxdvs2, u32val_rxdvs2);
		u32val_rxdvs2 &= (~RX_DVS2_MOD_FDLY);
	...
	//(3) set rxdvs2 to sw mode
	u32val_rxdvs2 = (u32val_rxdvs2 & (~RX_DVS2_MOD_MSK)) | RX_DVS2_MOD_SW;
	SHM_DDRPHY_WR(u32val_rxdvs2, u32addr_rxdvs2);
	pr_err("[%s] DDRPHY_WRITE[0x%08x] = 0x%08x\n",
		__func__, u32addr_rxdvs2, u32val_rxdvs2);

	//(4) set rxdvs2 to rising and falling separated mode
	if ((u8dir >= SHIFT_L_B_RISE) && (u8dir <= SHIFT_R_B_FALL))
		u32val_rxdvs2 |= RX_DVS2_MOD_FDLY;
	else // set rxdvs2 to rising and falling same mode
		u32val_rxdvs2 &= (~RX_DVS2_MOD_FDLY);
	SHM_DDRPHY_WR(u32val_rxdvs2, u32addr_rxdvs2);
	pr_err("[%s] DDRPHY_WRITE[0x%08x] = 0x%08x\n",
		__func__, u32addr_rxdvs2, u32val_rxdvs2);
...
static u8 dramc_rxwincenter_run_phase_perbit_chgrise(u8 u8dir,
	u8 u8changedelta, u8 u8BitIdx, u32 u32valtarget)
{
	u8 u8val_rxdqpbt_rise = 0;
	u32 u32Shift_rise = 0;

	// get shift value for each bit index
	u32Shift_rise =
		RX_PHASE_DQ_SFT*(((u8BitIdx%RX_RF_RGP_NUM) == 0) ? RX_RF_RGA_0 : RX_RF_RGA_2);
	u8val_rxdqpbt_rise = (u8)((u32valtarget >> u32Shift_rise) & RX_PHASE_DQ_MSK);
	if ((u8dir == SHIFT_L_B_RISE) || (u8dir == SHIFT_R_B_RISE)
		|| (u8dir == SHIFT_L_B_RFSM) || (u8dir == SHIFT_R_B_RFSM))
		pr_crit("[%s][RX][ORIGINAL_VALUE][DQ_Pb_R] 0x%08x\n",
			__func__, (u32)u8val_rxdqpbt_rise);

	// case for decrease (L)
	if ((u8dir == SHIFT_L_B_RISE) || (u8dir == SHIFT_L_B_RFSM)) {
		if (u8val_rxdqpbt_rise < (u8)u8changedelta) {
			pr_crit("[%s][phase] RX Phase DQ_Bit[%d] Rising touch MIN value 0x00 !!!\n",
				__func__, u8BitIdx);
			u8val_rxdqpbt_rise = 0;
		} else {
			u8val_rxdqpbt_rise -= (u8)u8changedelta;
		}
	} else { // case for increase (R)
		if ((u32)(u8val_rxdqpbt_rise+u8changedelta) > RX_PHASE_DQ_MAX) {
			pr_crit("[%s][phase] RX Phase DQ_Bit[%d] Rising touch MAX value 0x%08x !!!\n",
				__func__, u8BitIdx, RX_PHASE_DQ_MAX);
			u8val_rxdqpbt_rise = RX_PHASE_DQ_MAX;
		} else {
			u8val_rxdqpbt_rise += (u8)u8changedelta;
		}
	}
	return u8val_rxdqpbt_rise;
}

static u8 dramc_rxwincenter_run_phase_perbit_chgfall(u8 u8dir,
	u8 u8changedelta, u8 u8BitIdx, u32 u32valtarget)
{
	u8 u8val_rxdqpbt_fall = 0;
	u32 u32Shift_fall = 0;

	// get shift value for each bit index
	u32Shift_fall =
		RX_PHASE_DQ_SFT*(((u8BitIdx%RX_RF_RGP_NUM) == 0) ? RX_RF_RGA_1 : RX_RF_RGA_3);
	u8val_rxdqpbt_fall = (u8)((u32valtarget >> u32Shift_fall) & RX_PHASE_DQ_MSK);
	if ((u8dir == SHIFT_L_B_FALL) || (u8dir == SHIFT_R_B_FALL)
		|| (u8dir == SHIFT_L_B_RFSM) || (u8dir == SHIFT_R_B_RFSM))
		pr_crit("[%s][RX][ORIGINAL_VALUE][DQ_Pb_F] 0x%08x\n",
			__func__, (u32)u8val_rxdqpbt_fall);

	// case for decrease (L)
	if ((u8dir == SHIFT_L_B_FALL) || (u8dir == SHIFT_L_B_RFSM)) {
		if (u8val_rxdqpbt_fall < (u8)u8changedelta) {
			pr_crit("[%s][phase] RX Phase DQ_Bit[%d] Falling touch MIN value 0x00 !!!\n",
				__func__, u8BitIdx);
			u8val_rxdqpbt_fall = 0;
		} else {
			u8val_rxdqpbt_fall -= (u8)u8changedelta;
		}
	} else { // case for increase (R)
		if ((u32)(u8val_rxdqpbt_fall+u8changedelta) > RX_PHASE_DQ_MAX) {
			pr_crit("[%s][phase] RX Phase DQ_Bit[%d] Falling touch MAX value 0x%08x !!!\n",
				__func__, u8BitIdx, RX_PHASE_DQ_MAX);
			u8val_rxdqpbt_fall = RX_PHASE_DQ_MAX;
		} else {
			u8val_rxdqpbt_fall += (u8)u8changedelta;
		}
	}
	return u8val_rxdqpbt_fall;

}

static u32 dramc_rxwincenter_run_phase_perbit_chgval(u8 u8dir,
	u8 u8changedelta, u8 u8BitIdx, u32 u32valtarget)
{
	u8 u8val_rxdqpbt_rise = 0, u8val_rxdqpbt_fall = 0;
	u32 u32Shift_rise = 0, u32Shift_fall = 0;

	// process change value for rising
	if ((u8dir == SHIFT_L_B_RISE) || (u8dir == SHIFT_R_B_RISE)
		|| (u8dir == SHIFT_L_B_RFSM) || (u8dir == SHIFT_R_B_RFSM))
		u8val_rxdqpbt_rise = dramc_rxwincenter_run_phase_perbit_chgrise(
		u8dir, u8changedelta, u8BitIdx, u32valtarget);
	// process change value for falling
	if ((u8dir == SHIFT_L_B_FALL) || (u8dir == SHIFT_R_B_FALL)
		|| (u8dir == SHIFT_L_B_RFSM) || (u8dir == SHIFT_R_B_RFSM))
		u8val_rxdqpbt_fall = dramc_rxwincenter_run_phase_perbit_chgfall(
		u8dir, u8changedelta, u8BitIdx, u32valtarget);

	// update rising value
	if ((u8dir == SHIFT_L_B_RISE) || (u8dir == SHIFT_R_B_RISE)
		|| (u8dir == SHIFT_L_B_RFSM) || (u8dir == SHIFT_R_B_RFSM)) {
		u32valtarget &= (~(RX_PHASE_DQ_MSK << u32Shift_rise));
		u32valtarget |= (u32)(u8val_rxdqpbt_rise << u32Shift_rise);
		pr_crit("[%s][RX][CHANGE_VALUE][DQ_Pb_R] 0x%08x\n",
			__func__, (u32)u8val_rxdqpbt_rise);
	}
	// update falling value
	if ((u8dir == SHIFT_L_B_FALL) || (u8dir == SHIFT_R_B_FALL)
		|| (u8dir == SHIFT_L_B_RFSM) || (u8dir == SHIFT_R_B_RFSM)) {
		u32valtarget &= (~(RX_PHASE_DQ_MSK << u32Shift_fall));
		u32valtarget |= (u32)(u8val_rxdqpbt_fall << u32Shift_fall);
		pr_crit("[%s][RX][CHANGE_VALUE][DQ_Pb_F] 0x%08x\n",
			__func__, (u32)u8val_rxdqpbt_fall);
	}
	return u32valtarget;
}

static u8 dramc_rxwincenter_run_phase_perbit(u8 u8channel, u8 u8rank, u8 u8byte,
	u8 u8dir, u8 u8changedelta, u32 u32ta2looptimes, u8 u8BitIdx)
{
	//phase rxdvs0
	u32 u32addr_rxdvs0 = 0, u32val_rxdvs0 = 0;
	//phase dq
	u32 u32addr_rxdqdly = 0;
	u32 u32addr_rxdqpbt_target = 0, u32val_rxdqpbt_target = 0;

	pr_crit("\n\n########################\n");
	pr_crit("Run dramc_rxwincenter for Phase DQ Perbit\n");
	pr_crit("########################\n\n");
	...
	// preset for rxdvs2
	dramc_rxwincenter_run_phase_perbit_preset(u8channel, u8rank, u8byte, u8dir);

	//################################################
	// configure address offset
	//################################################
	u32addr_rxdqdly = DDRPHY_REG_SHU_R0_B0_RXDLY0_Ofst;
	u32addr_rxdqdly += RX_RF_RGA_NUM*(u8BitIdx/RX_RF_RGP_NUM);
	u32addr_rxdqpbt_target = TransAddr_ddrphy(
		u32addr_rxdqdly, u8channel, u8rank, u8byte);
	u32val_rxdqpbt_target = SHM_DDRPHY_RD(u32addr_rxdqpbt_target);
	pr_crit("[%s] DDRPHY_READ[0x%08x] = 0x%08x\n",
		__func__, u32addr_rxdqpbt_target, u32val_rxdqpbt_target);

	//################################################
	//process phase
	//################################################
	// get changed target value
	u32val_rxdqpbt_target = dramc_rxwincenter_run_phase_perbit_chgval(
		u8dir, u8changedelta, u8BitIdx, u32val_rxdqpbt_target);
	SHM_DDRPHY_WR(u32val_rxdqpbt_target, u32addr_rxdqpbt_target);
	pr_crit("[%s] DDRPHY_WRITE[0x%08x] = 0x%08x\n",
		__func__, u32addr_rxdqpbt_target, u32val_rxdqpbt_target);
	...
	u8 u8dir, u8 u8changedelta, u32 u32ta2looptimes, u8 u8BitIdx)
{
	...
	else if ((u8dir >= SHIFT_L_B_RISE) && (u8dir <= SHIFT_R_B_RFSM))
		u8Result = dramc_rxwincenter_run_phase_perbit(u8channel, u8rank, u8byte,
			u8dir, u8changedelta, u32ta2looptimes, u8BitIdx);
	else if ((u8dir == SHIFT_L_RX_DQS) || (u8dir == SHIFT_R_RX_DQS))
		u8Result = dramc_rxwincenter_run_phase_DQS(u8channel, u8rank, u8byte,
			u8dir, u8changedelta, u32ta2looptimes);
	else //rx phase
...
static ssize_t dramc_rxwincenter_store(struct device_driver *drv, const char *buf, size_t count)
{
	...
	count_sscan = sscanf(buf, "%d %d %d %d %d %d %d", &u32RxWinChannel, &u32RxWinRank,
	...
	if (count_sscan == 7) {
		if (u32RxWinChangeDelta != WIN_MAX) {
			...
			case SHIFT_L_B_RISE:
				str_dir = "[L]: decrease rx dq phase rising"; break;
			case SHIFT_R_B_RISE:
				str_dir = "[R]: increase rx dq phase rising"; break;
			case SHIFT_L_B_FALL:
				str_dir = "[L]: decrease rx dq phase falling"; break;
			case SHIFT_R_B_FALL:
				str_dir = "[R]: increase rx dq phase falling"; break;
			case SHIFT_L_B_RFSM:
				str_dir = "[L]: decrease rx dq phase rising+falling"; break;
			case SHIFT_R_B_RFSM:
				str_dir = "[R]: increase rx dq phase rising+falling"; break;
			default:
			...
			pr_crit("Direction = %d (%s) Bit_%d\n	Delta = %d\n	TA2LoopTimes = %d\n\n",
				u32RxWinDirection, str_dir, u32RxWinBit,
...
{
	u8 u8changedelta = 0, u8Ta2Result = 0, u8BitIdx = 0;
	u32 u32ta2looptimes;
	...
	while (1) {
		u8Ta2Result = dramc_rxwincenter_run(
			u8channel, u8rank, u8byte, u8dir, u8changedelta, u32ta2looptimes, u8BitIdx);
		if (u8Ta2Result == 0)
			return 0;
...
static ssize_t dramc_rxshmoo_store(struct device_driver *drv, const char *buf, size_t count)
{
	...
	count_sscan = sscanf(buf, "%d %d %d %d %d %d %d", &u32RxShmChannel, &u32RxShmRank,
	...
	if (count_sscan == 7) {
		...
		pr_crit("Direction = %d (%s) Bit_%d\n	DelayTime(sec) = %d\n	TA2LoopTimes = %d\n\n",
			u32RxShmDirection, str_dir, u32RxShmBit, u32RxShmDelaysec,
			u32RxShmTA2LoopTimes);
	...
	u8 u8dir, u8 u8mode, u8 u8changedelta, u32 u32ta2looptimes)
{
	...
	if ((u8dir == SHIFT_D) || (u8dir == SHIFT_U)) {
		...
		pr_crit("[%s][TX][ORIGINAL_VALUE][VREF][R%d] 0x%08x\n",
			__func__, s32val_txvref_range, (u32)s32val_txvref_vref);
	}
	...
	case SHIFT_D: //decrease vref
		...
		pr_crit("[%s][TX][UPDATE_VALUE][VREF][R%d] 0x%08x\n",
			__func__, s32val_txvref_range, (u32)s32val_txvref_vref);
	...
	case SHIFT_U: //increase vref
		...
		pr_crit("[%s][TX][UPDATE_VALUE][VREF][R%d] 0x%08x\n",
			__func__, (int)s32val_txvref_range, (u32)s32val_txvref_vref);
	...
	u8 u8dir, u8 u8mode, u8 u8changedelta, u32 u32ta2looptimes)
{
	...
	if ((u8dir == SHIFT_L) || (u8dir == SHIFT_R)) {
		...
		pr_crit("[%s][TX][ORIGINAL_VALUE][PHASE] 0x%08x\n",
			__func__, s32val_txphase_update);
	}
	...
	case SHIFT_L: //decrease phase
		...
		pr_crit("[%s][TX][CHANGE_VALUE][PHASE] 0x%08x\n", __func__, u32val_txphase_target);
...
#if defined(CONFIG_MTK_DRAMC_CA)
#if DRAMC_TA_TEST_FLOW
static u32 u32TeeRefRate;
static u32 u32TeeSpan;
static u32 u32TeeStep;

static ssize_t dramc_TEEC_Test_show(struct device_driver *drv, char *buf)
{

	pr_crit("[Dramc TEE] Test!!!\n");
	if (MTK_DRAMC_TEEC_TA_Open() != TA_SUCCESS)
		pr_crit("DRAMC TA Open fail\n");
	else
		pr_crit("DRAMC TA Open pass\n");

	return 0;
}

static ssize_t dramc_TEEC_Test_store(struct device_driver *drv, const char *buf, size_t count)
{
	size_t count_sscan;

	count_sscan = sscanf(buf, "%d %d %d", &u32TeeRefRate,
						&u32TeeSpan, &u32TeeStep);

	if (count_sscan == 3) {
		if (MTK_DRAMC_TEEC_Set_Ref_Rate(u32TeeRefRate)
				!= TA_SUCCESS) {
			pr_err("Ref rate adjust fail\n"
					"Rate: %x\n"
					, u32TeeRefRate);
		}

		if (MTK_DRAMC_TEEC_Set_SSC(u32TeeSpan, u32TeeStep)
				!= TA_SUCCESS) {
			pr_err("SSC adjust fail\n"
					"Span:%x Step:%x\n",
					u32TeeSpan, u32TeeStep);
		}

	} else {
		pr_crit("[Dramc TEE] Wrong Argument!!!\n");
	}
	return count;
}
static DRIVER_ATTR_RW(dramc_TEEC_Test);
#endif
#endif
...
#if defined(CONFIG_MTK_DRAMC_CA)
#if DRAMC_TA_TEST_FLOW
	err = driver_create_file(drv, &driver_attr_dramc_TEEC_Test);
	if (err) {
		pr_notice("[MTK_DRAMC]driver_attr_dramc_TEEC_Test create attribute error\n");
		return err;
	}
#endif
#endif
...
static int mtk_dramc_probe(struct platform_device *pdev)
{
	...
	//struct mtk_dramc_data *dramc;
	u32 tmp[MTK_DRAMC_CONF_NUM];
	u32 tmp2[MTK_TEMP_THRESHOLD_NUM];
	int ret;
	...
	ret = of_property_read_u32_array(dev->of_node,
					 "mediatek,dramc-config",
					 tmp, ARRAY_SIZE(tmp));
	if (ret) {
		dev_err(dev, "dramc config not found\n");
		return ret;
	}

	dramc->dref_en = tmp[0];
	dramc->dram_pkg = tmp[1];
	dramc->chipid = tmp[2];

	//pr_crit("dramc config 0 = %lu 1 = %lu\n",
		//dramc->dref_en, dramc->dram_pkg);

	ret = of_property_read_u32_array(dev->of_node,
					 "mediatek,temp-threshold",
					 tmp2, ARRAY_SIZE(tmp2));
	if (ret) {
		dev_err(dev, "temp-threshold not found\n");
		return ret;
	}
	dramc->iHigh = tmp2[DRAM_DREF_IHIGH];
	dramc->dHigh = tmp2[DRAM_DREF_DHIGH];
	dramc->iLow = tmp2[DRAM_DREF_ILOW];
	dramc->dLow = tmp2[DRAM_DREF_DLOW];

	//pr_crit("temp threshold 0 = %d 1 = %d 2 = %d 3 = %d\n",
		//dramc->iHigh, dramc->iLow, dramc->dHigh, dramc->dLow);
	//pr_crit("chipid = %lu\n", dramc->chipid);
...
static int __init mtk_dramc_init(void)
{
	...
	if (dramc == NULL)
		dramc = kmalloc(sizeof(struct mtk_dramc_data), GFP_KERNEL);
	if (dramc == NULL)
		return -ENOMEM;
	...
	u32DREFEn = dramc->dref_en;
	u16DramcChipID = dramc->chipid;//pm_top_get_chip_id();
	...
	if ((u16DramcChipID == CHIP_ID_MT5879)
		...
		if (g_u16VBE_Code_FT == 0) {
			g_u16VBE_Code_FT = MT5873_TS_VBE_CODE_FT_DEFAULT;
			pr_err("VBE_Code_FT no trim\n");
		}
		if (g_u16TA_Code == 0) {
			g_u16TA_Code = MT5873_TS_TA_CODE_DEFAULT;
			pr_err("TA_Code no trim\n");
		}
	...
	//Idle cnt thread
	pDramcRes->bIdleCntCreateTaskFlag = 0;
	pDramcRes->bIdleCntTaskProcFlag = 0;
	pDramcRes->sIdleCntPollingTaskID = -1;
	...
	if (!pDramcRes->bIdleCntCreateTaskFlag) {
		if (Dramc_Idle_Cnt_Task_Init())
			pDramcRes->bIdleCntCreateTaskFlag = 1;
