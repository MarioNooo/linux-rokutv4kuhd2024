...
static void mtk_dtv_gmac_phy_hw_init_7_12(struct net_device *ndev, bool is_suspend)
{
	...
	if (!is_suspend || priv->is_from_phy_power_down) {
		//wriu     0x0422a1             0x00
		reg_read = 0x00;
		mtk_writeb(REG_OFFSET_50_H(BANK_BASE_ALBANY_2), reg_read);
	}
...
static void mtk_dtv_gmac_hw_power_on_clk(struct net_device *ndev, bool is_suspend)
{
	...
	priv->is_from_phy_power_down = false;
}

static void mtk_dtv_gmac_hw_power_down_clk_7_12(struct net_device *ndev)
{
	struct mtk_dtv_gmac_private *priv = netdev_priv(ndev);
	...
	priv->is_from_phy_power_down = true;
}

static void mtk_dtv_gmac_hw_power_down_clk_22(struct net_device *ndev)
{
	struct mtk_dtv_gmac_private *priv = netdev_priv(ndev);
	...
	mtk_writeb(REG_OFFSET_7E_H(BANK_BASE_ALBANY_1), 0x01);
	//wriu     0x0422a1             0xb0
	mtk_writeb(REG_OFFSET_50_H(BANK_BASE_ALBANY_2), 0xb0);
	//wriu     0x0421cc             0x50
	mtk_writeb(REG_OFFSET_66_L(BANK_BASE_ALBANY_1), 0x50);
	//wriu     0x0421bb             0xc4
	mtk_writeb(REG_OFFSET_5D_H(BANK_BASE_ALBANY_1), 0xc4);
...
u32 _mtk_dtv_gmac_efuse_read(struct net_device *ndev, u16 addr)
{
	struct mtk_dtv_gmac_private *priv = netdev_priv(ndev);
	u16 efuse_addr = addr;
	u32 efuse_value = 0;
	...
	mtk_writew(REG_OFFSET_28_L(BANK_BASE_EFUSE), efuse_addr);
	...
	do {
		...
		efuse_addr = mtk_readw(REG_OFFSET_28_L(BANK_BASE_EFUSE));
		retry++;
		if (retry >= EFUSE_RETRY_MAX) {
			dev_err(priv->dev, "read efuse failed, retry=%d\n",
				retry);
			return 0;
		}
	...
	/* get efuse value */
	efuse_value = (mtk_readw(REG_OFFSET_2D_L(BANK_BASE_EFUSE)) << 16) |
		      (mtk_readw(REG_OFFSET_2C_L(BANK_BASE_EFUSE)));

	return efuse_value;
}

static void mtk_dtv_gmac_tx_swing_init(struct net_device *ndev)
{
	struct mtk_dtv_gmac_private *priv = netdev_priv(ndev);
	u32 eth_efuse;
	...
	if (priv->tx_swing_level) {
		eth_efuse = _mtk_dtv_gmac_efuse_read(ndev, EFUSE_ETH_ATOP_ADDR);
		dev_info(priv->dev, "eth_efuse=0x%x, tx_swing_level=0x%x\n",
		...
		if (eth_efuse & EFUSE_ETH_ATOP_ACTIVE) {
			...
			reg_write |= (mtk_readb(REG_OFFSET_34_L(BANK_BASE_ALBANY_2)) &
				      ~EFUSE_ETH_ATOP_CUR_100T_MASK);
			mtk_writeb(REG_OFFSET_34_L(BANK_BASE_ALBANY_2), reg_write);

			dev_info(priv->dev, "tx swing read=0x%x, level=0x%x, write=0x%x\n",
...
static void mtk_dtv_gmac_mem_init(struct net_device *ndev)
{
	...
	rx_desc_dma_addr = priv->rx_desc_dma_addr - priv->miu0_base;
	mtk_writel(BANK_BASE_MAC_0 + GMAC0_REG_0030_X32_GMAC0, rx_desc_dma_addr);

	/* set dma_addr over 32bits */
	msb_rx_desc = ((rx_desc_dma_addr & 0x300000000) >> 32);
	if (msb_rx_desc) {
		if (priv->mac_type == MAC_TYPE_EMAC) {
			reg_temp = mtk_readb(REG_OFFSET_23_H(BANK_BASE_MAC_1));
			mtk_writeb(REG_OFFSET_23_H(BANK_BASE_MAC_1),
				   ((reg_temp & ~(0x0C)) | (msb_rx_desc << 2)));
		} else {
			reg_temp = mtk_readb(REG_OFFSET_09_H(BANK_BASE_MAC_4));
			mtk_writeb(REG_OFFSET_09_H(BANK_BASE_MAC_4),
				   ((reg_temp & ~(0x30)) | (msb_rx_desc << 4)));
		}
	}
...
int mtk_dtv_gmac_hw_init(struct net_device *ndev, bool is_suspend)
{
	...
	mtk_dtv_gmac_tx_swing_init(ndev);
...
static netdev_tx_t mtk_dtv_gmac_start_xmit(struct sk_buff *skb, struct net_device *ndev)
{
	...
	tx_memcpy_dma_addr = priv->tx_memcpy_dma_addr - priv->miu0_base;
	...
	/* set dma_addr over 32bits */
	msb_tx_addr = ((dma_addr & 0x300000000) >> 32);
	if (msb_tx_addr) {
		if (priv->mac_type == MAC_TYPE_EMAC) {
			reg_temp = mtk_readb(REG_OFFSET_23_H(BANK_BASE_MAC_1));
			mtk_writeb(REG_OFFSET_23_H(BANK_BASE_MAC_1),
				   ((reg_temp & ~(0x30)) | (msb_tx_addr << 4)));
		} else {
			reg_temp = mtk_readb(REG_OFFSET_09_H(BANK_BASE_MAC_4));
			mtk_writeb(REG_OFFSET_09_H(BANK_BASE_MAC_4),
				   ((reg_temp & ~(0xc0)) | (msb_tx_addr << 6)));
		}
	}
...
static int _mtk_dtv_gmac_woc_sram_write(struct net_device *ndev)
{
	...
	for (i = 0; i < WOC_PATTERN_BYTES_MAX; i++) {
		...
		if (i == ETH_PROTOCOL_NUM_OFFSET ||
			...
			mtk_writeb(REG_OFFSET_0B_L(BANK_BASE_ALBANY_3),
				   (priv->woc_ipv4_enable_bit & 0xff));
			mtk_writeb(REG_OFFSET_0B_H(BANK_BASE_ALBANY_3),
				   ((priv->woc_ipv4_enable_bit & 0xff00) >> 8));
			val = mtk_readb(REG_OFFSET_0C_L(BANK_BASE_ALBANY_3));
			val |= ((priv->woc_ipv4_enable_bit & 0x0f0000) >> 16);
			mtk_writeb(REG_OFFSET_0C_L(BANK_BASE_ALBANY_3), val);
		} else if (i == ETH_PROT_IPV6_NUM_OFFSET ||
			   i == ETH_PROT_IPV6_DEST_PORT_H_OFFSET ||
			   i == ETH_PROT_IPV6_DEST_PORT_L_OFFSET) {
			/* enable all pattern byte check */
			mtk_writeb(REG_OFFSET_0B_L(BANK_BASE_ALBANY_3),
				   (priv->woc_ipv6_enable_bit & 0xff));
			mtk_writeb(REG_OFFSET_0B_H(BANK_BASE_ALBANY_3),
				   ((priv->woc_ipv6_enable_bit & 0xff00) >> 8));
			val = mtk_readb(REG_OFFSET_0C_L(BANK_BASE_ALBANY_3));
			val |= ((priv->woc_ipv6_enable_bit & 0x0f0000) >> 16);
			mtk_writeb(REG_OFFSET_0C_L(BANK_BASE_ALBANY_3), val);
		} else {
					       ...
					       char protocol_type, int *port_array, int count)
{
	...
	for (i = 0; i < count; i++) {
		...
		filter_enable_bit |= (1 << priv->woc_filter_index);
	...
	priv->woc_ipv4_enable_bit |= filter_enable_bit;

	_mtk_dtv_gmac_woc_sram_write(ndev);

	priv->is_woc = true;

	ret = priv->woc_filter_index - count;
...
static int _mtk_dtv_gmac_woc_protocol_ipv6_port_set(struct net_device *ndev,
						    char protocol_type, int *port_array, int count)
{
	struct mtk_dtv_gmac_private *priv = netdev_priv(ndev);
	int i, ret = 0;
	...
	for (i = 0; i < count; i++) {
		/* byte23 is udp/tcp protocol number. */
		priv->woc_sram_pattern[priv->woc_filter_index][ETH_PROT_IPV6_NUM_OFFSET] =
			protocol_type;

		/* byte56 and byte57 is udp/tcp ipv6 destination port. */
		priv->woc_sram_pattern[priv->woc_filter_index][ETH_PROT_IPV6_DEST_PORT_H_OFFSET] =
			(char)(port_array[i] >> 8);
		priv->woc_sram_pattern[priv->woc_filter_index][ETH_PROT_IPV6_DEST_PORT_L_OFFSET] =
			(char)(port_array[i] & 0xff);
	...
	priv->woc_ipv6_enable_bit |= filter_enable_bit;
...
//		mtk_writeb(REG_OFFSET_00_H(BANK_BASE_ALBANY_3), val);
		...
		/* wait for hw autoload ready, at least delay 1 ms */
		/* 100t mode delay 65 us */
		/* 10t mode delay 325 us */
		mdelay(1);
	}
}
...
static int mtk_dtv_gmac_set_woc_ioctl(struct net_device *ndev, struct ifreq *req, bool is_ipv6)
{
	struct mtk_dtv_gmac_private *priv = netdev_priv(ndev);
	...
	if (is_compat_task()) {
		...
		if (is_ipv6)
			ret = _mtk_dtv_gmac_woc_protocol_ipv6_port_set(ndev,
...
static int mtk_dtv_gmac_ioctl(struct net_device *ndev, struct ifreq *req, int cmd)
{
	...
	case SIOC_SET_WOC_CMD:
		ret = mtk_dtv_gmac_set_woc_ioctl(ndev, req, false);

		return ret;

	case SIOC_SET_WOC_IPV6_CMD:
		ret = mtk_dtv_gmac_set_woc_ioctl(ndev, req, true);
	...
	case SIOC_CLR_WOC_CMD:
		...
		priv->woc_ipv4_enable_bit = 0;
		priv->woc_ipv6_enable_bit = 0;
		priv->is_woc = false;
		mutex_unlock(&priv->woc_lock);
		return 0;
	/* end of woc */
	...
	default:
#ifdef CONFIG_ROKU_NET_MII_IOCTL
...
static int mtk_dtv_gmac_mdio_read(struct net_device *ndev, int addr, int reg)
{
	...
	if (reg == MII_BMSR)
	{
		hcd_link_st_ok = reg_ops->read_phy(ndev, phy_addr, 0x21);
		if (hcd_link_st_ok & 0x100UL) {
			/* link down */
			value = reg_ops->read_phy(ndev, phy_addr, reg);
			value |= 0x4UL;
		}
	}
	else
...
static int mtk_dtv_gmac_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (priv->str_phy_power_down && phy_power_down) {
		reg_ops->phy_power_down(ndev);
...
static int mtk_dtv_gmac_resume(struct platform_device *pdev)
{
	...
	/* load packet to mac */
	_mtk_dtv_gmac_woc_auto_load_trigger(ndev);
...
static int mtk_dtv_gmac_reboot_handler(struct notifier_block *nb,
				       unsigned long event,
				       void *unused)
{
	struct mtk_dtv_gmac_private *priv = container_of(nb,
	...
	case SYS_POWER_OFF:
		if (priv->eth_drv_stage & ETH_DRV_STAGE_OPEN) {
			_mtk_dtv_gmac_wol_config(ndev);
			/* disable woc always, if system power off */
			_mtk_dtv_gmac_woc_config(ndev, false);
			dev_info(priv->dev, "gmac power off with eth open\n");
			if (!priv->is_wol)
				phy_power_down = true;
		} else {
			_mtk_dtv_gmac_woc_config(ndev, false);
			_mtk_dtv_gmac_wol_config_force_disable(ndev);
			dev_info(priv->dev, "gmac power off without eth open\n");
			phy_power_down = true;
		}
...
static int mtk_dtv_gmac_of_get_bank(struct net_device *ndev, struct device_node *np)
{
	...
	if (!priv->mac_bank_0 || !priv->mac_bank_1 ||
	    ...
	    !priv->mpll_bank || !priv->efuse_bank) {
		...
		dev_err(priv->dev, "mpll(0x%lx), efuse(0x%lx)\n",
			(unsigned long)priv->mpll_bank, (unsigned long)priv->efuse_bank);
		return -ENOMEM;
	}
	...
	dev_dbg(priv->dev, "tzpc_aid_0(0x%lx), pm_misc(0x%lx)\n",
		(unsigned long)priv->tzpc_aid_bank_0, (unsigned long)priv->pm_misc_bank);
	dev_dbg(priv->dev, "mpll(0x%lx), efuse(0x%lx)\n",
		(unsigned long)priv->mpll_bank, (unsigned long)priv->efuse_bank);

	reg_ops = kmalloc(sizeof(*reg_ops), GFP_ATOMIC);
	if (!reg_ops)
		return -ENOMEM;
...
static int mtk_dtv_gmac_of_get_memory_info(struct net_device *ndev)
{
	struct mtk_dtv_gmac_private *priv = netdev_priv(ndev);
	...
	if (p) {
		priv->miu0_base = be32_to_cpup(p);
		of_node_put(target_memory_np);
		p = NULL;
	} else {
		dev_err(priv->dev, "can not find cpu_emi0_base info\n");
		of_node_put(target_memory_np);
		return -EINVAL;
	}
...
static void mtk_dtv_gmac_of_get_cust(struct net_device *ndev, struct device_node *np)
{
	struct mtk_dtv_gmac_private *priv = netdev_priv(ndev);

	/* get tx swing property */
	if (of_property_read_u32(np, "tx-swing", &priv->tx_swing_level)) {
		priv->tx_swing_level = 0;
		dev_info(priv->dev, "set tx_swing_level to default value (%d)\n",
			 priv->tx_swing_level);
	} else {
		if (priv->tx_swing_level > TX_SWING_LEVEL_MAX) {
			dev_info(priv->dev, "bad tx_swing_level(%d), set it to default value 0\n",
				 priv->tx_swing_level);
			priv->tx_swing_level = 0;
		}
		dev_dbg(priv->dev, "tx_swing_level (%d)\n",
			priv->tx_swing_level);
	}

	/* get str power down */
	if (of_property_read_u32(np, "str-phy-power-down", &priv->str_phy_power_down)) {
		priv->str_phy_power_down = 0;
		dev_info(priv->dev, "gmac set str_phy_power_down to default value (%d)\n",
			 priv->str_phy_power_down);
	} else {
		dev_dbg(priv->dev, "gmac str_phy_power_down (%d)\n",
			priv->str_phy_power_down);
	}
}
...
static void mtk_dtv_gmac_of_get_misc(struct net_device *ndev, struct device_node *np)
{
	...
	if (of_property_read_u32(np, "threaded-irq", &get_val)) {
		/* set 0 to disable */
		priv->enable_threaded_irq = 0;
		dev_info(priv->dev, "set enable_threaded_irq to default value (%d)\n",
			 priv->enable_threaded_irq);
	} else {
		priv->enable_threaded_irq = get_val;
		dev_info(priv->dev, "get enable_threaded_irq value (%d)\n",
			 priv->enable_threaded_irq);
	}
	...
	case PHY_PROCESS_7_12:
		...
		reg_ops->phy_power_down = mtk_dtv_gmac_hw_power_down_clk_7_12;
		dev_info(priv->dev, "set phy init to 7_12\n");
		break;
	case PHY_PROCESS_22:
		reg_ops->phy_hw_init = mtk_dtv_gmac_phy_hw_init_22;
		reg_ops->phy_power_down = mtk_dtv_gmac_hw_power_down_clk_22;
	...
	mtk_dtv_gmac_of_get_cust(ndev, np);
}
...
static int mtk_dtv_gmac_probe(struct platform_device *pdev)
{
	...
	ret = mtk_dtv_gmac_of_get_memory_info(ndev);
	if (ret)
		goto err_iounmap;
...
#ifdef CONFIG_ROKU_NET_MII_IOCTL
	priv->mii.phy_id_mask = 0xFF;
	priv->mii.reg_num_mask = 0xFF;
#endif
	...
	/* register irq */
	if (priv->enable_threaded_irq == ENABLE_THREADED_IRQ &&
	    priv->phy_type == PHY_TYPE_INTERNAL) {
		dev_info(&pdev->dev, "used request_threaded_irq\n");
		if (request_threaded_irq(ndev->irq, NULL, mtk_dtv_gmac_isr,
					 /*SA_INTERRUPT*/IRQF_ONESHOT, ndev->name, ndev) != 0) {
			dev_err(&pdev->dev, "request threaded irq %d fail\n", ndev->irq);
			ret = -ENODEV;
			goto err_free_dma;
		}
	} else {
	...
	/* always disable woc/wol and clear irq status for power off on case */
	_mtk_dtv_gmac_woc_config(ndev, false);
	_mtk_dtv_gmac_wol_config_force_disable(ndev);
	_mtk_dtv_gmac_woc_wol_clear_irq(ndev);
	...
	priv->reboot_notifier.notifier_call = mtk_dtv_gmac_reboot_handler;
