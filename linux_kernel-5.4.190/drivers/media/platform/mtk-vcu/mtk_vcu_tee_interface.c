...
#define VCU_TEE_HDL_TO_ID(hdl)  (((hdl) >> 4) & 0xFFFFF)
...
static int _vcu_tee_invoke_command(struct mtk_vcu_tee_context *vcu_tee_ctx,
				   ...
				   u32 *return_origin,
				   struct tee_ioctl_invoke_arg *pre_arg,
				   struct vcu_teec_operation *pre_op)
{
	...
	memset(pre_arg, 0x0, sizeof(*pre_arg));
	memset(params, 0x0, sizeof(params));
	memset(pre_op, 0x0, sizeof(*pre_op));
	memset(shm, 0x0, sizeof(shm));

	pre_arg->num_params = VCU_TEE_PARAM_COUNT;
	pre_arg->session = act_tee_ctx->session_id;
	pre_arg->func = command_id;

	pre_op->paramTypes = TEEC_PARAM_TYPES(
	...
	res = _teec_pre_process_operation(act_tee_ctx, pre_op, params, shm);
	if (res != 0) {
		eorig = VCU_TEE_RTN_OP;
		goto out_free_temp_refs;
	}

	rc = tee_client_invoke_func(act_tee_ctx->tee_context, pre_arg, params);
	if (rc) {
	...
	res = pre_arg->ret;
	eorig = pre_arg->ret_origin;
	_teec_post_process_operation(pre_op, params, shm);
out_free_temp_refs:
	_teec_free_temp_refs(pre_op, shm);
...
static int _vdec_tee_cmd_handle_task(void *data)
{
	...
	struct vcu_teec_operation *invoke_op;
	int ret = 0;

	invoke_arg = kmalloc(sizeof(*invoke_arg), GFP_KERNEL);
	invoke_op = kmalloc(sizeof(*invoke_op), GFP_KERNEL);
...
static int _vcu_tee_start_cmd_task(struct mtk_vcu_tee_handler *tee_handler)
{
	...
	if (!cmd_task->task) {
		...
		cmd_task->task = kthread_run(_vdec_tee_cmd_handle_task,
					     tee_handler,
					     "vcu-tee-%05x",
					     VCU_TEE_HDL_TO_ID((uint32_t)(uintptr_t)tee_handler));
