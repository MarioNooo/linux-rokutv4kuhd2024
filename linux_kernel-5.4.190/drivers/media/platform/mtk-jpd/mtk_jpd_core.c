...
#define JPD_BAND_BYTES_PER_PIXEL    2
...
static const struct v4l2_ctrl_config jpd_v4l2_ctrl_configs[] = {
	 ...
	 },
	{
	 ...
	 .id = V4L2_CID_JPD_GET_BANDWIDTH,
	 .name = "JPD get bandwidth",
	 .min = 0,
	 .max = 0xFF,
	 .step = 1,
	 .def = 0,
	 .dims = {sizeof(struct v4l2_jpd_bandwidth_parameter)},
	 },
};
...
static int mtk_jpd_g_v_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	jpd_n_debug(LEVEL_ALWAYS, "V4L2_CID_JPD_GET_BANDWIDTH %d", ctrl->id);

	switch (ctrl->id) {
	case V4L2_CID_JPD_FRAME_METADATA_FD:
		break;
	case V4L2_CID_JPD_GET_BANDWIDTH:
		if (ctx->active) {
			struct v4l2_jpd_bandwidth_parameter *param = ctrl->p_new.p;

			jpd_n_debug(LEVEL_ALWAYS, "V4L2_CID_JPD_GET_BANDWIDTH");
			param->value = JPD_BAND_BYTES_PER_PIXEL;
		}
		break;
	default:
		ret = -EINVAL;
