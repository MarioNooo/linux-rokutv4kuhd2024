...
#define DEBUG_MODE_SCALE_1_1 0x1
#define DEBUG_MODE_SCALE_1_2 0x2
#define DEBUG_MODE_SCALE_1_4 0x4
#define DEBUG_MODE_SCALE_1_8 0x8
#define DEBUG_MODE_PAUSE_BEFORE_DECODE 0x10
#define DEBUG_MODE_PAUSE_AFTER_DECODE 0x20
#define DEBUG_MODE_DISABLE_PITCH 0x40

static int jpd_api_debug;
module_param(jpd_api_debug, int, 0644);
static int jpd_api_debug_mode;
module_param(jpd_api_debug_mode, int, 0644);
...
static bool _Mapi_NJPD_Parser_Get_Output_Resolution(int handle, u16 width, u16 height)
{
	...
	njpd_parser_debug(handle, LEVEL_API_DEBUG, "scale factor = %d", eFactor);

	if (jpd_api_debug_mode) {
		if (jpd_api_debug_mode & DEBUG_MODE_SCALE_1_1) {
			njpd_parser_debug(handle, LEVEL_API_DEBUG, "debug mode = %x, scale 1/1",
				jpd_api_debug_mode);
			eFactor = E_NJPD_API_DOWNSCALE_ORG;
		} else if (jpd_api_debug_mode & DEBUG_MODE_SCALE_1_2) {
			njpd_parser_debug(handle, LEVEL_API_DEBUG, "debug mode = %x, scale 1/2",
				jpd_api_debug_mode);
			eFactor = E_NJPD_API_DOWNSCALE_HALF;
		} else if (jpd_api_debug_mode & DEBUG_MODE_SCALE_1_4) {
			njpd_parser_debug(handle, LEVEL_API_DEBUG, "debug mode = %x, scale 1/4",
				jpd_api_debug_mode);
			eFactor = E_NJPD_API_DOWNSCALE_FOURTH;
		} else if (jpd_api_debug_mode & DEBUG_MODE_SCALE_1_8) {
			njpd_parser_debug(handle, LEVEL_API_DEBUG, "debug mode = %x, scale 1/8",
				jpd_api_debug_mode);
			eFactor = E_NJPD_API_DOWNSCALE_EIGHTH;
		}
	...
	if (eFactor == E_NJPD_API_DOWNSCALE_ORG) {
		...
		pstRect->x = 0;
		pstRect->y = 0;
		pstOutPic->width = NJPD_Align(width, E_NUM16);
		pstOutPic->height = height;
	} else {
		// avoid scaling pixel lost
		// ex: 4001/4 -> 1000 (ng)
		// (4001+(4-1))/4 -> 1001 (ok)
		pstRect->width = NJPD_Align(pstSOFInfo->u16OriginalImageXSize,
								gau8Scale_factor[eFactor])
								/ gau8Scale_factor[eFactor];
		pstRect->height = NJPD_Align(pstSOFInfo->u16OriginalImageYSize,
								gau8Scale_factor[eFactor])
								/ gau8Scale_factor[eFactor];
		pstRect->x = 0;
		pstRect->y = 0;
		pstOutPic->width = NJPD_Align(pstRect->width, E_NUM16);
		pstOutPic->height = height;
	}
...
static bool _Mapi_NJPD_Parser_Get_Input_Format(int handle)
{
	NJPD_SOFInfo *pstSOFInfo;
	...
	if (pstSOFInfo->u8ComponentsInFrame == 1) {
		// 4:0:0
		njpd_parser_debug(handle, LEVEL_API_DEBUG, "input format is 4:0:0");
		pstSOFInfo->u8InputColorFormat = E_JPEG_GRAYSCALE;
	} else if (pstSOFInfo->u8ComponentsInFrame == 0) {
		// unsupport
		njpd_parser_err(handle, "u8ComponentsInFrame is 0");
		return false;
	} else if (pstSOFInfo->u8ComponentHSamp[1] != 1 ||
				pstSOFInfo->u8ComponentVSamp[1] != 1 ||
				pstSOFInfo->u8ComponentVSamp[E_NUM2] != 1 ||
				pstSOFInfo->u8ComponentVSamp[E_NUM2] != 1) {
		// unsupport
		njpd_parser_err(handle,
		"input format is unsupported,h[1]=%d, v[1]=%d, h[2]=%d, v[2]=%d",
		pstSOFInfo->u8ComponentHSamp[1], pstSOFInfo->u8ComponentVSamp[1],
		pstSOFInfo->u8ComponentHSamp[E_NUM2], pstSOFInfo->u8ComponentVSamp[E_NUM2]);
		return false;
		...
		// 4:4:4
		njpd_parser_debug(handle, LEVEL_API_DEBUG, "input format is 4:4:4");
		pstSOFInfo->u8InputColorFormat = E_JPEG_YH1V1;
	} else if (pstSOFInfo->u8ComponentHSamp[0] == E_NUM2 ||
				pstSOFInfo->u8ComponentVSamp[0] == 1) {
		// 4:2:2
		pstSOFInfo->u8InputColorFormat = E_JPEG_YH2V1;
		njpd_parser_debug(handle, LEVEL_API_DEBUG, "input format is 4:2:2");
	} else if (pstSOFInfo->u8ComponentHSamp[0] == 1 ||
				pstSOFInfo->u8ComponentVSamp[0] == E_NUM2) {
		// 4:4:0
		pstSOFInfo->u8InputColorFormat = E_JPEG_YH1V2;
		njpd_parser_debug(handle, LEVEL_API_DEBUG, "input format is 4:4:0");
	} else if (pstSOFInfo->u8ComponentHSamp[0] == E_NUM4 ||
				pstSOFInfo->u8ComponentVSamp[0] == 1) {
		// 4:1:1
		pstSOFInfo->u8InputColorFormat = E_JPEG_YH4V1;
		njpd_parser_debug(handle, LEVEL_API_DEBUG, "input format is 4:1:1");
	} else if (pstSOFInfo->u8ComponentHSamp[0] == E_NUM2 ||
				pstSOFInfo->u8ComponentVSamp[0] == E_NUM2) {
		// 4:2:0
		pstSOFInfo->u8InputColorFormat = E_JPEG_YH2V2;
		njpd_parser_debug(handle, LEVEL_API_DEBUG, "input format is 4:2:0");
	} else {
		njpd_parser_err(handle, "input format is unknown, h[0]=%d, v[0]=%d",
		pstSOFInfo->u8ComponentHSamp[0], pstSOFInfo->u8ComponentVSamp[0]);
		return false;
	}
	return true;
}
...
static u8 _Mapi_NJPD_Read_SOF_Marker(int handle, u32 u32Left)
{
	...
	if (!_Mapi_NJPD_Parser_Get_Input_Format(handle))
		return false;
						   ...
						   NJPD_API_DecCmd *pDecCmd)
{
	...
	if (ret == E_NJPD_API_HEADER_INVALID) {
		...
		pDecCmd->pitch_width = gNJPDParserInfo[handle].output_res.width;
		...
		njpd_parser_debug(handle, LEVEL_API_DEBUG, "width:%d, pitch_width:%d, height:%d",
				  pDecCmd->width, pDecCmd->pitch_width, pDecCmd->height);
...
NJPD_API_Result MApi_NJPD_Decoder_Start(NJPD_API_DecCmd *pDecCmd, NJPD_API_Buf write_buf)
{
	...
	if ((pDecCmd->ROI_rect.width == 0) &&
		!(jpd_api_debug_mode & DEBUG_MODE_DISABLE_PITCH))
		MDrv_NJPD_SetPitch(pDecCmd->pitch_width);
	...
	if (jpd_api_debug_mode & DEBUG_MODE_PAUSE_BEFORE_DECODE)	{
		njpd_debug(LEVEL_API_TRACE, "debug mode = %x, stop before decode",
			jpd_api_debug_mode);
	} else {
...
NJPD_API_DecodeStatus MApi_NJPD_Decoder_WaitDone(void)
{
	...
	if (u16EventFlag & E_NJPD_EVENT_DEC_DONE) {
		...
		if (jpd_api_debug_mode & DEBUG_MODE_PAUSE_AFTER_DECODE) {
			njpd_debug(LEVEL_API_TRACE, "debug mode = %x, stop after decode",
				jpd_api_debug_mode);
			return E_NJPD_API_DEC_DECODING;
		} else {
