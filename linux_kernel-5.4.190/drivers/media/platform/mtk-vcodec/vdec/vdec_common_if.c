	...
	struct vdec_fb *fb, unsigned int *src_chg)
{
	...
	if (fb != NULL) {
		...
		if (fb->dma_general_buf != 0) {
			...
			inst->vsi->general_buf_va = (uintptr_t)fb->general_buf_va;
			mtk_vcodec_debug(inst, "get_mapped_fd fb->dma_general_buf = %p, mapped fd = %d, size = %zu, va = 0x%llx",
			    fb->dma_general_buf, inst->vsi->general_buf_fd,
			    fb->dma_general_buf->size, inst->vsi->general_buf_va);
		} else {
			fb->general_buf_fd = -1;
			inst->vsi->general_buf_fd = -1;
			inst->vsi->general_buf_size = 0;
			inst->vsi->general_buf_va = 0;
...
static void get_aspect_ratio(struct vdec_inst *inst, struct v4l2_fract *aspect_ratio)
{
	if (inst->vsi == NULL)
		return;

	inst->vcu.ctx = inst->ctx;
	memcpy(aspect_ratio, &inst->vsi->aspect_ratio, sizeof(struct v4l2_fract));
}
				...
				struct v4l2_vdec_color_desc *v4l2_color_desc)
{
	...
	v4l2_color_desc->hdr_type = color_desc->hdr_type;
}
...
{
	inst->vcu.ctx = inst->ctx;
	vcu_dec_query_cap(&inst->vcu, GET_PARAM_MAX_BUF_INFO,
					  max_buf_info);
}
#endif
	...
	enum vdec_set_param_type type, void *in)
{
	...
	case SET_PARAM_FIXED_MAX_FRAME_BUFFER:
