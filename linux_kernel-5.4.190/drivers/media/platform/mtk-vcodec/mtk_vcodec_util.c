...
#define VCODEC_RACING_DISP_FENCE_CNT 4
...
void mtk_vcodec_init_slice_info(struct mtk_vcodec_ctx *ctx, struct mtk_video_dec_buf *dst_buf_info)
{
	...
	if (ctx->dec_params.slice_count == 1 && ctx->dec_params.racing_display == true) {
		//racing display set 4 fenses to signal
		fence = mtk_vcodec_create_fence(VCODEC_RACING_DISP_FENCE_CNT);
	} else {
...
struct vdec_fb *mtk_vcodec_get_fb(struct mtk_vcodec_ctx *ctx)
{
	...
	if (dst_buf != NULL) {
		...
		/*
		 * In some case, user queue capture buffer with same index but
		 * different fd, this may cause vdec reference frame freed before
		 * vdec driver reference it.
		 * Here is temp solution :
		 * Add dma referenece count when vdec get empty buf,
		 * decrease reference count when buf free.
		 */

		/* add dma-buf ref cnt for reference frame */
		get_dma_buf(dst_buf->planes[0].dbuf);
		...
		/* store dma_buf for dma_buf_get when buf is freed*/
		dst_buf_info->dbuf_frame_buf = dst_buf->planes[0].dbuf;
...
int mtk_dma_sync_sg_range(const struct sg_table *sgt,
	struct device *dev, unsigned int size, unsigned int offset,
	enum dma_data_direction direction)
{
	struct sg_table *sgt_tmp;
	struct scatterlist *s_sgl, *d_sgl;
	unsigned int contig_size = 0;
	unsigned int contig_size_tmp = 0;
	int ret, i;
	...
	for_each_sg(sgt->sgl, s_sgl, sgt->orig_nents, i) {
		mtk_v4l2_debug(VCODEC_DBG_L6, "%d contig_size_tmp %d data_offset 0x%x bytesused %d.\n",
			i, contig_size_tmp, offset, size);
		if (contig_size_tmp >= size)
			break;
		contig_size_tmp += sg_dma_len(s_sgl);

		if (contig_size_tmp >= offset) {
			memcpy(d_sgl, s_sgl, sizeof(*s_sgl));
			contig_size += sg_dma_len(s_sgl);
			d_sgl = sg_next(d_sgl);
			sgt_tmp->nents++;
			mtk_v4l2_debug(VCODEC_DBG_L6, "%d contig_size %d data_offset 0x%x bytesused %d.\n",
				i, contig_size, offset, size);
		}
	}
...
void v4l_fill_mtk_fmtdesc(struct v4l2_fmtdesc *fmt)
{
	...
	case V4L2_PIX_FMT_MT21C:
		descr = " Mediatek compressed block mode"; break;
	case V4L2_PIX_FMT_MT21:
		descr = "MTK 8-bit block mode, two non-contiguous planes"; break;
	default:
