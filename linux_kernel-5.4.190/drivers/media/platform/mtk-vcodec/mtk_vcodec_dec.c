...
static struct vb2_buffer *get_free_buffer(struct mtk_vcodec_ctx *ctx)
{
	...
	if (dstbuf->used) {
		...
		/* decrease dma ref cnt if frame buffer won't be used as reference anymore */
		if (dstbuf->dbuf_frame_buf) {
			mtk_v4l2_debug(VCODEC_DBG_L6, "[%d] id %d dma_buf_put %p", ctx->id,
				free_frame_buffer->index, dstbuf->dbuf_frame_buf);
			dma_buf_put(dstbuf->dbuf_frame_buf);
			dstbuf->dbuf_frame_buf = NULL;
		}
		dstbuf->used = false;
	}
	mutex_unlock(&ctx->buf_lock);
...
static void mtk_vdec_pic_info_update(struct mtk_vcodec_ctx *ctx)
{
	...
	if (ret == 0)
		ctx->last_hdr_type = color_desc.hdr_type;
	...
	mtk_v4l2_debug(
		...
		ctx->last_decoded_picinfo.buf_h, ctx->hdr_type, ctx->last_hdr_type);
}
...
static void mtk_vdec_worker(struct work_struct *work)
{
	...
	if (ret == 0 && res_chg) {
		...
		ret = vdec_if_get_param(ctx, GET_PARAM_COLOR_DESC, &color_desc);
		if (ret == 0) {
			ctx->hdr_type = color_desc.hdr_type;
			ctx->last_hdr_type = color_desc.hdr_type;
		} else {
...
static void mtk_vcodec_reset_dma_buf(struct mtk_vcodec_ctx *ctx, struct mtk_video_dec_buf *mtkbuf)
{
	mtkbuf->bs_buffer.buf_att = NULL;
	mtkbuf->bs_buffer.sgt = NULL;
}

static int mtk_vdec_init_dma_buf(struct mtk_vcodec_ctx *ctx, struct mtk_video_dec_buf *mtkbuf)
{
	struct vb2_buffer *vb = &mtkbuf->vb.vb2_buf;

	mtk_v4l2_debug(VCODEC_DBG_L4, "[%d] id=%d", ctx->id, vb->index);
	mtkbuf->bs_buffer.buf_att = dma_buf_attach(vb->planes[0].dbuf, &ctx->dev->plat_dev->dev);
	mtkbuf->bs_buffer.sgt = dma_buf_map_attachment(mtkbuf->bs_buffer.buf_att, DMA_TO_DEVICE);
	if (IS_ERR_OR_NULL(mtkbuf->bs_buffer.sgt)) {
		mtk_v4l2_err("dma_buf_map_attachment fail %d.\n",
			(int)PTR_ERR(mtkbuf->bs_buffer.sgt));
		dma_buf_detach(vb->planes[0].dbuf, mtkbuf->bs_buffer.buf_att);
		mtk_vcodec_reset_dma_buf(ctx, mtkbuf);
		return -EINVAL;
	}
...
static void mtk_vdec_deinit_dma_buf(struct mtk_vcodec_ctx *ctx, struct mtk_video_dec_buf *mtkbuf)
{
	struct vb2_buffer *vb = &mtkbuf->vb.vb2_buf;
	struct dma_buf_attachment *buf_att = mtkbuf->bs_buffer.buf_att;
	struct sg_table *sgt = mtkbuf->bs_buffer.sgt;

	if (mtkbuf->bs_buffer.sgt == NULL)
		return;
...
static void mtk_vdec_deinit_general_buf(struct mtk_vcodec_ctx *ctx,
	struct mtk_video_dec_buf *mtkbuf)
{
	...
	if (mtkbuf->frame_buffer.dma_general_buf != 0) {
		WARN_ON(!mtkbuf->general_vb2_buf);
		q->mem_ops->unmap_dmabuf(mtkbuf->general_vb2_buf);
		q->mem_ops->detach_dmabuf(mtkbuf->general_vb2_buf);
		dma_buf_put(mtkbuf->frame_buffer.dma_general_buf);
		mtkbuf->general_vb2_buf = 0;
		mtkbuf->frame_buffer.dma_general_buf = 0;
		mtk_v4l2_debug(VCODEC_DBG_L4,
			"[%d] id=%d general_buf deinit, dma_buf_put general_buf fd=%d, dma_buf=%p",
			ctx->id, vb->index,
...
static int vidioc_vdec_g_pixelaspect(struct file *file, void *priv,
	int buf_type, struct v4l2_fract *pixelaspect)
{
	struct mtk_vcodec_ctx *ctx = fh_to_ctx(priv);
	...
	memset(pixelaspect, 0, sizeof(struct v4l2_fract));

	if (vdec_if_get_param(ctx, GET_PARAM_ASPECT_RATIO,
			pixelaspect) != 0) {
		mtk_v4l2_err("[%d] Error!! Cannot get aspect ratio",
			ctx->id);
		return -EINVAL;
	}
...
static int vb2ops_vdec_buf_prepare(struct vb2_buffer *vb)
{
	...
	if (vb->vb2_queue->type != V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
		if (mtkbuf->general_buf_fd > -1) {
			struct vb2_queue *q = vb->vb2_queue;
			struct dma_buf *dbuf = dma_buf_get(mtkbuf->general_buf_fd);

			if (IS_ERR(dbuf)) {
				mtk_v4l2_err("dma_general_buf is err 0x%p.\n",
					mtkbuf->frame_buffer.dma_general_buf);
			...
			if (dbuf != mtkbuf->frame_buffer.dma_general_buf) {
				if (mtkbuf->frame_buffer.dma_general_buf != 0) {
					mtk_v4l2_debug(VCODEC_DBG_L4,
						"general_buf change, dma_buf_put general_buf fd=%d, dma_buf=%p",
						mtkbuf->general_buf_fd,
						mtkbuf->frame_buffer.dma_general_buf);
					mtk_vdec_deinit_general_buf(ctx, mtkbuf);
				}

				mtkbuf->frame_buffer.dma_general_buf = dbuf;
				mtkbuf->general_vb2_buf = q->mem_ops->attach_dmabuf(
					q->dev,
					mtkbuf->frame_buffer.dma_general_buf,
					mtkbuf->frame_buffer.dma_general_buf->size,
					DMA_BIDIRECTIONAL);
				if (IS_ERR(mtkbuf->general_vb2_buf)) {
					mtk_v4l2_err("general_vb2_buf is err 0x%p.\n",
						mtkbuf->general_vb2_buf);
				...
				q->mem_ops->map_dmabuf(mtkbuf->general_vb2_buf);

				mtkbuf->frame_buffer.general_buf_va =
					q->mem_ops->vaddr(mtkbuf->general_vb2_buf);
			} else {
				dma_buf_put(dbuf);
			}
		} else {
		...
		mtk_v4l2_debug(VCODEC_DBG_L4,
			"general_buf_fd = %d, dma_general_buf = %p va %p",
			mtkbuf->general_buf_fd,
			mtkbuf->frame_buffer.dma_general_buf,
			mtkbuf->frame_buffer.general_buf_va);
	}
	...
	if (vb->vb2_queue->memory == VB2_MEMORY_DMABUF &&
		...
		if (vb->vb2_queue->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
			...
			unsigned int sync_offset = vb->planes[0].data_offset;

			mtk_v4l2_debug(4, "[%d] Cache sync+", ctx->id);

			if (mtkbuf->bs_buffer.sgt == NULL)
				mtk_vdec_init_dma_buf(ctx, mtkbuf);

			sgt = mtkbuf->bs_buffer.sgt;
			if (IS_ERR_OR_NULL(sgt)) {
				mtk_v4l2_err("invalid sgt %d.\n", (int)PTR_ERR(sgt));
				return -EINVAL;
			}
			mtk_dma_sync_sg_range(sgt, &ctx->dev->plat_dev->dev,
				sync_size, sync_offset, DMA_TO_DEVICE);
...
static void vb2ops_vdec_buf_cleanup(struct vb2_buffer *vb)
{
	...
	if (vb->vb2_queue->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE)
		mtk_vdec_deinit_dma_buf(ctx, buf);
	else
		mtk_vdec_deinit_general_buf(ctx, buf);
}
...
static int vb2ops_vdec_buf_init(struct vb2_buffer *vb)
{
	...
	if (vb->vb2_queue->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) {
		...
		mtk_vcodec_reset_dma_buf(ctx, buf);
	} else {
...
static void vb2ops_vdec_stop_streaming(struct vb2_queue *q)
{
	...
	if (ctx->state >= MTK_STATE_HEADER) {
		...
		ctx->hdr_type = ctx->last_hdr_type;
...
static int m2mops_vdec_job_ready(void *m2m_priv)
{
	...
	if ((ctx->last_decoded_picinfo.pic_w != ctx->picinfo.pic_w) ||
		...
		(ctx->last_hdr_type != ctx->hdr_type))
		return 0;
...
static enum vdec_set_param_type CID2SetParam(u32 id)
{
	...
	case V4L2_CID_MPEG_MTK_FIXED_MAX_FRAME_BUFFER:
		return SET_PARAM_FIXED_MAX_FRAME_BUFFER;
...
static int mtk_vdec_g_v_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_VDEC_BANDWIDTH_INFO: {
		...
		info->modifier = bandwidth_info.modifier;
		break;
	}
	case V4L2_CID_VDEC_MAX_BUF_INFO: {
		...
		max_buf_info.pixelformat = ctx->max_buf_pixelformat;
		max_buf_info.max_width = ctx->max_buf_width;
		max_buf_info.max_height = ctx->max_buf_height;

		if (vdec_if_get_param(ctx, GET_PARAM_MAX_BUF_INFO, &max_buf_info)) {
			mtk_v4l2_err(
				"[%d]Error!! Cannot get param : GET_PARAM_BANDWIDTH_INFO ERR",
				ctx->id);
			ret = -EINVAL;
		...
		if (max_buf_info.max_internal_buf_size == 0 ||
			max_buf_info.max_dpb_count == 0 ||
			max_buf_info.max_frame_buf_size == 0) {
			mtk_v4l2_err(
		...
		info->pixelformat = max_buf_info.pixelformat;
		info->max_width = max_buf_info.max_width;
		info->max_height = max_buf_info.max_height;
		info->max_internal_buf_size = max_buf_info.max_internal_buf_size;
		info->max_dpb_count = max_buf_info.max_dpb_count;
		info->max_frame_buf_size = max_buf_info.max_frame_buf_size;
		break;
	}
	case V4L2_CID_VDEC_TRICK_MODE:
	case V4L2_CID_MPEG_MTK_COLOR_DESC: {
...
static int mtk_vdec_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_VDEC_DETECT_TIMESTAMP:
		...
		break;
	case V4L2_CID_VDEC_LOW_LATENCY: {
		struct v4l2_vdec_low_latency_parameter *low_latency_param = ctrl->p_new.p;

		ctx->dec_params.slice_count = low_latency_param->slice_count;
		ctx->dec_params.racing_display = low_latency_param->racing_display;
		break;
	}
#endif
	default:
...
static const struct v4l2_ctrl_config vdec_v4l2_ctrl_configs[] = {
	...
	},
	{
		.ops = &mtk_vcodec_dec_ctrl_ops,
		.type = V4L2_CTRL_TYPE_INTEGER,
		.id = V4L2_CID_MPEG_MTK_SEC_DECODE,
		.name = "VDEC set enable secure mode",
		.min = V4L2_VDEC_NONE_TEE,
		.max = V4L2_VDEC_OPEN_TEE,
		.step = 1,
		.def = 0,
	},
	{
	...
	},
	{
		.ops = &mtk_vcodec_dec_ctrl_ops,
		.type = V4L2_CTRL_TYPE_INTEGER,
		.id = V4L2_CID_MPEG_MTK_FIXED_MAX_FRAME_BUFFER,
		.flags = V4L2_CTRL_FLAG_EXECUTE_ON_WRITE,
		.name = "VDEC set fixed max frame buffer",
		.min = 0,
		.max = 65535,
		.step = 1,
		.def = 0,
	},
	{
		...
		.flags = V4L2_CTRL_FLAG_HAS_PAYLOAD | V4L2_CTRL_FLAG_VOLATILE |
			 V4L2_CTRL_FLAG_EXECUTE_ON_WRITE,
		.id = V4L2_CID_VDEC_MAX_BUF_INFO,
		.name = "get VDEC max buffer info",
		.min = 0,
		.max = 0xFF,
		.step = 1,
		.def = 0,
		.dims = {sizeof(struct v4l2_vdec_max_buf_info)},
	},
	{
		.ops = &mtk_vcodec_dec_ctrl_ops,
		.type = V4L2_CTRL_TYPE_U8,
		.flags = V4L2_CTRL_FLAG_HAS_PAYLOAD | V4L2_CTRL_FLAG_VOLATILE |
			 V4L2_CTRL_FLAG_EXECUTE_ON_WRITE,
		.id = V4L2_CID_VDEC_LOW_LATENCY,
		.name = "VDEC set low latency info",
		.min = 0,
		.max = 0xFF,
		.step = 1,
		.def = 0,
		.dims = {sizeof(struct v4l2_vdec_low_latency_parameter)},
	},
};
#else
