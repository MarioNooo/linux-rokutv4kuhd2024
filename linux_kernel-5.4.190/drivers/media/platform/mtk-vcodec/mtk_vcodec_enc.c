...
#include "mtk-iommu-dtv-contig.h"
#endif
...
// TODO: Change to original flow in M6.
	...
	case V4L2_CID_MPEG_MTK_COLOR_DESC:
		...
		break;
	case V4L2_CID_MPEG_MTK_VIDEO_QUALITY:
		mtk_v4l2_debug(2,
			"V4L2_CID_MPEG_MTK_VIDEO_QUALITY: 0x%x",
			ctrl->val);
		memcpy(&p->video_quality, ctrl->p_new.p_u32,
		sizeof(struct mtk_video_quality));
		ctx->param_change |= MTK_ENCODE_PARAM_VIDEO_QUALITY;
		break;
#ifndef TV_ENCODE_INTEGRATION
...
static int vidioc_venc_g_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_MPEG_MTK_RESOLUTION_CHANGE:
		...
		break;
#endif
	case V4L2_CID_MPEG_MTK_AVERAGE_QP:
		venc_if_get_param(ctx,
			GET_PARAM_AVERAGE_QP,
			&value);
		ctrl->val = value;
		break;
	default:
			  ...
			  struct mtk_vcodec_ctx *ctx)
{
	...
	if (f->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) {
		...
		for (i = 0; i < NUM_SUPPORTED_FRAMESIZE; i++)
			if (mtk_venc_framesizes[i].fourcc == pix_fmt_mp->pixelformat)
		...
		// redefine the alignment
		if (((pix_fmt_mp->width - 16) % 256) == 0) {
				 ...
				 struct v4l2_format *f)
{
...
#ifdef TV_ENCODE_INTEGRATION
	// TODO: This workaround is for utopia driver, and will remove in M6.
	...
	mtk_v4l2_debug(4, "q_data->sizeimage[0] = %d", q_data->sizeimage[i]);
	mtk_v4l2_debug(4, "f->fmt.pix_mp.width = %d", f->fmt.pix_mp.width);
	mtk_v4l2_debug(4, "f->fmt.pix_mp.height = %d",  f->fmt.pix_mp.height);
			    ...
			    struct v4l2_buffer *buf)
{
	...
	mtk_v4l2_debug(2, "buf->m.planes[0].bytesused = %d", buf->m.planes[0].bytesused);
	mtk_v4l2_debug(2, "buf->m.planes[1].bytesused = %d", buf->m.planes[1].bytesused);
	...
	if (buf->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
		if (buf->m.planes[0].bytesused == 0) {
			...
			mtk_v4l2_debug(0, "[%d] id=%d getdata FB(%d,%d) vb=%p pts=%llu ",
				ctx->id, buf->index,
...
static int mtk_venc_param_change(struct mtk_vcodec_ctx *ctx)
{
	...
	if (!ret &&
	mtk_buf->param_change & MTK_ENCODE_PARAM_VIDEO_QUALITY) {
		// avoid much copies
		enc_prm.video_quality = &mtk_buf->enc_params.video_quality;
		mtk_v4l2_err("[%d] idx=%d, i_max_qp=%d i_min_qp=%d p_max_qp=%d p_min_qp=%d",
				ctx->id,
				mtk_buf->vb.vb2_buf.index,
				enc_prm.video_quality->video_qp_i_max,
				enc_prm.video_quality->video_qp_i_min,
				enc_prm.video_quality->video_qp_p_max,
				enc_prm.video_quality->video_qp_p_min);
		ret |= venc_if_set_param(ctx,
					VENC_SET_PARAM_VIDEO_QUALITY,
					&enc_prm);
	}
...
static void mtk_venc_worker(struct work_struct *work)
{
	...
	for (i = 0; i < src_buf->num_planes ; i++) {
		...
		mtk_v4l2_debug(2,
			"fb_addr[%d].va %p, offset %d, dma_addr %pad, size %d\n",
	...
	mtk_v4l2_debug(2, "length = %d", length);
...
static const struct v4l2_ctrl_config mtk_enc_video_quality = {
	.ops = &mtk_vcodec_enc_ctrl_ops,
	.id = V4L2_CID_MPEG_MTK_VIDEO_QUALITY,
	.name = "Video encode quality",
	.type = V4L2_CTRL_TYPE_U32,
	.flags = V4L2_CTRL_FLAG_WRITE_ONLY,
	.min = 0x00000000,
	.max = 0xffffffff,
	.step = 1,
	.def = 0,
	.dims = { sizeof(struct mtk_video_quality)/sizeof(u32) }
};
...
int mtk_vcodec_enc_ctrls_setup(struct mtk_vcodec_ctx *ctx)
{
	...
	memset(&cfg, 0, sizeof(cfg));
	cfg.id = V4L2_CID_MPEG_MTK_AVERAGE_QP;
	cfg.type = V4L2_CTRL_TYPE_INTEGER;
	cfg.flags = V4L2_CTRL_FLAG_VOLATILE;
	cfg.name = "Video encode roi rc qp";
	cfg.min = 0;
...
#else
#ifdef TV_ENCODE_INTEGRATION
	src_vq->mem_ops		= &mtk_iommu_dtv_contig_memops;
#else
...
#else
#ifdef TV_ENCODE_INTEGRATION
	dst_vq->mem_ops     = &mtk_iommu_dtv_contig_memops;
#else
...
{
	struct vb2_v4l2_buffer *src_vb2_v4l2 = NULL, *dst_vb2_v4l2 = NULL;
