...
#include "mt5896_hwspinlock_coda.h"
#include "mt5896_hwspinlock_riu.h"
...
static int mtk_dtv_hwspinlock_trylock(struct hwspinlock *lock)
{
	...
	mtk_writew_relaxed(addr, HW_SPINLOCK_PROCESSOR_ID);
	val = mtk_readw_relaxed(addr);
...
arch_platform_driver(mtk_dtv_hwspinlock_driver);
#else
module_platform_driver(mtk_dtv_hwspinlock_driver);
#endif
MODULE_DESCRIPTION("MediaTek DTV SoC based HWSPINLOCK Driver");
