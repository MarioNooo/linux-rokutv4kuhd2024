...
// MEDIATEK-7747: move saradc initcall before roku_early_pwr_mode_handler_init to avoid iio device not found.
static int __init mtkdtv_saradc_init(void)
{
	return platform_driver_register(&mtkdtv_saradc_driver);
}

static void __exit mtkdtv_saradc_exit(void)
{
    platform_driver_unregister(&mtkdtv_saradc_driver);
}


fs_initcall(mtkdtv_saradc_init);
module_exit(mtkdtv_saradc_exit);
#else
