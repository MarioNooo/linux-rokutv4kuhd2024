...
#ifdef CONFIG_ROKU_SPI_MT5896_MODIFY
extern int roku_ldmled_init_after_spi_inited(int bus_num);
#endif
...
static void dtvspi_hw_set_bit_len_sel(struct dtv_spi *bs, struct spi_transfer *tfr)
{
	...
	for(i=0; i<wd_num; i++) {
		...
		for(j=0; j<SPI_OFFSET_IDX; j++) {
			u16 index = i * SPI_OFFSET_IDX + (SPI_OFFSET_IDX - j - 1); // index 3, 2, 1, 0 for shift
			reg <<= SPI_BIT_MUX_BITS;
			if(bs->dma_sel)
				reg |= (bs->xfer_bits[index] - 1); // dma mode use tr_mux table
			else {
				if((index == bit_len_num - 1) && (tfr->bits_per_word % SPI_BIT_LEN_FULL))
...
static void dtv_spi_ldmtx_config(struct dtv_spi *bs, struct spi_transfer *tfr)
{
	...
	if (bs->dma_sel) {
		...
		if(bs->dma_nByte2nByte) {
			...
			dtvspi_wrl(bs, SPI_REG_0120_MSPI0, (tfr->bits_per_word+(SPI_BIT_LEN_FULL-1))/SPI_BIT_LEN_FULL);
		}
...
#ifdef CONFIG_ROKU_SPI_MT5896_MODIFY
		roku_ldmled_init_after_spi_inited(bs->mspi_channel);
#endif
    }
...
arch_platform_driver(dtv_spi_driver);
#else
