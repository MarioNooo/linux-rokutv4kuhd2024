...
#define HALF_WORD						8
#define MAX_SPICI_HW_NUM				1
#define DIV_2N4							4
#define CISPI_1MCLK						1000000
#define SPI_CTL_H						8
#define ONE_M_SEC					1000

static struct spi_device *_ptr_array[MAX_SPICI_HW_NUM];
...
struct spi_device *kmci_spidev_node_get(u8 channel)
{
	return _ptr_array[channel];
}
EXPORT_SYMBOL(kmci_spidev_node_get);
...
int ci_slave_write_read_poll(struct spi_device *spi, u8 *txbuf, u8 tlen, u8 *rxbuf, u8 rlen)
{
	struct dtv_spi_ci *bs = spi_master_get_devdata(spi->master);
	...
	bs->tx_buf = txbuf;
	bs->len = tlen;
	dtvspi_hw_txfillfifo(bs, (bs->tx_buf), bs->len);
	dtvspici_wrh(bs, SPI_REG_0120_MSPI0, rlen);
	dtvspi_hw_transfer_trigger(bs);
	while (1) {
		if (dtvspici_rd(bs, SPI_REG_016C_MSPI0)) {
			dtvspi_hw_clear_done(bs);
		...
		udelay(1);
		timeout ++;
		if (timeout > ONE_M_SEC) {
			err = -ETIMEDOUT;
			break;
		}
	}
	dtvspi_hw_rxgetfifo_ext(bs, bs->rx_buf, rlen);
	(bs->rx_buf) += rlen;
	mutex_unlock(&bs->bus_mutex);

	return err;
}
EXPORT_SYMBOL(ci_slave_write_read_poll);
...
static int dtvci_spi_probe(struct platform_device *pdev)
{
	...
	if (bs->mspi_channel < MAX_SPICI_HW_NUM)
