	...
	if (((mdp->data->duty_cycle) != duty_t) || ((mdp->pre_syn_t) != mdp->data->sync_times)){
		...
		if (vsync_t == SEG_2_T_V) {
			...
			if (counter) {
				...
				counter_tmp = counter + (duty_t*((V_16384_P)/(HALF_DUTY*vsync_t)));
		...
		if (vsync_t == SEG_4_T_V) {
			...
			if (counter) {
				...
				position |= counter;
				dtvsegpwm_wr(mdp, REG_0020_PWM_SEG, position);
				counter = (duty_t + (mdp->data->shift))*(V_16384_P/2);
				counter /= FULL_D;
				if (counter > V_16384_P)
					counter -= V_16384_P;
				position = 0;
		...
		if (vsync_t == SEG_16_T_V) {
			...
			//dtvsegpwm_wr(mdp, REG_0048_PWM_SEG, 1);
...
static void mtk_segpwm_pad_switch(struct mtk_segpwm *mdp, bool enable)
{
	if (enable) {
		mtk_writew_relaxed(mdp->base_n + REG_01CC_PWM_SEG, SCAN_PWM_OUT);
	} else {
		if (mdp->channel == 0) {
			mtk_writew_relaxed(mdp->base_y, SEG_PWM0_OUT_SINGLE);
			mtk_writew_relaxed(mdp->base_n + REG_01CC_PWM_SEG, SEG_PWM0_OUT);
...
int mtk_segpwm_to_sw(u8 channel, bool enable)
{
	struct pwm_chip *pwmchip = _ptr_array[channel];
	struct mtk_segpwm *mdp = to_mtk_segpwm(pwmchip);
	mtk_segpwm_pad_switch(mdp, enable);
	return 0;
}
EXPORT_SYMBOL(mtk_segpwm_to_sw);
