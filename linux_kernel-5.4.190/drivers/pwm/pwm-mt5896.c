...
static void mtk_pwm_pol(struct mtk_pwm *mdp, unsigned int data)
{
	...
	reg_tmp = dtvpwm_rd(mdp, REG_0014_PWM);
	// Fix MTK ref code
	if (data)
		reg_tmp |= (0x01 << PM_PWM_POL);
	else
		reg_tmp &= ~(0x01 << PM_PWM_POL);
	dtvpwm_wr(mdp, REG_0014_PWM, reg_tmp);
	mdp->data->pol = data;
#else
	reg_tmp = dtvpwm_rd(mdp, REG_0014_PWM);
	if (data)
...
#ifndef CONFIG_ROKU_PWM_MT5896_MODIFY
	// don't do this, the register bit set in this API seems incorrect.
	// Once we update driver codes we need to review this API.
...
static int mtk_pwm_polarity(struct pwm_chip *chip,
							struct pwm_device *pwm,
							enum pwm_polarity polarity)
{
	struct mtk_pwm *mdp = to_mtk_pwm(chip);
	mtk_pwm_pol(mdp, polarity);
	return 0;
}
