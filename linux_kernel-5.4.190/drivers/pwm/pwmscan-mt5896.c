...
#define CKGEN_BASE				2
#define BYTE_SHIFT				8
#define COMPESATION				10
#define OFFSET_FULL_L			0xFFFF
#define OFFSET_FULL_H			0xFF
...
static void mtk_scanpwm_full_set(struct mtk_scanpwm *mdp, unsigned int data)
{
	u16 reg_tmp;
	...
	if (data) {
		...
		mdp->duty_full_dbg = 1;
		dtvscanpwm_wr(mdp, REG_0010_PWM_SCAN0, OFFSET_FULL_L);
		dtvscanpwm_wr(mdp, REG_0014_PWM_SCAN0, OFFSET_FULL_H);
	} else {
		reg_tmp &= ~(0x1 << BYTE_SHIFT);
		mdp->duty_full_dbg = 0;
		dtvscanpwm_wr(mdp, REG_0010_PWM_SCAN0, 0);
		dtvscanpwm_wr(mdp, REG_0014_PWM_SCAN0, 0);
	}
						...
						int duty_t, int period_t)
{
	...
	u32 compesation_set = 0;

	/*12hz XTAL timing is nsec*/
	period_set = (XTAL_MHZ*period_t);
	if (period_set % COMPESATION) {
		compesation_set = COMPESATION - (period_set % COMPESATION);
		period_set += compesation_set;
	}
	period_set /= FREQ_SCALE;
	duty_set = (XTAL_MHZ*duty_t);
	if (duty_set % COMPESATION) {
		compesation_set = COMPESATION - (duty_set % COMPESATION);
		duty_set += compesation_set;
	}
	duty_set /= FREQ_SCALE;
	offset_set = (XTAL_MHZ*(mdp->data->shift));
	if (offset_set % COMPESATION) {
		compesation_set = COMPESATION - (offset_set % COMPESATION);
		offset_set += compesation_set;
	}
	offset_set /= FREQ_SCALE;
	mtk_scanpwm_shift(mdp, offset_set);
	mtk_scanpwm_rstcnt(mdp, mdp->data->rstcnt);
	mtk_scanpwm_rstmux(mdp, mdp->data->rst_mux);
	if (mdp->vsync_free_dbg == 0)
...
#ifdef CONFIG_ROKU_PWM_MT5896_MODIFY
	mdp->data->enable = 1;
#endif
	return 0;
}
...
static ssize_t fullscale_store(struct device *child,
			      struct device_attribute *attr,
			      const char *buf, size_t size)
{
	struct mtk_scanpwm *mdp = dev_get_drvdata(child);
	...
	ret = kstrtouint(buf, 0, &val);
	mtk_scanpwm_full_set(mdp, val);
	return ret ? : size;
}
static DEVICE_ATTR_RW(fullscale);

static ssize_t alignsync_show(struct device *child,
			   struct device_attribute *attr,
			   char *buf)
{
	struct mtk_scanpwm *mdp = dev_get_drvdata(child);
	...
	reg_tmp = dtvscanpwm_rd(mdp, REG_0000_PWM_SCAN0);
	reg_tmp |= (dtvscanpwm_rd(mdp, REG_0004_PWM_SCAN0) << PWM_HALF_WORD);
	freq_tmp = (XTAL_MHZ*FREQ_SCALE*FREQ_SCALE)/reg_tmp;

	return scnprintf(buf, LENSIZE, "%d\n", freq_tmp);
}


static ssize_t alignsync_store(struct device *child,
			      struct device_attribute *attr,
			      const char *buf, size_t size)
{
	struct mtk_scanpwm *mdp = dev_get_drvdata(child);
	...
	ret = kstrtouint(buf, 0, &val);
	reg_tmp = XTAL_MHZ*FREQ_SCALE*FREQ_SCALE/val;
	dtvscanpwm_wr(mdp, REG_0000_PWM_SCAN0, (u16)reg_tmp);
	dtvscanpwm_wr(mdp, REG_0004_PWM_SCAN0, (reg_tmp >> PWM_HALF_WORD));
	return ret ? : size;
}
static DEVICE_ATTR_RW(alignsync);

static ssize_t fixperiod_show(struct device *child,
			   struct device_attribute *attr,
			   char *buf)
{
	struct mtk_scanpwm *mdp = dev_get_drvdata(child);
...
static ssize_t fixperiod_store(struct device *child,
			      struct device_attribute *attr,
			      const char *buf, size_t size)
{
	struct mtk_scanpwm *mdp = dev_get_drvdata(child);
	...
	ret = kstrtouint(buf, 0, &val);
	mdp->period_fix_dbg = val;
	return ret ? : size;
}
static DEVICE_ATTR_RW(fixperiod);
...
static struct attribute *pwm_extend_attrs[] = {
	...
	&dev_attr_fixperiod.attr,
	&dev_attr_alignsync.attr,
	NULL,
};
...
static int mtk_scanpwm_probe(struct platform_device *pdev)
{
	...
	err = of_property_read_u32(pdev->dev.of_node,
				"sysfs-support", &mdp->sysfs_en);
	if (err)
		dev_warn(&pdev->dev, "could not get sysfs-support ns resource\n");
