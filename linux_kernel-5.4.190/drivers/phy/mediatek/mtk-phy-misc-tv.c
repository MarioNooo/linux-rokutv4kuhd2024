...
/* UPLL */
#define U3P_PHY_UPLL_REG00	0x000
#define PA0_RG_U3_PLL_PD	BIT(1)
			...
			struct mtk_phy_instance *instance)
{
	...
	case PHY_TYPE_USB2:
		if (tphy->t_misc.shared_fmreg && (instance->index != 0)) {
			u2_banks->fmreg = tphy->phys[0]->port_base
				+ SSUSB_SIFSLV_V2_U2FREQ;
			dev_info(dev, "shared fmreg bank\n");
		}
		break;
	case PHY_TYPE_USB3:
	case PHY_TYPE_PCIE:
				...
				struct mtk_tphy *tphy)
{
	...
	tphy->t_misc.shared_fmreg =
		device_property_read_bool(dev, "mediatek,shared-fmreg");

	return retval;
}

static void misc_u3_phy_instance_power_on(struct mtk_tphy *tphy,
	struct mtk_phy_instance *instance)
{
	struct u3mphy_banks *u3_mphy = &instance->i_misc.u3_mphy;
	void __iomem *atop = u3_mphy->atop;
	...
	if (tphy->t_misc.mixed_mode) {
		/* upll power on */
		tmp = readl(upll + U3P_PHY_UPLL_REG00);
		tmp &= ~PA0_RG_U3_PLL_PD;
		writel(tmp, upll + U3P_PHY_UPLL_REG00);
		/* wait for upll stable*/
		mdelay(1);
...
static void misc_u2_phy_instance_power_on(struct mtk_tphy *tphy,
	struct mtk_phy_instance *instance)
{
	struct u2phy_banks *u2_banks = &instance->u2_banks;
	void __iomem *com = u2_banks->com;
	...
	/* wait 250 usec. (PHY 3.3v & 1.8v & 1.2v power stable time) */
	udelay(250);
	...
	tmp = readl(com + U3P_U2PHYDTM0);
	tmp &= ~(P2C_FORCE_DP_PULLDOWN | P2C_FORCE_DM_PULLDOWN);
	writel(tmp, com + U3P_U2PHYDTM0);
...
static void misc_u3_phy_instance_power_off(struct mtk_tphy *tphy,
	struct mtk_phy_instance *instance)
{
	struct u3mphy_banks *u3_mphy = &instance->i_misc.u3_mphy;
	void __iomem *atop = u3_mphy->atop;
...
static void misc_u2_phy_instance_power_off(struct mtk_tphy *tphy,
	struct mtk_phy_instance *instance)
{
	struct u2phy_banks *u2_banks = &instance->u2_banks;
	void __iomem *com = u2_banks->com;
...
int mtk_misc_phy_power_on(struct phy *phy)
{
	...
	if (instance->type == PHY_TYPE_USB3)
		misc_u3_phy_instance_power_on(tphy, instance);
	else if (instance->type == PHY_TYPE_USB2)
		misc_u2_phy_instance_power_on(tphy, instance);
...
int mtk_misc_phy_power_off(struct phy *phy)
{
	...
	if (instance->type == PHY_TYPE_USB3)
		misc_u3_phy_instance_power_off(tphy, instance);
	else if (instance->type == PHY_TYPE_USB2)
		misc_u2_phy_instance_power_off(tphy, instance);
