...
#define PA2_RG_USB20_CLKREF_REV		GENMASK(7, 0)
#define PA2_RG_USB20_CLKREF_REV_VAL(x)	(0xff & (x))
#define PA2_RG_USB20_CLKREF_REV1	GENMASK(15, 8)
#define PA2_RG_USB20_CLKREF_REV1_VAL(x)	((0xff & (x)) << 8)
#endif
...
#define PA4_RG_USB20_LS_CR		GENMASK(14, 12)
#define PA4_RG_USB20_LS_CR_VAL(x)	((0x7 & (x)) << 12)
#endif
...
struct instance_misc {
	...
	/* U3 2.5G mode */
	bool degradation_en;
	u32 degradation_mode;
	void __iomem *base_mphy;
	void __iomem *base_upll;
	void __iomem *base_u3phy;
...
struct mtk_tphy {
	...
	bool src_hw;  /* HW itself does slew rate calibrate */
#ifdef CONFIG_USB_MTK_DTV_XHCI
	struct tphy_misc t_misc;
#endif
};

#ifdef CONFIG_USB_MTK_DTV_XHCI
#include "mtk-phy-misc-tv.c"
#endif
	...
	struct mtk_phy_instance *instance)
{
	...
	/* HW itself does it, or use force value */
	if (tphy->src_hw || instance->eye_src)
		return;
...
#ifdef CONFIG_USB_MTK_DTV_XHCI
	else if (tphy->t_misc.shared_fmreg)
				     ...
				     enum phy_mode mode)
{
	...
	/* There is no VBUS_VALID , so we have to force phy enter device mode */
#ifdef CONFIG_USB_MTK_DTV_XHCI
		tmp = 0x3e2e;
#else
	...
	/* There is no VBUS_VALID , so we have to force phy enter host mode */
#ifdef CONFIG_USB_MTK_DTV_XHCI
		tmp = 0x3e2c;
#else
		tmp |= P2C_FORCE_IDDIG;
...
#ifdef CONFIG_USB_MTK_DTV_XHCI
	mtk_misc_parse_property(tphy, instance);
#endif
...
#ifdef CONFIG_USB_MTK_DTV_XHCI
	mtk_misc_i2c_init(tphy, instance);
#endif
	switch (instance->type) {
	case PHY_TYPE_USB2:
...
#ifdef CONFIG_USB_MTK_DTV_XHCI
		mtk_misc_u2_instance_init_and_set(tphy, instance);
#endif
		break;
	case PHY_TYPE_USB3:
		u3_phy_instance_init(tphy, instance);
#ifdef CONFIG_USB_MTK_DTV_XHCI
		mtk_misc_u3_instance_init(tphy, instance);
#endif
		break;
	case PHY_TYPE_PCIE:
...
#ifdef CONFIG_USB_MTK_DTV_XHCI
		mtk_misc_v2_banks_init(tphy, instance);
#endif
	} else {
...
static int mtk_tphy_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_USB_MTK_DTV_XHCI
	retval = mtk_misc_get_res(pdev, tphy);
	if (retval)
		return retval;
#endif
	...
	tphy->src_hw =
		device_property_read_bool(dev, "mediatek,src_hw_capable");
...
#ifdef CONFIG_USB_MTK_DTV_XHCI
		retval = mtk_misc_instance_get_res(pdev, child_np, instance, phy);
		if (retval)
			goto put_child;
#endif
...
#ifdef CONFIG_USB_MTK_DTV_XHCI
		/* ref_clk is optional for TV SOC */
