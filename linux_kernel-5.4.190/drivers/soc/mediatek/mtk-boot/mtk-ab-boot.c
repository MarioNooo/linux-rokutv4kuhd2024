...
static int mtk_invoke_avb_cmd(uint32_t cmd, uint32_t sess_id, struct uboot_avb_data *avb_data)
{
	int rc = 0, i;
	...
	if (!strcmp(avb_data->magic, AVB_MAGIC)) {
...
static int mtk_update_avb_vbmeta(void)
{
	...
	if (!is_found) {
		pr_info("%s: no avb data need to be updated", __func__);
		goto exit;
	}
	...
	rc = mtk_invoke_avb_cmd(TA_AVB_CMD_WRITE_ROLLBACK_INDEX, sess_id, avb_data);
	if (rc)
