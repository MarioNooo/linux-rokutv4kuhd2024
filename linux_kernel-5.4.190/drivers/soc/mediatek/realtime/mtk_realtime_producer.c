...
#define USLEEP_RANGE_MIN (50)
#define USLEEP_RANGE_MAX (100)
#define WAIT_TIMES  (1000000/USLEEP_RANGE_MAX)

struct rt_poll mtk_realtime_poll;

#if defined(AOW_ENABLE) || defined(AOSP_ENABLE)
static void mtk_realtime_get_ret_value(MI_RESULT *ret,
									MI_S32 s32CmdIdx,
									MI_S8 *pszDomainName)
{
	const int times = WAIT_TIMES;
	int i = 0;

	while (mtk_realtime_poll.ret_value == INIT_RET_VALUE && i++ < times)
		usleep_range(USLEEP_RANGE_MIN, USLEEP_RANGE_MAX);
	if (i > times) {
		pr_alert("%s TIMEOUT=%d, ret=%d, CmdIdx=%d, Domain=%s\n", __func__, i,
				mtk_realtime_poll.ret_value, s32CmdIdx,
				(pszDomainName == NULL ? (MI_S8 *)"N/A" : pszDomainName));
	}
	//else
...
MI_REALTIME_Init(MI_REALTIME_MODE_e eRtMode)
{
	...
	mtk_realtime_get_ret_value(&Ret, E_MI_REALTIME_Init, NULL);
	return Ret;
}
...
MI_REALTIME_Deinit(void)
{
	...
	mtk_realtime_send_poll_message(Cmd);
	mtk_realtime_get_ret_value(&Ret, E_MI_REALTIME_Deinit, NULL);
	return Ret;
}
...
MI_REALTIME_CreateDomain(MI_S8 *pszDomainName)
{
	...
	mtk_realtime_send_poll_message(Cmd);
	mtk_realtime_get_ret_value(&Ret, E_MI_REALTIME_CreateDomain, pszDomainName);
	return Ret;
}
...
MI_REALTIME_DeleteDomain(MI_S8 *pszDomainName)
{
	...
	mtk_realtime_send_poll_message(Cmd);
	mtk_realtime_get_ret_value(&Ret, E_MI_REALTIME_DeleteDomain, pszDomainName);
	return Ret;
}
					...
					void (*sigRoutine)(int))
{
	...
	mtk_realtime_send_poll_message(Cmd);
	mtk_realtime_get_ret_value(&Ret, E_MI_REALTIME_AddThread, pszDomainName);
	return Ret;
}
...
MI_REALTIME_RemoveThread(MI_S8 *pszDomainName, MI_S32 s32Pid)
{
	...
	mtk_realtime_send_poll_message(Cmd);
	mtk_realtime_get_ret_value(&Ret, E_MI_REALTIME_RemoveThread, pszDomainName);
	return Ret;
}
						...
						MI_S32 *s32CurBudget)
{
	...
	mtk_realtime_send_poll_message(Cmd);
	mtk_realtime_get_param_out(&param_out, E_MI_REALTIME_QueryBudget, pszDomainName);
	mtk_realtime_get_ret_value(&Ret, E_MI_REALTIME_QueryBudget, pszDomainName);
						...
						void (*sigRoutine)(int))
{
	...
	mtk_realtime_send_poll_message(Cmd);
	mtk_realtime_get_ret_value(&Ret, E_MI_REALTIME_AddThread_Lite, pszDomainName);
	return Ret;
}
							...
							MI_S32 s32Pid)
{
	...
	mtk_realtime_send_poll_message(Cmd);
	mtk_realtime_get_ret_value(&Ret, E_MI_REALTIME_UpdateThreadUtil, pszDomainName);
	return Ret;
}
...
MI_REALTIME_DumpDomainInfo(MI_S8 *pszDomainName)
{
	...
	mtk_realtime_send_poll_message(Cmd);
	mtk_realtime_get_ret_value(&Ret, E_MI_REALTIME_DumpDomainInfo, pszDomainName);
	return Ret;
}
...
MI_REALTIME_GetAllDomainName(MI_S8 *ps8Buf, MI_U32 u32BufSize)
{
	...
	mtk_realtime_send_poll_message(Cmd);
	mtk_realtime_get_ret_value(&Ret, E_MI_REALTIME_GetAllDomainName, NULL);
	return Ret;
}
							...
							MI_S32 s32Iteration)
{
	...
	mtk_realtime_send_poll_message(Cmd);
	mtk_realtime_get_ret_value(&Ret, E_MI_REALTIME_GetDomainCpuUsage, pszDomainName);
	return Ret;
}
									...
									MI_S32 *ps32MemIndex)
{
	...
	mtk_realtime_send_poll_message(Cmd);
	mtk_realtime_get_param_out(&param_out, E_MI_REALTIME_EnableMemoryHighPriority, NULL);
	mtk_realtime_get_ret_value(&Ret, E_MI_REALTIME_EnableMemoryHighPriority, NULL);
...
MI_REALTIME_DisableMemoryHighPriority(MI_S32 s32Index)
{
	...
	mtk_realtime_send_poll_message(Cmd);
	mtk_realtime_get_ret_value(&Ret, E_MI_REALTIME_DisableMemoryHighPriority, NULL);
	return Ret;
}
...
MI_REALTIME_GetAvailableHighPriorityResource(MI_S32 *ps32Avail)
{
	...
	mtk_realtime_get_param_out(&param_out, E_MI_REALTIME_GetAvailableHighPriorityResource,
								NULL);
	mtk_realtime_get_ret_value(&Ret, E_MI_REALTIME_GetAvailableHighPriorityResource, NULL);
	*ps32Avail = param_out;
	return Ret;
}
...
MI_REALTIME_SetDebugLevel(MI_DBG_LEVEL eDbgLevel)
{
	...
	mtk_realtime_send_poll_message(Cmd);
	mtk_realtime_get_ret_value(&Ret, E_MI_REALTIME_SetDebugLevel, NULL);
	return Ret;
}
...
MI_REALTIME_DmipsToUs(MI_U32 u32Dmips, MI_U32 *u32MicroSec)
{
	...
	mtk_realtime_send_poll_message(Cmd);
	mtk_realtime_get_param_out(&param_out, E_MI_REALTIME_DmipsToUs, NULL);
	mtk_realtime_get_ret_value(&Ret, E_MI_REALTIME_DmipsToUs, NULL);
