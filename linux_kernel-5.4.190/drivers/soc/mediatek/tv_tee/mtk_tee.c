...
static int __init tee_create_xtest(void)
{
	...
	timeout = proc_create("optee_xtest", 0660, xtest_root, &tz_fops_misc);
	if (!timeout) {
...
static int __init mtk_tee_init(void)
{
	...
	dir = proc_create("log_level", 0660, root, &tz_fops);
	if (!dir) {
	...
	dir = proc_create("version", 0660, root, &tz_fops_version);
	if (!dir) {
