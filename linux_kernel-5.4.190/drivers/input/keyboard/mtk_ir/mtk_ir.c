...
#define IR_GET_KEYMAP_FROM_DTS     0
#else
...
static int mtk_ir_get_keymap_from_dts(struct device_node *np)
{
	...
	key_map_lists = kzalloc(sizeof(struct key_map_list) * MAX_PROFILE_NUM, GFP_KERNEL);
	if ((key_map_lists == NULL)) {
		IRDBG_ERR("%s: kzalloc key_map_lists fail!\n", __func__);
	...
	for (profile_index = 0; profile_index < MAX_PROFILE_NUM; profile_index++) {
		...
		key_map_lists[profile_index].map.scan = table[profile_index] ;
		key_map_lists[profile_index].map.headcode = u32headcode_Tmp;
		key_map_lists[profile_index].map.size = keymap_size;
		key_map_lists[profile_index].map.name = dts_str;
		...
		MIRC_Map_Register(&key_map_lists[profile_index]);
