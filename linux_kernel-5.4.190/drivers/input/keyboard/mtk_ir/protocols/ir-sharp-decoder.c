...
static int ir_sharp_decode(struct mtk_ir_dev *dev, struct ir_raw_data ev)
{
    ...
    switch (data->eStatus)
    {
        ...
        case STATE_BIT_DATA:
            ...
            {
#ifdef CONFIG_ROKU_IR_MODIFY
                if (dev->support_ir[i].u32HeadCode == NUM_KEYMAP_ROKU_TV && dev->support_ir[i].eIRType == IR_TYPE_SHARP)
                {
                    current_key.map_num = NUM_KEYMAP_ROKU_TV;
                }
#else
...
#ifdef CONFIG_ROKU_IR_MODIFY
                    sc->scancode = (reverse_5bit(current_key.syscode) << 16) | reverse_8bit(front_key.databit);
#else
