...
ret_ok:
            for (i= 0;i<dev->support_num;i++)
            {
                ...
                if (u16Addr != dev->support_ir[i].u32HeadCode && u16Addr != NUM_KEYMAP_ROKU_TV)
                {
                    continue;
                }
                if(((dev->support_ir[i].eIRType == IR_TYPE_RC6_MODE0)||(dev->support_ir[i].eIRType == IR_TYPE_RC6))&&(dev->support_ir[i].u8Enable == TRUE))
#else
