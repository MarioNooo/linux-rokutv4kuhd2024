...
u8 u8Cmd, u8 u8CmdInv, u8 u8Addr, u8 u8AddrInv)
{
	...
	u32 headCode = (u8Addr << 8) | u8AddrInv;
#endif
	for (i = 0; i < dev->support_num; i++) {
#ifdef CONFIG_ROKU_IR_MODIFY
		if (dev->support_ir[i].eIRType == IR_TYPE_NEC && dev->support_ir[i].u8Enable == TRUE) {
			// ignore HeadCode checking when KeyMap is Roku
			if (dev->support_ir[i].u32HeadCode == NUM_KEYMAP_ROKU_TV || headCode == dev->support_ir[i].u32HeadCode) {
#else
