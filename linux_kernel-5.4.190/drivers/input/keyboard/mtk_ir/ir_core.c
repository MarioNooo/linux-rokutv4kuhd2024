...
#ifdef CONFIG_ROKU_IR_MODIFY
#ifdef CONFIG_ROKU_EARLY_PWR_MODE_HANDLER
typedef void (*Roku_early_pwr_mode_handler_notify_fn_t)(u32 data);
static Roku_early_pwr_mode_handler_notify_fn_t roku_early_pwr_mode_handler_notify = NULL;
void MDrv_IR_Roku_Set_Early_Power_Mode_Handler(Roku_early_pwr_mode_handler_notify_fn_t notify_fn)
{
    if(notify_fn != NULL)
    {
        u32 event;
        ...
        // Enable early notification of IR key events
        roku_early_pwr_mode_handler_notify = notify_fn;
        struct mtk_ir_dev *dev = IRDev.pIRDev;
        if(dev == NULL || dev->raw == NULL)
        	return;

        // If there is already data available, process it now...
    	spin_lock_irqsave(&dev->raw->lock, flags);
    	if (dev->raw->this_sc.scancode != 0x0)
    	{
            sample_ready = true;
            event = dev->raw->this_sc.scancode;
			dev->raw->prev_sc.scancode_protocol = dev->raw->this_sc.scancode_protocol;
			dev->raw->prev_sc.scancode = dev->raw->this_sc.scancode;
            dev->raw->this_sc.scancode_protocol = 0;
            dev->raw->this_sc.scancode = 0x0;
            dev->raw->u8RepeatFlag = 0;
        }
        spin_unlock_irqrestore(&dev->raw->lock, flags);

        if (sample_ready)
            roku_early_pwr_mode_handler_notify(event);
    }
    else
    {
        // Disable early notification of IR key events
        roku_early_pwr_mode_handler_notify = NULL;
    }
}
EXPORT_SYMBOL(MDrv_IR_Roku_Set_Early_Power_Mode_Handler);
#endif // CONFIG_ROKU_EARLY_PWR_MODE_HANDLER
#endif // CONFIG_ROKU_IR_MODIFY
...
static int MIRC_Key_process(struct ir_raw_data_ctrl *raw)
{
	...
	// same protocol
	else if (raw->this_sc.scancode & 0x1FFFF00 != raw->prev_sc.scancode & 0x1FFFF00)
	{
		// diff key press
		//if before timeout send keyup + keydown
		if (time_after_eq((raw->keyup_jiffies + msecs_to_jiffies(eventtimeout)),jiffies))
	...
	{
		if ((raw->u8RepeatFlag == 0) && (!raw->dev->keypressed))
		{
		// press the same fast
		// if before timeout send keyup + keydown
			if (time_after_eq((raw->keyup_jiffies + msecs_to_jiffies(eventtimeout)), jiffies))
...
#ifdef CONFIG_ROKU_IR_MODIFY
#ifdef CONFIG_ROKU_EARLY_PWR_MODE_HANDLER
				// Check notify hook to see if we're in early pwr handling mode or not.
				// If we are, we don't wake-up the caller (if any), instead call the hook.
				// Following return, data is zerod and continue.
				if(roku_early_pwr_mode_handler_notify)
				{
					roku_early_pwr_mode_handler_notify(raw->this_sc.scancode);
				}
				else
#endif // CONFIG_ROKU_EARLY_PWR_MODE_HANDLER
				{
						  ...
						  dev, bool sync)
{
	...
	input_event(dev->input_dev, EV_MSC, MSC_SCAN,
				dev->last_scancode | KEY_RELEASE_FLAG);
#endif
							...
							u32 keycode)
{
	...
	dev->last_scancode = scancode;
    input_event(dev->input_dev, EV_MSC, MSC_SCAN,
				scancode | KEY_PRESS_FLAG);
#else
	input_event(dev->input_dev, EV_MSC, MSC_SCAN,
				scancode);
#endif
	if (new_event && keycode != KEY_RESERVED) {
#ifndef CONFIG_ROKU_IR_MODIFY
		/* Register a keypress */
		dev->keypressed = true;
		dev->last_scancode = scancode;
#endif
...
#ifdef CONFIG_ROKU_IR_MODIFY
void MIRC_KeyRepeat(struct mtk_ir_dev *dev)
{
	unsigned long flags	 = 0;
	if (!dev->keypressed)
		return;
	...
	input_event(dev->input_dev, EV_MSC, MSC_SCAN,
				dev->last_scancode | KEY_REPEAT_FLAG);
	input_sync(dev->input_dev);
