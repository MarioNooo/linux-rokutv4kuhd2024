...
retry:
	err = mtd_read(ubi->mtd, addr, len, &read, buf, NOT_ENCRYPTED);
	if (err) {
		 ...
		 int len)
{
	...
	addr = (loff_t)pnum * ubi->peb_size + offset;
	err = mtd_write(ubi->mtd, addr, len, &written, buf, NOT_ENCRYPTED);
	if (err) {
...
static int nor_erase_prepare(struct ubi_device *ubi, int pnum)
{
	...
	err = ubi_io_read_ec_hdr(ubi, pnum, &ec_hdr, NOT_ENCRYPTED);
	if (err != UBI_IO_BAD_HDR_EBADMSG && err != UBI_IO_BAD_HDR &&
	    err != UBI_IO_FF){
		err = mtd_write(ubi->mtd, addr, 4, &written, (void *)&data, NOT_ENCRYPTED);
		if(err)
			goto error;
	}
			    ...
			    int offset, int len)
{
	...
	err = mtd_read(ubi->mtd, addr, len, &read, buf1, NOT_ENCRYPTED);
