		...
		size_t *retlen, u_char *buf, int encrypted)
{
	...
	res = part->parent->_read(part->parent, from + part->offset, len,
				  retlen, buf, 0);
	// retlen, buf,  part->mtd.flags & MTD_ENCRYPTED);
		...
		struct mtd_oob_ops *ops, int encrypted)
{
	...
	BUG_ON(encrypted);
	stats = part->parent->ecc_stats;
	res = part->parent->_read_oob(part->parent, from + part->offset, ops, encrypted);
	if (unlikely(mtd_is_eccerr(res)))
		mtd->ecc_stats.failed +=
			part->parent->ecc_stats.failed - stats.failed;
	else
		...
		struct mtd_oob_ops *ops, int encrypted)
{
	struct mtd_part *part = mtd_to_part(mtd);
	BUG_ON(encrypted);
	return part->parent->_write_oob(part->parent, to + part->offset, ops, encrypted);
}
		...
		unsigned long count, loff_t to, size_t *retlen, int encrypted)
{
	...
	return part->parent->_writev(part->parent, vecs, count,
				     to + part->offset, retlen, encrypted);
