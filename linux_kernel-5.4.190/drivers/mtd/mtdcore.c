			      ...
			      void *val, size_t bytes)
{
	...
	err = mtd_read(mtd, offset, bytes, &retlen, val, 0);
	if (err && err != -EUCLEAN)
		return err;
...
int mtd_write(struct mtd_info *mtd, loff_t to, size_t len, size_t *retlen,
	      const u_char *buf, int encrypted)
{
	struct mtd_oob_ops ops = {
		.len = len,
		.datbuf = (u8 *)buf,
		    ...
		    const u_char *buf, int encrypted)
{
	*retlen = 0;
	...
	if (to < 0 || to >= mtd->size || len > mtd->size - to)
		return -EINVAL;
	if (!(mtd->flags & MTD_WRITEABLE)) {
#ifdef CONFIG_ROKU_PARTCONTROL
 		if (partcontrol_ops.write) {
 			int ret = partcontrol_ops.write(mtd, to, len, buf, encrypted);
 			if (ret < 0) {
 				return ret;
 			}
 		} else {
	...
	return mtd->_panic_write(mtd, to, len, retlen, buf, encrypted);
}
...
int mtd_read_oob(struct mtd_info *mtd, loff_t from, struct mtd_oob_ops *ops)
{
	...
	if (mtd->_read_oob)
		ret_code = mtd->_read_oob(mtd, from, ops, 0); // we send 0
	else
				...
				struct mtd_oob_ops *ops)
{
	...
	if (!(mtd->flags & MTD_WRITEABLE)) {
#ifdef CONFIG_ROKU_PARTCONTROL
		if (partcontrol_ops.write_oob) {
			int ret = partcontrol_ops.write_oob(mtd, to, ops);
			if (ret < 0) {
				return ret;
			}
		} else {
	...
	if (mtd->_write_oob)
		return mtd->_write_oob(mtd, to, ops, 0);
	else
	       ...
	       unsigned long count, loff_t to, size_t *retlen, int encrypted)
{
	*retlen = 0;
	...
	return mtd->_writev(mtd, vecs, count, to, retlen, encrypted);
}
