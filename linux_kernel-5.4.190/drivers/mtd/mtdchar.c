...
#ifdef CONFIG_ROKU_SWUPD
struct mtd_info *get_mtd_info(int minor)
{
	int devnum = minor >> 1;
			...
			loff_t *ppos)
{
	...
	while (count) {
		...
		default:
			ret = mtd_read(mtd, *ppos, len, &retlen, kbuf, 0);
		}
			...
			loff_t *ppos)
{
	...
	while (count) {
		...
		default:
			ret = mtd_write(mtd, *ppos, len, &retlen, kbuf, 0);
		}
...
#ifdef CONFIG_ROKU_SWUPD
int mtdchar_erase(int start, int length, struct mtd_info *mtd)
{
