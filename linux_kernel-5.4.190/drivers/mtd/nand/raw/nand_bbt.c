...
/* Used as the new last parameter to MTD read/write calls that require the encrypted flag. */
...
{
	/*
	 * Roku - Checking just the first byte against 0xFF is not sufficient
	 * to determine the presence of a bad block. If the number of '1' bits in
	 * the first 2 bytes of OOB is < 8 then the block can be added to bad block
	 * table with more centainity.
	 */
#if 0
	if (memcmp(buf + td->offs, td->pattern, td->len))
		return -1;
#else
	if (hweight8(*(buf + td->offs)) + hweight8(*(buf + td->offs + 1)) < 8) {
		return -1;
	}
#endif
	return 0;
		    ...
		    struct nand_bbt_descr *td, int offs)
{
	...
	while (totlen) {
		...
		res = mtd_read(mtd, from, len, &retlen, buf, NOT_ENCRYPTED);
		if (res < 0) {
			  ...
			  struct nand_bbt_descr *td)
{
	...
	return mtd_read(mtd, offs, len, &retlen, buf, NOT_ENCRYPTED);
}
		     ...
		     int chipsel)
{
	...
	while (chip < nrchips) {
		...
		if (td->options & NAND_BBT_SAVECONTENT) {
			...
			res = mtd_read(mtd, to, len, &retlen, buf, NOT_ENCRYPTED);
			if (res < 0) {
				if (retlen != len) {
