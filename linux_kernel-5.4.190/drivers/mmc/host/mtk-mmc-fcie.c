...
static void emmc_check_life(struct mtk_fcie_host *mtk_fcie_host,
		u32 DataByteCnt);
static void mtk_fcie_m6e3_execute_tuning(struct mtk_fcie_host *mtk_fcie_host);
...
{
	REG_SETBIT(REG_ADDR(mtk_fcie_host->fciebase, BOOT_CONFIG),
		   BIT_EMMC_RSTZ_EN);
				...
				u32 hz)
{
	...
	if (clk_param & EMMC_PLL_FLAG) {
		...
		Tmp = clk_param;
		//2sec= 0x10000 x 1/12MHZ x 0x170
		Tmp2 = 0x170;
		REG_CLRBIT(mtk_fcie_host->fcieclkreg,
			mtk_fcie_host->clk_mask);
		mtk_fcie_host->fcie_top_clk_rate =
...
static u32 mtk_fcie_pads_hs400(struct mtk_fcie_host *mtk_fcie_host)
{
	...
	if (mtk_fcie_host->emmc_pll_flash_macro)
		REG_SETBIT(REG_ADDR(mtk_fcie_host->emmcpllbase, reg_emmcpll_0x5f),
			BIT_FLASH_MACRO_TO_FICE);
	if (mtk_fcie_host->emmc_pll_skew1_inv) {
		REG_SETBIT(REG_ADDR(mtk_fcie_host->emmcpllbase, reg_emmcpll_0x69),
			BIT_SKEW1_CLK_INV);
		REG_CLRBIT(REG_ADDR(mtk_fcie_host->emmcpllbase, reg_emmcpll_0x03),
			BIT_SKEW1_MASK);
		REG_SETBIT(REG_ADDR(mtk_fcie_host->emmcpllbase, reg_emmcpll_0x03),
			BIT_SKEW1_FOR_INV);
	}
...
static void mtk_protect_dram(void)
{
#define TRIGGER_DRAM_PROTECTION 0x82000011
	struct arm_smccc_res res;

	arm_smccc_smc(TRIGGER_DRAM_PROTECTION, 0, 0, 0, 0, 0, 0, 0, &res);
}
...
irqreturn_t mtk_fcie_irq_thread(int irq, void *dummy)
{
	...
	if ((REG(REG_ADDR(mtk_fcie_host->fciebase, PWR_SAVE_CTL))&
		...
		gpio_request(PAD_OSDA0, "pad_osda0");
		gpio_set_value(PAD_OSDA0, 0); // Backlight SOC_BL_ON
		...
		*((uint32_t*)mmio_base_2426 + 0x0E) = *((uint32_t*)mmio_base_2426 + 0x0E) | 0x00; //16bit address TEST_MOD[5]=0:OP disable=0
		...
		gpio_request(PAD_SAR3, "pad_sar3");
		gpio_set_value(PAD_SAR3, 0); // Panel SOC_PANEL_ON

		gpio_request(PAD_I2SIN_SD2_A, "pad_i2sin_sd2_a");
		gpio_set_value(PAD_I2SIN_SD2_A, 0); // Speakers AMP_MUTEn

		gpio_request(PAD_I2SIN_WS_A, "pad_i2sin_ws_a");
		gpio_set_value(PAD_I2SIN_WS_A, 0); // Headphones HP_MUTEn

		gpio_request(PAD_VID2, "pad_vid2");
		gpio_set_value(PAD_VID2, 0); // USB USB_EN

		gpio_request(PAD_LED_PWM1, "pad_led_pwm1");
		gpio_set_value(PAD_LED_PWM1, 0); // Wifi WIFI_RSTn

		gpio_request(PAD_PCM_CE_N, "pad_pcm_ce_n");
		gpio_set_value(PAD_PCM_CE_N, 0); // PCM_PWR_EN

		if (mtk_fcie_host->enable_drop_recovery) {
			// reset FCIE power-saving mode
			REG_W(REG_ADDR(mtk_fcie_host->fciebase, PWR_SAVE_CTL), 0); /* active low */
			mtk_fcie_hw_timer_delay(HW_TIMER_DELAY_100US);
			REG_W(REG_ADDR(mtk_fcie_host->fciebase, PWR_SAVE_CTL),
				BIT_SD_POWER_SAVE_RST);

			mtk_fcie_hw_timer_delay(mtk_fcie_host->drop_recovery_time *
				HW_TIMER_DELAY_1MS);

			regval = REG(REG_ADDR(mtk_fcie_host->vplugpwrbase, reg_vplug_pwr_0x62));

			if ((regval & BIT_VPLUG_IN_PWRGD) == 0) {
				mtk_protect_dram();//M6L by Ben-YC
				...
				mtk_fcie_host->error |= REQ_VPLUG_DROP;
...
static int mtk_fcie_hw_init(struct mtk_fcie_host *mtk_fcie_host)
{
	...
	if (mtk_fcie_host->host_driving) {
		//1.8V must set this bit
		REG_CLRBIT(REG_ADDR(mtk_fcie_host->emmcpllbase, reg_emmcpll_0x45),
			BIT_HOST_DRIVING_MASK);
		REG_SETBIT(REG_ADDR(mtk_fcie_host->emmcpllbase, reg_emmcpll_0x45),
...
void  mtk_fcie_set_tx_reg(struct mtk_fcie_host *mtk_fcie_host)
{
	u8 Skew2 = 0;
	struct fcie_emmc_driver *emmc_drv = mtk_fcie_host->emmc_drv;

	if (emmc_drv->t_hs400_table.setcnt)
		Skew2 = emmc_drv->t_hs400_table.set_skew4.skew2;

	if (emmc_drv->t_hs400_table.setcnt) {
		REG_CLRBIT(REG_ADDR(mtk_fcie_host->emmcpllbase, reg_emmcpll_0x03),
			BIT_SKEW2_MASK);
		REG_SETBIT(REG_ADDR(mtk_fcie_host->emmcpllbase, reg_emmcpll_0x03),
			Skew2 << BIT_SKEW2_SHIFT);
	}
}

void  mtk_fcie_set_skew4_rsp_reg(struct mtk_fcie_host *mtk_fcie_host)
{
	u8 skew4 = 0, reg_2ch = 0;
	struct fcie_emmc_driver *emmc_drv = mtk_fcie_host->emmc_drv;

	skew4 = emmc_drv->t_hs400_table.set_skew4.skew4_cmd;
	reg_2ch = emmc_drv->t_hs400_table.set_skew4.reg2ch_cmd;

	if (reg_2ch) {
		REG_SETBIT(REG_ADDR(mtk_fcie_host->emmcpllbase, reg_emmcpll_0x6c),
			BIT_SKEW4_REG2C_INV);
	} else {
		REG_CLRBIT(REG_ADDR(mtk_fcie_host->emmcpllbase, reg_emmcpll_0x6c),
			BIT_SKEW4_REG2C_INV);
	}

	REG_CLRBIT(REG_ADDR(mtk_fcie_host->emmcpllbase, reg_emmcpll_0x03), BIT_SKEW4_MASK);
	REG_SETBIT(REG_ADDR(mtk_fcie_host->emmcpllbase, reg_emmcpll_0x03),
		skew4 << BIT_SKEW4_SHIFT);
}

void mtk_fcie_set_skew4_data_reg(struct mtk_fcie_host *mtk_fcie_host)
{
	u8  skew4 = 0, reg_2ch = 0;
	struct fcie_emmc_driver *emmc_drv = mtk_fcie_host->emmc_drv;

	skew4 = emmc_drv->t_hs400_table.set_skew4.skew4_data;
	reg_2ch = emmc_drv->t_hs400_table.set_skew4.reg2ch_data;
...
void mtk_fcie_apply_timing_set(u8 idx, struct mtk_fcie_host *mtk_fcie_host)
{
	...
	if (emmc_drv->t_table_g.set_cnt == 0) {
		if (mtk_fcie_host->emmc_pll_skew4 && emmc_drv->t_hs400_table.setcnt) {
			mtk_fcie_clock_setting(emmc_drv->t_hs400_table.set_skew4.clk,
					mtk_fcie_host, EMMC_CLK_200M);
			mtk_fcie_set_tx_reg(mtk_fcie_host);
		} else {
	...
	u8 pad_type, u8 *buf)
{
	...
	if (mtk_fcie_host->emmc_pll_skew4 &&
		...
		reg_2ch = tab->set[0].reg2ch;
		skew4 = tab->set[0].skew4;
		emmc_drv->cur_skew4 = (reg_2ch > 0) ? (PLL_SKEW4_CNT+skew4) : skew4;
	}
	return EMMC_ST_SUCCESS;
}
...
static u32 _mtk_fcie_load_hs400timing_table(struct mtk_fcie_host *mtk_fcie_host, u8 *buf)
{
	u32 err;
	u32 chk_sum, emmc_blk_addr = eMMC_HS400SKEW4TABLE_BLK_0;
	struct fcie_hs400_timing_table *tab = (struct fcie_hs400_timing_table *)buf;
	struct fcie_emmc_driver *emmc_drv = mtk_fcie_host->emmc_drv;
	u8 *sect_buf;
	u8  reg_2ch = 0, skew4 = 0;

	sect_buf = kzalloc(EMMC_SECTOR_512BYTE, GFP_NOIO);
	if (!sect_buf)
		return EMMC_ST_ERR_PARAMETER;
	...
	reg_2ch = tab->set_skew4.reg2ch_data;
	skew4 = tab->set_skew4.skew4_data;
	emmc_drv->cur_skew4 = (reg_2ch > 0) ? (PLL_SKEW4_CNT + skew4) : skew4;
...
no_table:
	...
	emmc_drv->t_hs400_table.setcnt = 0;
	emmc_drv->t_hs400_table.chk_sum = 0;
	emmc_drv->t_hs400_table.ver_no = 0;
	...
	u8 pad_type)
{
	...
	if (mtk_fcie_host->emmc_pll_skew4) {
		if (pad_type == FCIE_EMMC_HS400) {
			err = _mtk_fcie_load_hs400timing_table(mtk_fcie_host,
				(u8 *)&emmc_drv->t_hs400_table);
			if (err != EMMC_ST_SUCCESS)
				goto no_table;
		} else {
			//--------------------------------------
...
static void mtk_fcie_request_done(struct mtk_fcie_host *mtk_fcie_host, struct mmc_request *mrq)
{
	struct mmc_data *data = NULL;
	int ret = 0;
	...
	if (mrq->sbc)
		sbc_error = mrq->sbc->error;

	//Cancel delayed work when  no CMD23 error & REQ_CMD_BUSY & REQ_INVALID_CMD
	if (!sbc_error && (mtk_fcie_host->error & REQ_WITHOUT_WORK_QUEUE) == 0) {
		/*
		 * No need check the return value of cancel_delayed_work, as only ONE
		 * path will go here!
		 */
		cancel_delayed_work(&mtk_fcie_host->req_timeout_work);
	}
	...
	if (data) {
		...
		if (mrq->cmd->opcode == MMC_SEND_EXT_CSD && data->error == 0)
			mtk_fcie_config_ecsd(data, mtk_fcie_host->mmc);
	}

	if (mtk_fcie_host->error & REQ_ERR_RETRY_MASK) {
/*
 ...
 */
		...
		if (mtk_fcie_host->emmc_pll_skew4) {
			if ((mtk_fcie_host->error & (REQ_CMD_EIO|REQ_DAT_EILSEQ)) &&
	...
	spin_lock_irqsave(&mtk_fcie_host->lock, flags);
	mtk_fcie_host->request = NULL;
	spin_unlock_irqrestore(&mtk_fcie_host->lock, flags);

	mtk_fcie_unlock_host((u8 *)__func__, mtk_fcie_host);
	mmc_request_done(mtk_fcie_host->mmc, mrq);
}

static int mtk_fcie_skip_response_err(struct mmc_command *cmd)
{
	int ret = 0;
...
static void mtk_fcie_rsp_136(struct mtk_fcie_host *mtk_fcie_host,
	struct mmc_command *cmd,
	u32 *fifo)
{
	fifo[CMD_FIFO_INDEX_0] = REG(REG_ADDR(mtk_fcie_host->fciebase, CMD_FIFO));
	fifo[CMD_FIFO_INDEX_1] = REG(REG_ADDR(mtk_fcie_host->fciebase, CMD_FIFO+CMD_FIFO_INDEX_1));
	fifo[CMD_FIFO_INDEX_2] = REG(REG_ADDR(mtk_fcie_host->fciebase, CMD_FIFO+CMD_FIFO_INDEX_2));
...
static void mtk_fcie_m6e3_skip_rsp_err_es(struct mtk_fcie_host *mtk_fcie_host,
	struct mmc_command *cmd)
{
	struct fcie_emmc_driver *emmc_drv = NULL;
...
static void mtk_fcie_m6e3_skip_rsp_err_without_es(struct mtk_fcie_host *mtk_fcie_host,
	struct mmc_command *cmd)
{
	u16 status = 0;
	...
	if (cmd->error != 0) {
		EMMC_DBG(mtk_fcie_host,
			EMMC_DBG_LVL_ERROR, 1,
			"eMMC Err: ST:%Xh, CMD%u(%xh) RT:%u\n",
			status, cmd->opcode,
			cmd->arg, cmd->retries);
	}
}

static void mtk_fcie_chk_r1(struct mtk_fcie_host *mtk_fcie_host,
	struct mmc_command *cmd)
{
	...
	u16 events)
{
	...
	if (cmd->flags & MMC_RSP_PRESENT)
		mtk_fcie_rsp_136(mtk_fcie_host, cmd, fifo);
	...
	if (events & BIT_ERR_STS) {
		...
		EMMC_DBG(mtk_fcie_host,
			EMMC_DBG_LVL_ERROR, 1,
			"eMMC Err: ST:%Xh, EVT:%Xh, CMD%u(%xh) RT:%u\n",
		...
		if (status & BIT_SD_RSP_TIMEOUT) {
			...
			if (mtk_fcie_host->emmc_pll_skew4 &&
				(emmc_drv->pad_type == FCIE_EMMC_HS400_5_1 &&
				emmc_drv->t_table_g.set_cnt)) {
				mtk_fcie_m6e3_skip_rsp_err_es(mtk_fcie_host, cmd);
			} else {
				cmd->error = -EILSEQ;
				mtk_fcie_host->error |= REQ_CMD_EIO;
			}
		...
		if (!(mmc_resp_type(cmd) & MMC_RSP_CRC)) {
			...
			if (mtk_fcie_host->emmc_pll_skew4 &&
				emmc_drv->pad_type == FCIE_EMMC_HS400 &&
				emmc_drv->t_hs400_table.setcnt) {
				mtk_fcie_m6e3_skip_rsp_err_without_es(mtk_fcie_host, cmd);
			} else {
				cmd->error = 0;
			}
		}
	...
	if (mmc_resp_type(cmd) == MMC_RSP_R1 ||
		...
		mtk_fcie_chk_r1(mtk_fcie_host, cmd);
	}
}
				...
				struct mmc_request *mrq, struct mmc_data *data)
{
	if (mmc_op_multi(mrq->cmd->opcode) && mrq->stop && !mrq->stop->error &&
		...
		mtk_fcie_request_done(mtk_fcie_host, mrq);
}

static void mtk_fcie_m6e3_chk_data_crc_without_es(struct mtk_fcie_host *mtk_fcie_host,
	struct mmc_command *cmd,
	struct mmc_data *data)
{
	...
	} else if (status & (BIT_SD_R_CRC_ERR | BIT_DAT_WR_CERR)) {
		data->error = -EILSEQ;
				...
				struct mmc_request *mrq, struct mmc_data *data)
{
	...
	if (events & BIT_ERR_STS) {
		...
		if (mtk_fcie_host->emmc_pll_skew4 &&
			emmc_drv->t_hs400_table.setcnt &&
			emmc_drv->pad_type == FCIE_EMMC_HS400) {
			mtk_fcie_m6e3_chk_data_crc_without_es(mtk_fcie_host, cmd, data);
		} else {
			data->error = 0;
		}
	}
...
dma_end:
	...
	mtk_fcie_data_xfer_next(mtk_fcie_host, mrq, data);
	...
	struct mmc_data *data)
{
	...
	if ((mtk_fcie_host->emmc_drv->partition_config &
		EXT_CSD_PART_CONFIG_ACC_MASK) == 0) {
	...
	struct mmc_command *cmd)
{
	...
	reg_val = REG(REG_ADDR(mtk_fcie_host->fciebase, MIE_EVENT));
...
static void emmc_skew4_data_reg(struct mtk_fcie_host *mtk_fcie_host)
{
	struct fcie_emmc_driver *emmc_drv = NULL;
	...
	if (mtk_fcie_host->emmc_pll_skew4 &&
		emmc_drv->t_hs400_table.setcnt &&
		emmc_drv->pad_type == FCIE_EMMC_HS400) {
		mtk_fcie_set_skew4_data_reg(mtk_fcie_host);
	}
}

static void emmc_skew4_rsp_reg(struct mtk_fcie_host *mtk_fcie_host)
{
	struct fcie_emmc_driver *emmc_drv = NULL;
	...
	if (mtk_fcie_host->emmc_pll_skew4 &&
		emmc_drv->t_hs400_table.setcnt &&
		emmc_drv->pad_type == FCIE_EMMC_HS400) {
		mtk_fcie_set_skew4_rsp_reg(mtk_fcie_host);
	}
}
	...
	struct mmc_command *cmd)
{
	...
	if (data) {
		...
		if (data->flags & MMC_DATA_READ) {
			...
			emmc_skew4_data_reg(mtk_fcie_host);
		}

		if (data->flags & MMC_DATA_WRITE)
			emmc_skew4_rsp_reg(mtk_fcie_host);
	} else {
		emmc_skew4_rsp_reg(mtk_fcie_host);
	}
	...
	if (mtk_fcie_host->emmc_cmd_log_enable) {
		EMMC_DBG(mtk_fcie_host, EMMC_DBG_LVL_ERROR, 0,
	...
	struct mmc_data *data)
{
	...
	if (mtk_fcie_host->emmc_pll_skew4 &&
		emmc_drv->t_hs400_table.setcnt &&
		emmc_drv->pad_type == FCIE_EMMC_HS400) {
		mtk_fcie_host->reg_int_en |= BIT_DMA_END;
	} else {
		mtk_fcie_host->reg_int_en |= (BIT_DMA_END | BIT_ERR_STS);
	}
	REG_W(REG_ADDR(mtk_fcie_host->fciebase, MIE_INT_EN), mtk_fcie_host->reg_int_en);
	...
	if (data->flags & MMC_DATA_WRITE) {
		...
		if (mtk_fcie_host->emmc_pll_skew4 &&
			emmc_drv->t_hs400_table.setcnt &&
			emmc_drv->pad_type == FCIE_EMMC_HS400) {
			sd_ctl = (BIT_SD_DTRX_EN | BIT_SD_DAT_DIR_W);
		} else {
			sd_ctl = (BIT_SD_DTRX_EN | BIT_SD_DAT_DIR_W | BIT_ERR_DET_ON);
		}
...
irqreturn_t mtk_fcie_irq(int irq, void *dummy)
{
	...
	while (true) {
		...
		if (!mrq) {
			EMMC_DBG(mtk_fcie_host, EMMC_DBG_LVL_ERROR, 1,
				"eMMC Err: MRQ=NULL; events=%08X; event_mask=%08X\n",
				events, event_mask);
			break;
		}
		...
		if (cmd)
			...
			mtk_fcie_data_xfer_done(mtk_fcie_host, events, mrq, data);
	...
	struct mmc_request *mrq)
{
	...
	if (mmc_host->ios.power_mode == MMC_POWER_OFF) {
		...
		mtk_fcie_host->error |= REQ_INVALID_CMD;
		EMMC_DBG(mtk_fcie_host, EMMC_DBG_LVL_ERROR, 0,
	...
	if ((mrq->cmd->opcode & CMD_OPCODE_MASK) == MMC_GO_INACTIVE_STATE) {
		mtk_fcie_host->request->cmd->error = 0;
		mtk_fcie_host->error |= REQ_INVALID_CMD;
	...
	if (mrq->cmd->opcode != MMC_SEND_STATUS) {
		if (!mtk_fcie_cmd_is_ready(mtk_fcie_host)) {
			...
			mtk_fcie_host->error |= REQ_CMD_BUSY;
			mtk_fcie_request_done(mtk_fcie_host, mrq);
			return;
		}
	}
	...
	if (mrq->data)
		mtk_fcie_pre_dma_transfer(mtk_fcie_host, mrq->data, NULL);
...
static void mtk_fcie_req_timeout(struct work_struct *work)
{
	...
	u16 event, int_events, mmc_debug_mode;
	struct mmc_request *mrq = NULL;
	unsigned long flags;
	...
	if (mrq) {
		...
		if (mtk_fcie_host->cmd) {
			EMMC_DBG(mtk_fcie_host, EMMC_DBG_LVL_ERROR, 0,
				"eMMC Err: aborting cmd: event=%Xh, int_events=%Xh\n",
				event,
				int_events);
			mtk_fcie_cmd_done(mtk_fcie_host, BIT_INT_CMDTMO,
				mrq, mtk_fcie_host->cmd);
		} else if (mtk_fcie_host->data) {
			//Checked if waiting for a start bit from device timeout happended or not?
			...
			REG_SETBIT(REG_ADDR(mtk_fcie_host->fciebase, TEST_MODE),
				BIT_DATA_DBUS_MODE << BIT_TEST_MODE_SHIFT);
			mmc_debug_mode = REG(REG_ADDR(mtk_fcie_host->fciebase, EMMC_DEBUG_BUS0));

			EMMC_DBG(mtk_fcie_host, EMMC_DBG_LVL_ERROR, 0,
			    "eMMC Err: abort data: event = %Xh, int_events = %Xh, mmc_debug_mode = %Xh\n",
			    event,
			    int_events,
			    mmc_debug_mode);

			mtk_fcie_data_xfer_done(mtk_fcie_host, BIT_INT_DATATMO,
				mrq, mtk_fcie_host->data);
		}
	}
}
/*
 * Ioctl fuzzer send illegal command without data to eMMC,
 * but kernel did not reinit eMMC. No command response may
 * always happen.
 */
static void mtk_fcie_recovery(struct work_struct *work)
{
	...
	if (mtk_fcie_host->emmc_pll_skew4) {
		if (mtk_fcie_host->error & (REQ_CMD_EIO|REQ_DAT_EILSEQ)) {
			mtk_fcie_m6e3_execute_tuning(mtk_fcie_host);
			mtk_fcie_unlock_host((u8 *)__func__, mtk_fcie_host);
			mtk_fcie_request(mtk_fcie_host->mmc, mtk_fcie_host->request);
			return;
		}
		if (mtk_fcie_host->error & REQ_VPLUG_DROP) {
			mtk_fcie_err_handler(mtk_fcie_host);
			if (mtk_fcie_host->emmc_active) {
				mtk_fcie_unlock_host((u8 *)__func__, mtk_fcie_host);
				mtk_fcie_request(mtk_fcie_host->mmc, mtk_fcie_host->request);
			}
		...
		if (mmc_ios->timing == MMC_TIMING_LEGACY) {
			...
			if (!has_t_table) {
				if (emmc_drv->ecsd184_stroe_support &&
					...
					EMMC_DBG(mtk_fcie_host,
						0, 1, "Ld HS400ES\n");
					err = mtk_fcie_load_timing_table(mtk_fcie_host,
						FCIE_EMMC_HS400_5_1);
					if (err)
						EMMC_DBG(mtk_fcie_host,
							0, 1,
				...
				if ((emmc_drv->ecsd196_devtype & ECSD_DDR) &&
					(emmc_drv->ecsd196_devtype & ECSD_HS200_HS400_MASK) == 0 &&
...
load_end:
			...
			if (mtk_fcie_host->emmc_pll_skew4) {
				if (emmc_drv->t_table_g.set_cnt &&
					pad_type == FCIE_EMMC_HS400_5_1) {
					curset_idx = emmc_drv->t_table_g.curset_idx;
				} else {
					curset_idx = EMMC_TIMING_SET_MAX;
				}
			} else {
				if (emmc_drv->t_table_g.set_cnt &&
					(pad_type == FCIE_EMMC_HS400 ||
			...
			mtk_fcie_apply_timing_set(curset_idx, mtk_fcie_host);
...
static u32 mtk_fcie_cmd6(struct mtk_fcie_host *mtk_fcie_host, u32 arg)
{
	...
	err = mtk_fcie_err_send_cmd(mtk_fcie_host, emmc_drv->reg10_mode,
		ctrl, arg, MMC_SWITCH, EMMC_R1B_BYTE_CNT);
	...
	if (reg & (BIT_SD_RSP_TIMEOUT|BIT_SD_RSP_CRC_ERR)) {
		if (reg & BIT_SD_RSP_CRC_ERR)
			err = EMMC_ST_ERR_CMD6_RESP_CRC;
		else
...
static u32 mtk_fcie_cmd6_skew4(struct mtk_fcie_host *mtk_fcie_host, u32 arg)
{
	struct fcie_emmc_driver *emmc_drv = mtk_fcie_host->emmc_drv;
	u32 err;
	u16 ctrl, reg;
	...
	if (emmc_drv->pad_type == FCIE_EMMC_HS400 && emmc_drv->t_hs400_table.setcnt) {
		skew = REG(REG_ADDR(mtk_fcie_host->emmcpllbase, reg_emmcpll_0x03));
		reg_2ch = (REG(REG_ADDR(mtk_fcie_host->emmcpllbase, reg_emmcpll_0x6c)) &
			BIT_SKEW4_REG2C_INV) ? 1 : 0;
		mtk_fcie_set_skew4_rsp_reg(mtk_fcie_host);
	}
	...
	if (emmc_drv->pad_type == FCIE_EMMC_HS400 &&
		emmc_drv->t_hs400_table.setcnt) {
		REG_W(REG_ADDR(mtk_fcie_host->emmcpllbase, reg_emmcpll_0x03), skew);
		if (reg_2ch) {
			REG_SETBIT(REG_ADDR(mtk_fcie_host->emmcpllbase, reg_emmcpll_0x6c),
				BIT_SKEW4_REG2C_INV);
		} else {
...
static u32  mtk_fcie_detect_timing_skew4_ex_not_wrapped(struct mtk_fcie_host *mtk_fcie_host,
	u16 start_bit,
	u16 end_bit,
	u32 skew4_result,
	u8 *skew4_idx,
	u8 *reg_2ch)
{
	...
	for (i = 0; i < TOTAL_PLL_SKEW4_CNT; i++) {
		if ((skew4_result&1<<i) && start_bit == START_BIT_DEFAULT)
			start_bit = i;
		else if (start_bit != START_BIT_DEFAULT && (skew4_result&1<<i) == 0)
			end_bit = i-1;

		if (end_bit != END_BIT_DEFAULT) {
			if (end_bit-start_bit+1 < M6E3_MIN_OK_SKEW_CNT) {
				start_bit = START_BIT_DEFAULT;
				end_bit = END_BIT_DEFAULT;
			} else {
				break;
			}
		}
	...
	if (end_bit-start_bit+1 < M6E3_MIN_OK_SKEW_CNT) {
		EMMC_DBG(mtk_fcie_host, 0, 1, "eMMC Err: Clk:%uMHz not enough skew4: %Xh %Xh\n",
			emmc_drv->clk_khz/EMMC_CLK_KHZ, skew4_result, end_bit-start_bit+1);
		return EMMC_ST_ERR_SKEW4;
	}

	if (((start_bit+end_bit)>>1) < PLL_SKEW4_CNT) {
		*reg_2ch = 0;
		*skew4_idx = (start_bit + end_bit) >> 1;
	} else {
		*reg_2ch = 1;
		*skew4_idx = ((start_bit+end_bit) >> 1) - PLL_SKEW4_CNT;
	}
...
static u32  mtk_fcie_detect_timing_skew4_ex_wrapped(struct mtk_fcie_host *mtk_fcie_host,
	u16 start_bit,
	u16 end_bit,
	u32 skew4_result,
	u8 *skew4_idx,
	u8 *reg_2ch)
{
	...
	// normal judgement
	for (i = 0; i < TOTAL_PLL_SKEW4_CNT; i++) {
		if ((skew4_result&1<<i) == 0 && start_bit == START_BIT_DEFAULT)
			start_bit = i-1;
		else if (start_bit != START_BIT_DEFAULT && (skew4_result&1<<i))
			end_bit = i;
	...
	if (start_bit+1 > TOTAL_PLL_SKEW4_CNT-end_bit) {
		*reg_2ch = 0;
		*skew4_idx = (start_bit - (TOTAL_PLL_SKEW4_CNT - end_bit)) >> 1;
	} else {
		*reg_2ch = 1;
		*skew4_idx = end_bit +
			(((TOTAL_PLL_SKEW4_CNT - end_bit) + start_bit) >> 1) -
			PLL_SKEW4_CNT;
	}
...
static u32  mtk_fcie_detect_timing_skew4_ex(struct mtk_fcie_host *mtk_fcie_host,
	u32 skew4_result,
	u8 *skew4_idx,
	u8 *reg_2ch)
{
	u16 i, start_bit = START_BIT_DEFAULT, end_bit = END_BIT_DEFAULT, cnt;
	struct fcie_emmc_driver *emmc_drv = NULL;
	u32 err;
	...
	if ((skew4_result & ((1 << TOTAL_PLL_SKEW4_CNT) - 1)) ==
		((1 << TOTAL_PLL_SKEW4_CNT) - 1)) {
		*skew4_idx = 1;
		*reg_2ch = 0;
	} else if ((skew4_result&BIT(0)) == 0 ||
			(skew4_result & (1 << (TOTAL_PLL_SKEW4_CNT - 1))) == 0) {
		err = mtk_fcie_detect_timing_skew4_ex_not_wrapped(mtk_fcie_host,
			start_bit,
			end_bit,
			skew4_result,
			skew4_idx,
		...
		for (i = TOTAL_PLL_SKEW4_CNT-1; i > 0; i--) {
			if (skew4_result & (1<<i))
				cnt++;
			else {
				end_bit = i+1;
		...
		if (cnt < M6E3_MIN_OK_SKEW_CNT) {
			skew4_result &= ~((1 << (start_bit + 1)) - 1);
			return mtk_fcie_detect_timing_skew4_ex(mtk_fcie_host,
				skew4_result,
				skew4_idx,
				reg_2ch);
		}
		...
		err = mtk_fcie_detect_timing_skew4_ex_wrapped(mtk_fcie_host,
			start_bit,
			end_bit,
			skew4_result,
			skew4_idx,
...
static dma_addr_t mtk_fcie_dma_map_address(struct mtk_fcie_host *mtk_fcie_host,
	uintptr_t ulongBuffer,
	u32 ByteCnt,
	int mode)
{
	dma_addr_t dma_addr;
	...
	if (mode == 0)//write
		dma_addr = dma_map_single(mtk_fcie_host->dev,
			(void *)ulongBuffer, ByteCnt, DMA_TO_DEVICE);
	else
		dma_addr = dma_map_single(mtk_fcie_host->dev,
			(void *)ulongBuffer, ByteCnt, DMA_FROM_DEVICE);

	if (dma_mapping_error(mtk_fcie_host->dev, dma_addr)) {
		dma_unmap_single(mtk_fcie_host->dev, dma_addr, ByteCnt,
			(mode) ? DMA_FROM_DEVICE : DMA_TO_DEVICE);
		EMMC_DBG(mtk_fcie_host, EMMC_DBG_LVL_ERROR, 1,
			"eMMC Err: CeMMC_DMA_MAP_address: Kernel can't mapping dma correctly\n");
	}

	return dma_addr;
}

static void mtk_fcie_dma_unmap_address(struct mtk_fcie_host *mtk_fcie_host,
	dma_addr_t dma_DMAAddr,
	u32 ByteCnt,
	int mode)
{

	if (mode == 0)   //write
		dma_unmap_single(mtk_fcie_host->dev, dma_DMAAddr, ByteCnt, DMA_TO_DEVICE);
	else
		dma_unmap_single(mtk_fcie_host->dev, dma_DMAAddr, ByteCnt, DMA_FROM_DEVICE);
}

static u32 mtk_fcie_cmd17(struct mtk_fcie_host *mtk_fcie_host,
	u8 *data_buf,
	ulong blkaddr)
{
	struct fcie_emmc_driver *emmc_drv = NULL;
	...
	arg = blkaddr << (emmc_drv->if_sector_mode?0:EMMC_SECTOR_512BYTE_BITS);
	ctrl = (BIT_SD_CMD_EN | BIT_SD_RSP_EN | BIT_SD_DAT_EN);
	mtk_fcie_clear_events(mtk_fcie_host);

	REG_W(REG_ADDR(mtk_fcie_host->fciebase, JOB_BL_CNT), 1);
	REG_W(REG_ADDR(mtk_fcie_host->fciebase, BLK_SIZE), EMMC_SECTOR_512BYTE);
	dma_DataMapAddr = mtk_fcie_dma_map_address(mtk_fcie_host,
		(uintptr_t)data_buf, EMMC_SECTOR_512BYTE, 1);
	uint_DataDMAAddr = dma_DataMapAddr - (dma_addr_t)mtk_fcie_host->miu0_base;

	REG_W(REG_ADDR(mtk_fcie_host->fciebase, MIU_DMA_LEN_15_0),
		EMMC_SECTOR_512BYTE & DMA_15_0_MASK);
	REG_W(REG_ADDR(mtk_fcie_host->fciebase, MIU_DMA_LEN_31_16),
		EMMC_SECTOR_512BYTE >> DMA_31_16_SHIFT);
	REG_W(REG_ADDR(mtk_fcie_host->fciebase, JOB_BL_CNT),
		EMMC_SECTOR_512BYTE>>EMMC_SECTOR_512BYTE_BITS);

	REG_W(REG_ADDR(mtk_fcie_host->fciebase, MIU_DMA_ADDR_15_0),
		uint_DataDMAAddr & DMA_15_0_MASK);
	REG_W(REG_ADDR(mtk_fcie_host->fciebase, MIU_DMA_ADDR_31_16),
		(uint_DataDMAAddr >> DMA_31_16_SHIFT) & DMA_15_0_MASK);

	if (sizeof(dma_addr_t) == DMA_ADDR_64_BYTE)
		REG_W(REG_ADDR(mtk_fcie_host->fciebase, MIU_DMA_ADDR_47_32),
			(u64)uint_DataDMAAddr >> DMA_47_32_SHIFT);

	err = mtk_fcie_err_send_cmd(mtk_fcie_host, emmc_drv->reg10_mode,
		ctrl, arg, MMC_READ_SINGLE_BLOCK, EMMC_R1_BYTE_CNT);
	...
	err = mtk_fcie_err_polling(mtk_fcie_host,
		(uintptr_t)REG_ADDR(mtk_fcie_host->fciebase, MIE_EVENT),
		BIT_DMA_END, HW_TIMER_DELAY_10MS);
	...
	if (err != EMMC_ST_SUCCESS) {
		...
		if (reg & BIT_SD_FCIE_ERR_FLAGS) {
			EMMC_DBG(mtk_fcie_host, EMMC_DBG_LVL_LOW, 1,
				"eMMC Err: CMD17 SD_STS: %04Xh\n", reg);

			if (reg & BIT_SD_RSP_CRC_ERR)
				err = EMMC_ST_ERR_CMD17_RESP_CRC;
			else if (reg & BIT_SD_R_CRC_ERR)
				err = EMMC_ST_ERR_CMD17_DATA_CRC;
			else
				err = EMMC_ST_ERR_CMD17_MIU;
		} else {
			err = mtk_fcie_check_r1(mtk_fcie_host);
			if (err != EMMC_ST_SUCCESS) {
				EMMC_DBG(mtk_fcie_host, EMMC_DBG_LVL_LOW, 1,
					"eMMC Err: CMD17 check R1 error\n");
			}
		}
	}
...
LABEL_END:
	...
	mtk_fcie_dma_unmap_address(mtk_fcie_host, dma_DataMapAddr, EMMC_SECTOR_512BYTE, 1);
	return err;
}

static u32 mtk_fcie_cmd17_skew4(struct mtk_fcie_host *mtk_fcie_host,
	u8 *data_buf,
	ulong blkaddr)
{
	struct fcie_emmc_driver *emmc_drv = NULL;
	...
	dma_addr_t uint_DataDMAAddr = 0;
	dma_addr_t dma_DataMapAddr = 0;
	u16 status_err_flag = BIT_SD_FCIE_ERR_FLAGS;
	...
	status_err_flag &= ~BIT_CMD_RSP_CERR;//Ignore repsonse crc error in HS400 mode on M6E3
...
static u32 mtk_fcie_m6e3_probe_timing(struct mtk_fcie_host *mtk_fcie_host,
	u8 *data_buf,
	u16 skew4)
{
	u32 err = 0, arg = 0;
	u8 partition_config = 0;
	u8 switch_to_user_area = 0;
	struct fcie_emmc_driver *emmc_drv = NULL;
	...
	if (mtk_fcie_host->emmc_drv->partition_config & EXT_CSD_PART_CONFIG_ACC_MASK)
		switch_to_user_area = 1;
	...
	if (skew4 < PLL_SKEW4_CNT) {
		...
		REG_SETBIT(REG_ADDR(mtk_fcie_host->emmcpllbase, reg_emmcpll_0x6c),
			BIT_SKEW4_REG2C_INV);
		REG_SETBIT(REG_ADDR(mtk_fcie_host->emmcpllbase, reg_emmcpll_0x03),
			(skew4-PLL_SKEW4_CNT)<<BIT_SKEW4_SHIFT);
	}

	if (switch_to_user_area) {
		partition_config = mtk_fcie_host->emmc_drv->partition_config;
		partition_config &= ~EMMC_EXTCSD_ACCESS_MASK;
		arg = ((EMMC_EXTCSD_WBYTE & EMMC_EXTCSD_ACCESS_MASK) << EMMC_EXTCSD_ACCESS_SHIFT) |
			(EXT_CSD_PART_CONFIG << EMMC_EXTCSD_IDX_SHIFT) |
			(partition_config << EMMC_EXTCSD_VALUE_SHIFT);

		err = mtk_fcie_cmd6_skew4(mtk_fcie_host, arg);

		if (err)
			goto LABEL_END;
	}
	if (emmc_drv->pad_type == FCIE_EMMC_HS400)
		err = mtk_fcie_cmd17_skew4(mtk_fcie_host, data_buf, EMMC_CRAZY_PATTERN_BLK);
	else
		err = mtk_fcie_cmd17(mtk_fcie_host, data_buf, EMMC_CRAZY_PATTERN_BLK);
...
static u16 mtk_fcie_m6e3_skew4_start_bit(int start_bit)
{
	u16 startbit = 0;

	if (start_bit < 0)
		startbit = (u16)(start_bit + TOTAL_PLL_SKEW4_CNT);
	else if (start_bit > (TOTAL_PLL_SKEW4_CNT - 1))
		startbit = (u16)(TOTAL_PLL_SKEW4_CNT - start_bit);
	else
		startbit = (u16)start_bit;

	return startbit;
}

static u32 mtk_fcie_m6e3_skew4_right_result(struct mtk_fcie_host *mtk_fcie_host,
	u8 *data_buf,
	u16 start_bit,
	u16 *next_start_bit,
	u16 *skew4_cnt,
	...
	u32 *scanned_skew4)
{
	...
	for (i = start_bit; i < TOTAL_PLL_SKEW4_CNT; i++) {
		err = mtk_fcie_m6e3_probe_timing(mtk_fcie_host,
		data_buf,
		i);
		cur_start_bit = i;
		(*scanned_skew4) |= (1<<i);
		if (!err) {
			(*skew4_cnt) += 1;
			(*skew4_result) |= (1<<i);
			if ((*skew4_cnt) >= M6E3_MIN_OK_SKEW_CNT)
				goto LABEL_END;
		} else {
			if (err == EMMC_ST_ERR_CMD17_RESP_CRC ||
				err == EMMC_ST_ERR_CMD17_DATA_CRC ||
				err == EMMC_ST_ERR_CMD6_RESP_CRC) {
				ret = _mtk_fcie_reset(mtk_fcie_host);
				if (ret)
...
LABEL_END:
	*next_start_bit = mtk_fcie_m6e3_skew4_start_bit(cur_start_bit + 1);

	return ret;
}

static u32 mtk_fcie_m6e3_skew4_left_result(struct mtk_fcie_host *mtk_fcie_host,
	u8 *data_buf,
	u16 start_bit,
	u16 *next_start_bit,
	u16 *skew4_cnt,
	...
	u32 *scanned_skew4)
{
	...
	for (i = (TOTAL_PLL_SKEW4_CNT-1); i > start_bit; i--) {
		err = mtk_fcie_m6e3_probe_timing(mtk_fcie_host,  data_buf, i);
		cur_start_bit = i;
		(*scanned_skew4) |= (1 << i);
		if (!err) {
			(*skew4_cnt) += 1;
...
static u32 mtk_fcie_m6e3_chk_skew4(u32 *scanned_skew4)
{
	u32 err = 0;
	u16 skew4_cnt = 0;
	int i;
	...
	for (i = 0; i < TOTAL_PLL_SKEW4_CNT; i++) {
		if (((*scanned_skew4) & (1 << i)) == 0)
			skew4_cnt++;
	}
    //The remainder of unscanned pahse are lee than M6E3_MIN_OK_SKEW_CNT
	if (skew4_cnt < M6E3_MIN_OK_SKEW_CNT)
		err = eMMC_ST_ERR_SKEW4_NOT_ENOUGH;

	return err;
}


static u32 mtk_fcie_m6e3_scan_skew4(struct mtk_fcie_host *mtk_fcie_host)
{
	u32 skew4_result = 0, err = 0, scanned_skew4 = 0;
	u16 skew4_cnt = 0, next_right = 0, next_left = 0;
	...
	data_buf = kzalloc(EMMC_SECTOR_512BYTE, GFP_NOIO);
	if (!data_buf)
		return -ENOMEM;

	mtk_fde_func_dis(mtk_fcie_host);
	_mtk_fcie_reset(mtk_fcie_host);
	right_start = mtk_fcie_m6e3_skew4_start_bit(emmc_drv->cur_skew4 + PLL_SKEW4_SHIFT);
	left_start = mtk_fcie_m6e3_skew4_start_bit(emmc_drv->cur_skew4 - PLL_SKEW4_SHIFT);
	scanned_skew4 |= (u32)(1 << emmc_drv->cur_skew4);

	while (true) {
		err = mtk_fcie_m6e3_chk_skew4(&scanned_skew4);
		if (err)
			break;
		err = mtk_fcie_m6e3_skew4_right_result(mtk_fcie_host,
			data_buf,
			(u16)right_start,
			&next_right,
			&skew4_cnt,
		...
		err = mtk_fcie_m6e3_skew4_left_result(mtk_fcie_host,
			data_buf,
			(u16)left_start,
			&next_left,
			&skew4_cnt,
	...
	kfree(data_buf);
	if (skew4_result == 0 || skew4_cnt < M6E3_MIN_OK_SKEW_CNT) {
		EMMC_DBG(mtk_fcie_host, EMMC_DBG_LVL_ERROR, 1,
			"eMMC Warn: SkewResult: %Xh SkewCnt: %u\n",
			skew4_result,
			skew4_cnt);
		return EMMC_ST_ERR_SKEW4;
	}
	...
	if (emmc_drv->pad_type == FCIE_EMMC_HS400) {
		emmc_drv->t_skew4_tmp_t.skew4_result_data = skew4_result;
		err = mtk_fcie_detect_timing_skew4_ex(mtk_fcie_host,
			skew4_result,
			&emmc_drv->t_skew4_tmp_t.skew4_data,
			&emmc_drv->t_skew4_tmp_t.reg2ch_data);
	} else {
		emmc_drv->t_set_tmp_t.scan_result = skew4_result;
		err = mtk_fcie_detect_timing_skew4_ex(mtk_fcie_host,
			skew4_result,
			&emmc_drv->t_set_tmp_t.skew4,
			&emmc_drv->t_set_tmp_t.reg2ch);
	}
...
static void mtk_fcie_m6e3_execute_tuning(struct mtk_fcie_host *mtk_fcie_host)
{
	u32 err = 0, arg = 0;
	struct fcie_emmc_driver *emmc_drv = NULL;
	struct fcie_reg_set *regset = NULL;
	u8 switch_to_original_area = 0;
	...
	if (mtk_fcie_host->emmc_drv->partition_config &
		...
		switch_to_original_area = 1;
	}
	regset = mtk_fcie_host->emmc_drv->t_table_g.regset;
	EMMC_DBG(mtk_fcie_host, EMMC_DBG_LVL_ERROR, 0,
		"eMMC Warn:scan M6E3 skew4, please wait...\n");
	// ---------------------------
	// search and set the Windows
	err = mtk_fcie_m6e3_scan_skew4(mtk_fcie_host);
	...
	if (!err) {
		if (emmc_drv->pad_type == FCIE_EMMC_HS400) {
			EMMC_DBG(mtk_fcie_host, EMMC_DBG_LVL_ERROR, 0,
				"eMMC: original HS400 skew4 result:%Xh, new skew4 result:%Xh\n",
				emmc_drv->t_hs400_table.set_skew4.skew4_result_data,
				emmc_drv->t_skew4_tmp_t.skew4_result_data);

			emmc_drv->t_hs400_table.set_skew4.reg2ch_data =
				emmc_drv->t_skew4_tmp_t.reg2ch_data;
			emmc_drv->t_hs400_table.set_skew4.skew4_data =
				emmc_drv->t_skew4_tmp_t.skew4_data;
			emmc_drv->t_hs400_table.set_skew4.skew4_result_data =
				emmc_drv->t_skew4_tmp_t.skew4_result_data;

			if (emmc_drv->t_hs400_table.set_skew4.reg2ch_data)
				emmc_drv->cur_skew4 =
					emmc_drv->t_skew4_tmp_t.skew4_data + PLL_SKEW4_CNT;
			else
				emmc_drv->cur_skew4 = emmc_drv->t_skew4_tmp_t.skew4_data;
		} else {
			EMMC_DBG(mtk_fcie_host, EMMC_DBG_LVL_ERROR, 0,
				"eMMC: original HS400-ES skew4 result:%Xh,new skew4 result:%Xh\n",
				emmc_drv->t_table.set[EMMC_TIMING_SET_MAX].scan_result,
				emmc_drv->t_set_tmp_t.scan_result);

			emmc_drv->t_table.set[EMMC_TIMING_SET_MAX].reg2ch =
				emmc_drv->t_set_tmp_t.reg2ch;
			emmc_drv->t_table.set[EMMC_TIMING_SET_MAX].skew4 =
				emmc_drv->t_set_tmp_t.skew4;
			emmc_drv->t_table.set[EMMC_TIMING_SET_MAX].scan_result =
				emmc_drv->t_set_tmp_t.scan_result;

			if (emmc_drv->t_set_tmp_t.reg2ch) {
				regset[GT_OFFSET_SKEW4_INV_BIT].opcode = REG_OP_SETBIT;
				emmc_drv->cur_skew4 = emmc_drv->t_set_tmp_t.skew4 + PLL_SKEW4_CNT;
			} else {
				regset[GT_OFFSET_SKEW4_INV_BIT].opcode = REG_OP_CLRBIT;
				emmc_drv->cur_skew4 = emmc_drv->t_set_tmp_t.skew4;
			}
			regset[GT_OFFSET_SKEW4_VALUE].opcode = REG_OP_SETBIT;
			regset[GT_OFFSET_SKEW4_VALUE].reg_value =
				emmc_drv->t_set_tmp_t.skew4 << BIT_SKEW4_SHIFT;
			mtk_fcie_apply_timing_set(emmc_drv->t_table_g.curset_idx, mtk_fcie_host);
		}
		EMMC_DBG(mtk_fcie_host, EMMC_DBG_LVL, 0, "\n");
	} else {
		EMMC_DBG(mtk_fcie_host, EMMC_DBG_LVL_ERROR, 0,
			"eMMC Err: scan M6E3 skew4 fail\n");
	}

	if (switch_to_original_area) {
		arg = ((EMMC_EXTCSD_WBYTE&EMMC_EXTCSD_ACCESS_MASK) << EMMC_EXTCSD_ACCESS_SHIFT) |
			(EXT_CSD_PART_CONFIG<<EMMC_EXTCSD_IDX_SHIFT) |
			(mtk_fcie_host->emmc_drv->partition_config << EMMC_EXTCSD_VALUE_SHIFT);
...
static ssize_t emmc_cmd_log_enable_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct mmc_host *mmc;
	struct mtk_fcie_host *mtk_fcie_host;
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n",
		mtk_fcie_host->emmc_cmd_log_enable);
}

static ssize_t emmc_cmd_log_enable_store(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count)
{
	unsigned long temp = 0;
	...
	mtk_fcie_host->emmc_cmd_log_enable = temp;
...
static DEVICE_ATTR_RW(emmc_cmd_log_enable);
	...
	struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "Debug Help:\n"
		...
		"- emmc_cmd_log_enable <RW>: To control flag for command log.\n"
...
static struct attribute *mtk_fcie_attr[] = {
	...
	&dev_attr_emmc_cmd_log_enable.attr,
...
static int mtk_fcie_probe(struct platform_device *pdev)
{
	...
	res = platform_get_resource(pdev, IORESOURCE_MEM, VPLUG_PWR_IORE_MEM_IDX);
	mtk_fcie_host->vplugpwrbase =
		devm_ioremap_resource(&pdev->dev, res);
	if (IS_ERR(mtk_fcie_host->vplugpwrbase)) {
		ret = PTR_ERR(mtk_fcie_host->vplugpwrbase);
		goto fail_end;
	}
	...
	if (!of_property_read_u32(pdev->dev.of_node, "drop-time",
				  &mtk_fcie_host->drop_recovery_time))
		dev_dbg(&pdev->dev, "drop-time: %x\n",
			mtk_fcie_host->drop_recovery_time);
	...
	if (of_property_read_bool(pdev->dev.of_node, "drop_recovery"))
		mtk_fcie_host->enable_drop_recovery = 1;
	else
		mtk_fcie_host->enable_drop_recovery = 0;

	if (of_property_read_bool(pdev->dev.of_node, "emmc_skew1_inv"))
		mtk_fcie_host->emmc_pll_skew1_inv = 1;
	else
		mtk_fcie_host->emmc_pll_skew1_inv = 0;

	if (of_get_property(pdev->dev.of_node, "gpio-ctl-emmc-vccq", NULL)) {
		if (!of_property_read_u32(pdev->dev.of_node, "gpio-ctl-emmc-vccq",
			&mtk_fcie_host->gpio_ctl_emmc_vccq))
			dev_dbg(&pdev->dev, "gpio-ctl-emmc-vccq: %u\n",
				mtk_fcie_host->gpio_ctl_emmc_vccq);

		if (mtk_fcie_host->gpio_ctl_emmc_vccq)
			mtk_fcie_host->emmc_sleep = 1;
		else
			mtk_fcie_host->emmc_sleep = 0;
	} else {
		if (of_property_read_bool(pdev->dev.of_node, "emmc_sleep"))
			mtk_fcie_host->emmc_sleep = 1;
		else
			mtk_fcie_host->emmc_sleep = 0;
	}
	...
	mmc_host->caps2 |= MMC_CAP2_NO_SD|MMC_CAP2_NO_SDIO;
	if (!mtk_fcie_host->emmc_sleep)
		mmc_host->caps2 |= MMC_CAP2_FULL_PWR_CYCLE;
...
static int mtk_fcie_suspend(struct device *dev)
{
	...
	if (mmc_host) {
		...
		REG_CLRBIT(
			REG_ADDR(mtk_fcie_host->fciebase, PWR_SAVE_CTL),
			BIT_POWER_SAVE_MODE|BIT_POWER_SAVE_MODE_INT_EN);
...
static int mtk_fcie_resume(struct device *dev)
{
	...
	REG_SETBIT(REG_ADDR(mtk_fcie_host->fciebase, BOOT_CONFIG),
		BIT_EMMC_RSTZ_EN|BIT_EMMC_RSTZ);
...
static int mtkcqe_error_flags(u16 status)
{
	...
	if (status & (BIT_SD_R_CRC_ERR|BIT_DAT_WR_CERR|BIT_CMD_RSP_CERR))
