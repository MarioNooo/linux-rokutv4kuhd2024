...
#define BIT_CMD_DBUS_MODE		    1
#define BIT_DATA_DBUS_MODE		    2
...
#define VPLUG_PWR_IORE_MEM_IDX  9
//------------------------------------------------------------------
/*
 * Power Save FIFO Cmd*
 */
...
#define BIT_SKEW1_FOR_INV               (BIT(2)|BIT(1)|BIT(0))
...
/* emmcpll 0x45 */
#define reg_emmcpll_0x45               0x45
#define BIT_HOST_DRIVING_MASK          0x0fff
...
#define BIT_SKEW1_CLK_INV               BIT(3)
...
#define BIT_SKEW4_REG2C_INV            BIT(7)

//===========================================================
// Vplug power registser
//==========================================================
/* vplugpwr 0x62 */
#define  reg_vplug_pwr_0x62            0x62
#define  BIT_VPLUG_IN_PWRGD            BIT(14)
...
#define EMMC_ST_ERR_CMD6_RESP_CRC		(0x0a2d | EMMC_ST_PLAT)
...
#define EMMC_CLK_200M                   200000000
#define EMMC_CLK_48M                    48000000
...
#define EMMC_CIS_BLK_END			eMMC_HS400SKEW4TABLE_BLK_1
...
#define ECSD_HS200_HS400_MASK (BIT(7)|BIT(6)|BIT(5)|BIT(4))
// ECSD[185]
#define SPEED_OLD            0
#define SPEED_HIGH           1
...
struct fcie_atop_set_skew4 {
	u32 skew4_result_cmd;
	u32 skew4_result_data;
	u16 clk;
	u8 skew4_cmd, skew4_data;
	u8 skew2;
	u8 reg2ch_cmd, reg2ch_data;
} EMMC_PACK1;
...
struct fcie_hs400_timing_table {
	struct fcie_atop_set_skew4 set_skew4;
	u16 cmd_ok_cnt;
	u8 setcnt;
	u8 cid[EMMC_MAX_RSP_BYTE_CNT];
	u32 device_driving;
...
struct fcie_emmc_driver {
	...
	struct fcie_hs400_timing_table t_hs400_table;
	int cur_skew4;
	...
	u32 clk_khz;
	u16 clk_reg_val;
	struct fcie_atop_set t_set_tmp_t;
	struct fcie_atop_set_skew4 t_skew4_tmp_t;
	// ----------------------------------------
	// EMMC
	// ----------------------------------------
	u8	if_sector_mode;
...
struct mtk_fcie_host {
	...
	u32	clk_2xp;
	u32	v_emmc_pll;
	u32	drop_recovery_time;
	u32	gpio_ctl_emmc_vccq;
	u64	miu0_base;
	u8 enable_ssc;
	u8 enable_sar5;
	u8 enable_fde;
	...
	u8 emmc_pll_skew1_inv;
	u8 enable_drop_recovery;
	u8 emmc_sleep;
	//attribute
	u32	emmc_read_log_enable;
	u32	emmc_write_log_enable;
	u32	emmc_cmd_log_enable;
...
#define REQ_VPLUG_DROP          (0x1 << 9)
#define REQ_INVALID_CMD         (0x1 << 10)
#define REQ_ERR_RETRY_MASK      (REQ_CMD_EIO|REQ_CMD_NORSP|REQ_DAT_EIO|REQ_DAT_EILSEQ)
#define REQ_WITHOUT_WORK_QUEUE  (REQ_CMD_BUSY|REQ_INVALID_CMD)

//CMD6 argument
#define CMD6_INDEX_MASK 0xff0000
#define CMD6_INDEX_SHIFT 16

#define CMD6_VALUE_MASK 0xff00
#define CMD6_VALUE_SHIFT 8
