...
#define GPIO_VALUE_MASK			0xffff
#define GPIO_VALUE_BIT			16
#define GET_FIQ_ADDRESS(i)		(((i % 4) * 4) + ((i / 4) * 0x80))
#define GET_IRQ_ADDRESS(i)		(((i % 4) * 4) + ((i / 4) * 0x80) + 0x40)
...
static void _mtk_dtv_gpio_irq_status_reset(struct mtk_dtv_gpio *pdata, bool is_fiq)
{
	int bit, cnt, i;
	u32 reg_offset;
	ulong val_status;
	...
	cnt = (MTK_GPIO_MAX_NUMBER / GPIO_VALUE_BIT);
	/* unmask all */
	for (i = 0; i < cnt; i++) {
		reg_offset = MTK_GPIO_IRQC_MASK_OFFSET;

		if (is_fiq == true)
			reg_offset += GET_FIQ_ADDRESS(i);
		else
			reg_offset += GET_IRQ_ADDRESS(i);

		val = GPIO_VALUE_MASK;
		writew_relaxed(val, (pdata->reg_base_irq + reg_offset));
	}
	...
	for (i = 0; i < cnt; i++) {
		...
		if (val_status) {
			for_each_set_bit(bit, &val_status, GPIO_VALUE_BIT) {
				mask = OFFSET_TO_BIT(bit);

				/* clear status */
				writew_relaxed(mask, pdata->reg_base_irq + reg_offset);
			}
		}
	}
}
...
#ifdef CONFIG_PM
void mtk_dtv_gpio_shutdown(struct platform_device *pdev)
{
	struct mtk_dtv_gpio *pdata = platform_get_drvdata(pdev);

	if (!pdata)
		return;

	/* clear FIQ ISR */
	_mtk_dtv_gpio_irq_status_reset(pdata, true);

	/* clear IRQ ISR */
	_mtk_dtv_gpio_irq_status_reset(pdata, false);
}
#endif
...
#ifdef CONFIG_PM
	.shutdown = mtk_dtv_gpio_shutdown,
#endif
};

#ifdef CONFIG_ROKU_KPI_ADJUST_INIT_FLOW
arch_platform_driver(mtk_dtv_gpio_driver);
#else
