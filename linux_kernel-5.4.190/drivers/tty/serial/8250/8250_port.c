...
static int uart_cfg_tx_enabled;

static int __init rokutxenable(char *s)
{
	int val;
	if (get_option(&s, &val) == 1) {
		uart_cfg_tx_enabled = val;
		return 0;
	}
	return -EINVAL;
}
early_param("rokutxenable", rokutxenable);
#endif
...
{
#ifdef CONFIG_ROKU_SERIAL_8250
		up->uart_in_dl_write = 1;
#endif
	serial_out(up, UART_DLL, value & 0xff);
	serial_out(up, UART_DLM, value >> 8 & 0xff);
#ifdef CONFIG_ROKU_SERIAL_8250
		up->uart_in_dl_write = 0;
#endif
}
	...
	if (options)
        ...
        if (baud == 0) {
               struct uart_8250_port *up = up_to_u8250p(port);
                ...
                pr_notice("baud == 0: no input allowed\n");
                up->uart_noinput = 1;
                if (uart_cfg_tx_enabled)
                        up->uart_nooutput = 0;
                else
                        up->uart_nooutput = 1;
                baud = 115200;
