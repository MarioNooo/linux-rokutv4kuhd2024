...
#define MT5897_DEFAULT_T_SENSOR_REF            (740)
#define MT5897_DEFAULT_T_SENSOR_SHIFT          (27000)
#define MT5897_VBE_CODE_MULTIPLIER     (753)
#define MT5897_VBE_CODE_FT_MULTIPLIER     (740)
#define MT5876_DEFAULT_T_SENSOR_REF         (400)
#define MT5876_DEFAULT_T_SENSOR_SHIFT       (27500)
#define MT5876_VBE_CODE_MULTIPLIER     (1250)
#define MT5876_VBE_CODE_FT_MULTIPLIER     (1270)
#define MT5879_DEFAULT_T_SENSOR_REF         (400)
#define MT5879_DEFAULT_T_SENSOR_SHIFT       (27500)
#define MT5879_VBE_CODE_MULTIPLIER        (1250)
#define MT5879_VBE_CODE_FT_MULTIPLIER     (1270)
#define MT5873_DEFAULT_T_SENSOR_REF         (400)
#define MT5873_DEFAULT_T_SENSOR_SHIFT       (27500)
#define MT5873_VBE_CODE_MULTIPLIER     (1250)
#define MT5873_VBE_CODE_FT_MULTIPLIER     (1250)
...
#define MT5896_REG_TA_CODE_FT_SHIT            (17)
#define MT5876_REG_TA_CODE_FT_SHIT        (16)
...
void get_VBE_CODE(void)
{
	...
	if ((chip_id == MT5896_ID) || (chip_id == MT5897_ID))
		tsensor_shift = (u32VBETemp >> MT5896_REG_TA_CODE_FT_SHIT) & REG_TA_CODE_FT_MASK;
	else
		tsensor_shift = (u32VBETemp >> MT5876_REG_TA_CODE_FT_SHIT) & REG_TA_CODE_FT_MASK;
...
int get_cpu_temperature_m6l(void)
{
	 ...
	 * 2022/09/16 NEW Equation
	 * temp = 0.74*(VBE_CODE_FT - VBE_CODE )+(TA_CODE/10+0)
	 */
	...
	if (tsensor_shift == 0)
		tsensor_shift = MT5897_DEFAULT_T_SENSOR_SHIFT;

	if (tsensor_ref == 0)
		tsensor_ref = MT5897_DEFAULT_T_SENSOR_REF;
	// read VBE_CODE
	iio_read_channel_processed(SAR_Thermal, &temperature);

	if (tsensor_ref == MT5897_DEFAULT_T_SENSOR_REF) {
		temperature = MT5897_DEFAULT_T_SENSOR_SHIFT +
		((MT5897_DEFAULT_T_SENSOR_REF - temperature) * MT5897_VBE_CODE_MULTIPLIER);
	} else {
		temperature = tsensor_shift + ((tsensor_ref - temperature)
			* MT5897_VBE_CODE_FT_MULTIPLIER);
	}
	temperature /= TEMP_DIV_1000;
	return temperature;
}
...
int get_cpu_temperature_mokona(void)
{
	...
	if (tsensor_shift == 0)
		tsensor_shift = MT5876_DEFAULT_T_SENSOR_SHIFT;

	if (tsensor_ref == 0)
		tsensor_ref = MT5876_DEFAULT_T_SENSOR_REF;

	iio_read_channel_processed(SAR_Thermal, &temperature);

	if (tsensor_ref == MT5876_DEFAULT_T_SENSOR_REF) {
		temperature = MT5876_VBE_CODE_MULTIPLIER * (tsensor_ref - temperature)
			+ tsensor_shift;
	} else {
		temperature = MT5876_VBE_CODE_FT_MULTIPLIER * (tsensor_ref - temperature)
...
int get_cpu_temperature_miffy(void)
{
	...
	if (tsensor_shift == 0)
		tsensor_shift = MT5879_DEFAULT_T_SENSOR_SHIFT;

	if (tsensor_ref == 0)
		tsensor_ref = MT5879_DEFAULT_T_SENSOR_REF;

	iio_read_channel_processed(SAR_Thermal, &temperature);

	if (tsensor_ref == MT5879_DEFAULT_T_SENSOR_REF) {
		temperature = MT5879_VBE_CODE_MULTIPLIER * (tsensor_ref - temperature)
			+ tsensor_shift;
	} else {
		temperature = MT5879_VBE_CODE_MULTIPLIER * (tsensor_ref - temperature)
...
int get_cpu_temperature_moka(void)
{
	...
	if (tsensor_shift == 0)
		tsensor_shift = MT5873_DEFAULT_T_SENSOR_SHIFT;

	if (tsensor_ref == 0)
		tsensor_ref = MT5873_DEFAULT_T_SENSOR_REF;

	iio_read_channel_processed(SAR_Thermal, &temperature);

	if (tsensor_ref == MT5873_DEFAULT_T_SENSOR_REF) {
		temperature = MT5873_VBE_CODE_MULTIPLIER * (tsensor_ref - temperature)
			+ tsensor_shift;
	} else {
		temperature = MT5873_VBE_CODE_FT_MULTIPLIER * (tsensor_ref - temperature)
			+ (tsensor_shift + NUM_300);
	}
	temperature /= TEMP_DIV_1000;
	return temperature;
}
