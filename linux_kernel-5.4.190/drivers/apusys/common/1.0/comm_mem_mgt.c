...
int comm_mem_alloc(struct comm_kmem *mem)
{
	...
	if (!ret) {
		...
		if (mr != NULL) {
			...
			mr->kmem.property = APU_COMM_MEM_PROP_ALLOC;
...
int comm_mem_free(struct comm_kmem *mem, u32 close)
{
	int ret = -EFAULT;
	struct mem_record *mr = NULL;
	struct list_head *tmp = NULL, *list_ptr = NULL;
	...
	case APU_COMM_MEM_DRAM_DMA:
		...
		list_for_each_safe(list_ptr, tmp, &g_mem_mgr.list) {
			...
			if (mr->kmem.iova == mem->iova &&
				...
				ret = plat_mem_free(&g_mem_mgr, &mr->kmem, close);
	...
	if (ret)
		comm_err("[error] %s: iova:%llx, fd:%d, close:%u\n",
				__func__, mem->iova, mem->fd, close);

	return ret;
}

int comm_mem_cache_ctrl(struct comm_kmem *mem, u32 cache_op)
{
	int ret = 0;
	struct mem_record *mr = NULL;
	struct list_head *tmp = NULL, *list_ptr = NULL;
	...
	case APU_COMM_MEM_DRAM_DMA:
		...
		list_for_each_safe(list_ptr, tmp, &g_mem_mgr.list) {
			...
			if (mr->kmem.fd == mem->fd &&
				mr->kmem.tgid == task_tgid_nr(current) &&
				mem->size <= mr->kmem.size && mr->kmem.cache) {
				ret = plat_mem_cache_ctrl(&g_mem_mgr, &mr->kmem, cache_op);
				comm_print_kmem("flush/invalidate", &mr->kmem);
				break;
			}
		}
		mutex_unlock(&g_mem_mgr.list_mtx);
...
int comm_mem_map_iova(struct comm_kmem *mem)
{
	...
	if (ret) {
		ret = plat_mem_map_iova(&g_mem_mgr, mem);
		if (ret) {
			comm_err("fail, fd:%d tgid:%x\n",
				mem->fd, task_tgid_nr(current));
		}
	}
	mutex_unlock(&g_mem_mgr.list_mtx);
...
int comm_mem_map_kva(struct comm_kmem *mem)
{
	...
	if (ret) {
		ret = plat_mem_map_kva(&g_mem_mgr, mem);
		if (ret) {
			comm_err("fail, fd:%d tgid:%x\n",
				mem->fd, task_tgid_nr(current));
		}
