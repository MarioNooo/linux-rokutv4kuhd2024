...
//#ifdef CONFIG_APUSYS_PM_WAKELOCKS
...
#define MDW_CMD_DEFAULT_TIMEOUT (6*10000) //60s
...
{
#if defined CONFIG_APUSYS_PM_WAKELOCKS
	char ws_name[16];
	...
	ws_cnt = 0;
	mutex_init(&ws_mtx);
	//mdw_usr_ws = wakeup_source_register((const char *)ws_name);
...
static struct mdw_mem *mdw_usr_mem_create(struct apusys_mem *um, struct mdw_usr *u, int mem_op)
{
	int ret = 0;
	struct mdw_mem *mm = NULL;
	...
	/* alloc mem */
	mdw_mem_u2k(um, (unsigned long long)u, &mm->kmem);
	switch (mem_op) {
...
int mdw_usr_mem_alloc(struct apusys_mem *um, struct mdw_usr *u)
{
	...
	mm = mdw_usr_mem_create(um, u, APUSYS_MEM_PROP_ALLOC);
	if (!mm)
		return -ENOMEM;
...
int mdw_usr_mem_import(struct apusys_mem *um, struct mdw_usr *u)
{
	...
	mm = mdw_usr_mem_create(um, u, APUSYS_MEM_PROP_IMPORT);
	if (!mm)
		return -ENOMEM;
...
int mdw_usr_mem_map(struct apusys_mem *um, struct mdw_usr *u)
{
	...
	mm = mdw_usr_mem_create(um, u, APUSYS_MEM_PROP_MAP);
	if (!mm)
		return -ENOMEM;
...
int mdw_usr_mem_flush(struct apusys_mem *um, struct mdw_usr *u)
{
	int ret = 0;
	struct mdw_mem *mm = NULL;
	...
	mdw_mem_u2k(um, (uint64_t)u, &mm->kmem);

	mdw_drv_debug("mm->kmem.fd = %x\n", mm->kmem.fd);
	mdw_drv_debug("mm->kmem.size = %x\n", mm->kmem.size);

	ret = apusys_mem_flush(&mm->kmem);
...
int mdw_usr_mem_invalidate(struct apusys_mem *um, struct mdw_usr *u)
{
	int ret = 0;
	struct mdw_mem *mm = NULL;
	...
	ret = apusys_mem_invalidate(&mm->kmem);
...
int mdw_usr_run_cmd_async(struct mdw_usr *u, struct apusys_ioctl_cmd *in)
{
	...
	mdw_mem_debug("run_cmd_async in->mem_fd =%x\n", in->mem_fd);
...
void mdw_usr_destroy(struct kref *kref)
{
	...
	list_for_each_safe(list_ptr, tmp, &u->mem_list) {
		...
		//add release_flag for tv
		mm->kmem.release_flag = 1;
