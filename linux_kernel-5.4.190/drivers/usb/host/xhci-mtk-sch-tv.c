...
void drop_ep_all_quirk(struct xhci_hcd_mtk *mtk)
{
	struct xhci_hcd *xhci = hcd_to_xhci(mtk->hcd);
	struct mu3h_sch_ep_info *sch_ep;
	...
	hash_for_each_safe(mtk->sch_ep_hash, bkt, hn,
				    sch_ep, hentry) {

		sch_bw = sch_ep->bw_info;

		ep = sch_ep->ep;
		if (!ep) {
			xhci_err(xhci, "%s() null ep!!\n", __func__);
			continue;
		}
		udev = (struct usb_device *)ep->hcpriv;
		if (!udev) {
			xhci_err(xhci, "%s() null udev!!\n", __func__);
			continue;
		}
		destroy_sch_ep(mtk, udev, sch_ep);
	}
