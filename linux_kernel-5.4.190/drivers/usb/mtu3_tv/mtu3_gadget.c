		...
		u32 epnum, u32 is_in)
{
	...
	result = sprintf(mep->name, "ep%d%s", epnum,
		!epnum ? "" : (is_in ? "in" : "out"));

	if (result < 0)
		dev_dbg(mtu->dev, "%s,sprintf failed\n", __func__);
...
int mtu3_gadget_setup(struct mtu3 *mtu)
{
	...
	mtu->g.irq = mtu->irq;//to tell class driver our irq number
	mtu->is_active = 0;
	mtu->delayed_status = false;
...
{
#ifdef ADB_DEBUG
	dev_crit(mtu->dev, "gadget RESUME\n");
#else
...
{
#ifdef ADB_DEBUG
	dev_crit(mtu->dev, "gadget SUSPEND\n");
#else
...
{
#ifdef ADB_DEBUG
		dev_crit(mtu->dev, "gadget DISCONNECT\n");
#else
...
void mtu3_gadget_disconnect_normal(struct mtu3 *mtu)
{
	dev_dbg(mtu->dev, "mtu3 gadget DISCONNECT norm\n");

	mtu3_dev_on_off(mtu, 0);	//turn off softconn
