...
static int crypt_ctr(struct dm_target *ti, unsigned int argc, char **argv)
{
	...
	if(strstr(argv[1], "roku:") != NULL) {
		...
		if (!dt_node) {
			ti->error = "dt node not found";
			return -EINVAL;
		}
		prop = of_get_property(dt_node, strstr(argv[1], ":") + 1, &len);
		if (!prop) {
			DMWARN("dt property %s not found\n", strstr(argv[1], ":") + 1);
			return -EINVAL;
		}
		if(len > 32) {
			ti->error = "invalid len";
			return -EINVAL;
		}
		for(i = 0; i < len; i++) {
			sprintf(key_roku + (i * 2), "%02x", prop[i]);
		}
		key_ptr = key_roku;

		// Remove the key from the device tree
		st_prop = of_find_property(dt_node, strstr(argv[1], ":") + 1, &len);
		if (!st_prop) {
			DMWARN("dt property %s not found, unable to remove\n", strstr(argv[1], ":") + 1);
		}
		else {
			if (of_remove_property(dt_node, st_prop))
				DMWARN("failed to remove %s", strstr(argv[1], ":") + 1);
			else
				DMWARN("removed %s", strstr(argv[1], ":") + 1);
		}
	}

	key_size = strlen(key_ptr) >> 1;
...
#else
	ret = crypt_ctr_cipher(ti, argv[0], key_ptr);
#endif
	if (ret < 0)
		goto bad;
