...
#define SCHEDUTIL_GOVERNOR_NAME  "schedutil"
struct sugov_policy {
...
#define MT5897_CLK_DSU(CORE) (CORE*668/1000)   // DSU = CORE x 0.668
...
#define MT5897_CPUFREQ_MAX                 (2000000)
#define MT5897_CPUFREQ_MAX_HZ              (2000000000)
...
/* chip ID */
#define MT5896_ID  (0x101)
#define MT5897_ID  (0x109)
#define MT5876_ID  (0x10C)
#define MT5879_ID  (0x10D)
#define MT5873_ID  (0x111)
#define UNKNOWN_ID  (0)
		...
		unsigned int u32Target)
{
	...
	case MT5897_ID:
		if (u32Target >  u32Current) {
			if (u32Target == MT5897_CPUFREQ_MAX)
				MTK_CLK_DSU_Adjust(MT5897_CLK_DSU(u32Current),
						CLK_DSU(u32Target));
			else
				MTK_CLK_DSU_Adjust(MT5897_CLK_DSU(u32Current),
						MT5897_CLK_DSU(u32Target));
			MTK_CLK_CORE_Adjust(u32Current, u32Target);
		} else {
			MTK_CLK_CORE_Adjust(u32Current, u32Target);
			if (u32Current == MT5897_CPUFREQ_MAX)
				MTK_CLK_DSU_Adjust(CLK_DSU(u32Current),
						MT5897_CLK_DSU(u32Target));
			else
				MTK_CLK_DSU_Adjust(MT5897_CLK_DSU(u32Current),
						MT5897_CLK_DSU(u32Target));
...
static void mtktv_boost_init(void)
{
	...
	for (index = 0; index < E_MTKTV_BOOST_NONE; index++) {
		...
		ret = snprintf(node_name, DEVICE_NAME, "boost-%d", index);
		if (ret > DEVICE_NAME) {
			pr_err("node_name is over length\n");
			return;
		}
		of_property_read_u32(node, node_name, &value);
		available_boost_list[index].rate = value/1000;
		CPUFREQ_INFO("[available_boost_list]\n"
				"index = %d, clk = %d, %s\n",
				index,
...
static void mtktv_boost_enable(unsigned int enable, s32 rate)
{
	...
	if (!policy) {
		pr_err("cpu policy is not initialized.\n");
		return;
	}
	...
	if (enable == 0) {
		...
		if ((rate == 0 && current_boost.id < E_MTKTV_BOOST_NONE)) {
			if (chip_id == MT5879_ID && current_boost.id == E_MTKTV_IR_BOOST) {
...
int mtktv_scaling_to_boost_freq(unsigned int enable)
{
	...
	freq_qos_update_request(policy->min_freq_req, target_min);
	freq_qos_update_request(policy->max_freq_req, target_max);
	CPUFREQ_DEBUG("scaling freq min = %d, max = %d\n",
			target_min, target_max);
	...
	if (input_boost_enable == 1) {
		for (i = 0; i < count; i++) {
			if (vals[i].type == EV_KEY) {
				CPUFREQ_INFO("dev %s send EV_KEY (state=%c)\n",
						handle->dev->name,
		...
		const struct input_device_id *id)
{
	...
	CPUFREQ_INFO("device %s connected\n", handle->dev->name);
...
static void input_disconnect(struct input_handle *handle)
{
	...
	input_unregister_handle(handle);
	CPUFREQ_INFO("device %s disconnected\n", handle->dev->name);
	kfree(handle);
}
...
static void input_boost_init(void)
{
	...
	case MT5879_ID:
		...
		if (err)
			pr_err("register input boost handler fail! err=%d\n", err);
...
static int mtktv_cpufreq_opp_init(void)
{
	...
	for (i = 0; i < opp_cnt; i++) {
		...
		rcu_read_unlock();
		CPUFREQ_INFO("clock = %ld, voltage = %d\n",
				freq_hz, vproc);
	}
	kfree(freq_table);
	return 0;
...
static void mtktv_cpufreq_ready(struct cpufreq_policy *policy)
{
...
#endif
#if IS_ENABLED(CONFIG_CPU_FREQ_GOV_SCHEDUTIL)
	struct cpumask sugov_task_cmask;
	struct task_struct	*sugov_task;
	...
	if (thermal_done == 0) {
		CPUFREQ_INFO("register cooling device\n");
		...
		if (IS_ERR(cdev))
			...
			CPUFREQ_INFO("successful to register cooling device\n");
		thermal_done = 1;
		policy->cdev = cdev;
...
#endif
#if IS_ENABLED(CONFIG_CPU_FREQ_GOV_SCHEDUTIL)
	// setup task to core 0 only
	if (!strncmp(policy->governor->name, SCHEDUTIL_GOVERNOR_NAME,
		strlen(SCHEDUTIL_GOVERNOR_NAME))) {
		if (policy->governor_data) {
			sg_policy = policy->governor_data;
			sugov_task = sg_policy->thread;
			CPUFREQ_INFO("sugov task: %d (%s)\n", task_pid_nr(sugov_task)
				    , sugov_task->comm);
			cpumask_copy(&sugov_task_cmask, cpu_online_mask);
			for_each_online_cpu(i)
				if ((i < CONFIG_NR_CPUS) && (i > 0))
					cpumask_clear_cpu(i, &sugov_task_cmask);
			set_cpus_allowed_ptr(sugov_task, &sugov_task_cmask);
		}
	}
#endif
}
...
static int mtktv_cpufreq_suspend(struct cpufreq_policy *policy)
{
	...
	if (info == NULL) {
		pr_err("dvfs info for cpu%d is not initialized.\n",
				cpu);
		pr_err("update policy fail in suspend.\n");
		return 0;
	}
...
static int mtktv_cpu_dvfs_info_init(struct mtktv_cpu_dvfs_info *info, int cpu)
{
	...
	if (IS_ERR(opp)) {
		pr_err("failed to get intermediate opp\n"
			"for cpu%d\n", cpu);
		ret = PTR_ERR(opp);
		...
		unsigned long event, void *ptr)
{
	...
	{
		mutex_lock(&target_mutex);
		pr_info("PM_SUSPEND_PREPARE\n");
		if (MTKTV_CPUFREQ_TEEC_Suspend() != TA_SUCCESS) {
			pr_err("CPUFREQ TA Suspend fail\n");
			mutex_unlock(&target_mutex);
			return NOTIFY_BAD;
		};
		suspend = 1;
		u32Current = suspend_frequency;
		pr_info("Suspend Clock %d KHz\n", u32Current);
		mutex_unlock(&target_mutex);
		break;
	}
	default:
...
static int mtktv_cpufreq_probe(struct platform_device *pdev)
{
	...
	for_each_possible_cpu(cpu) {
		...
		if (ret) {
			dev_err(&pdev->dev,
			"failed to initialize dvfs info\n"
			"for cpu%d\n", cpu);
			goto release_dvfs_info_list;
		}
	...
	if (ret) {
		dev_err(&pdev->dev, "failed to mtktv_cpufreq_opp_init\n");
		goto release_dvfs_info_list;
	}
	...
	if (IS_ERR(lunch_boost_task)) {
		pr_err("lunch_boost_task creat failed!\n");
		return 0;
	}
...
static int __init mtktv_cpufreq_driver_init(void)
{
	...
	if (!np) {
		pr_err("device tree node\n");
		return -ENODEV;
	}
	match = of_match_node(mtktv_cpufreq_machines, np);
	of_node_put(np);
	if (!match) {
		pr_err("Machine is not support dvfs\n");
		return -ENODEV;
	}
...
static void MTK_CLK_CORE_Adjust(unsigned int cur, unsigned int target)
{
	...
	if ((chip_id == MT5896_ID) || (chip_id == MT5897_ID)) {
		///start_freq
	...
	if ((chip_id == MT5876_ID) || (chip_id == MT5879_ID) || (chip_id == MT5873_ID)) {
		//start_freq
...
static int __init cpufreq_input_boost(char *str)
{
	if (strcmp(str, "1") == 0) {
		pr_info("\033[32m cpufreq input boost enable \033[0m\n");
		input_boost_enable = 1;
	} else {
		input_boost_enable = 0;
	}
	return 0;
}
#endif
...
early_param("CPUFREQ_INPUT_BOOST", cpufreq_input_boost);
#else
...
module_param(input_boost_enable, int, 0644);
MODULE_PARM_DESC(input_boost_enable, "CPUFREQ_INPUT_BOOST");
#endif
