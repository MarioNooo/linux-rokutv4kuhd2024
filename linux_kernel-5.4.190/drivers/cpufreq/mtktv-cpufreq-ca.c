...
static u32 session;
static u32 ta_init = false;
static u32 dvfs_init = false;
/*********************************************************************
 *                         Private Function                          *
 *********************************************************************/
...
static u32 _InitializeContext(void)
{
	...
	pr_info("only support optee version 3\n");
	return TA_ERROR;
}
...
u32 MTKTV_CPUFREQ_TEEC_TA_Open(void)
{
	...
	if (!is_ta_file_open) {
		...
		ta_init == true;
		//pr_debug("Cpufreq TA Open Successfully\n");
...
u32 MTKTV_CPUFREQ_TEEC_Adjust(u32 frequency)
{
	...
	MTKTV_CPUFREQ_TEEC_TA_Open();
	MTKTV_CPUFREQ_TEEC_Init();
...
u32 MTKTV_CPUFREQ_TEEC_Simulate_Corner(u32 corner_type)
{
	ta_result res;
	struct tee_param params;
	params.attr = TEE_IOCTL_PARAM_ATTR_TYPE_VALUE_INPUT;
	params.u.value.a = 0;
	params.u.value.b = 0;
	...
	params.u.value.a = corner_type;

	res = _InvokeCmd(E_DVFS_OPTEE_CORECORNER, params);

	if (res != TA_SUCCESS) {
		pr_err("invoke fail, cmd id: %d\n", E_DVFS_OPTEE_CORECORNER);
		return TA_ERROR;
	}

	pr_info("Simulate Core Corner Info Successfully\n");

	return TA_SUCCESS;
}
EXPORT_SYMBOL(MTKTV_CPUFREQ_TEEC_Simulate_Corner);
