...
#define ENABLE_I2S_RX_MASTER_2STAGE_SETTING 1
...
struct mtk_voice_setting_t {
	uint32_t sample_rate;
	uint8_t mic_numbers;
	uint8_t bit_width;
	uint8_t mic_input_mux;
	bool pad_init;
};
//------------------------------------------------------------------------------
//	Global Variables
//------------------------------------------------------------------------------
static struct mtk_voice_reg_t g_voc_reg;
static struct mtk_voice_setting_t g_voc_setting;
//------------------------------------------------------------------------------
//	Local Variables
//------------------------------------------------------------------------------
...
{
	#define DMIC_PAD_PDM   0x0111
	#define DMIC_PAD_I2S   0x0333
	#define DMIC_PAD_MASK  0x0FFF

	uint16_t value;

	if (g_voc_setting.pad_init)
		return;
	g_voc_setting.pad_init = 1;

	if ((INREG16(g_voc_reg.pm_pmux|OFFSET8(VOC_REG_0X20)) & DMIC_PAD_MASK) != 0)
		return; //already set by pinctrl framework.

	if (g_voc_setting.mic_input_mux >= VOC_MIC_INPUT_I2S_RX_MASTER) {
		//PAD_DMIC_SD2 function 3 for I2S SD1
		INSREG8((g_voc_reg.pm_pmux)|(OFFSET8(VOC_REG_0X33)), VOC_BIT_0XF0, VOC_REG_0X30);
		value = DMIC_PAD_I2S;
	} else {
		value = DMIC_PAD_PDM;
	}
	INSREG16((g_voc_reg.pm_pmux)|(OFFSET8(VOC_REG_0X20)), DMIC_PAD_MASK, value);
}
//==============================================================================
...
{
	return g_voc_setting.mic_numbers;
}
...
int voc_hal_paganini_set_channel(uint8_t u8Channel)
{
	...
	g_voc_setting.mic_numbers = u8Channel;
	if (g_voc_setting.mic_input_mux >= VOC_MIC_INPUT_I2S_RX_MASTER)
		OUTREG8(g_voc_reg.paganini + OFFSET8(VOC_REG_0X28), chn);
...
{
	return g_voc_setting.sample_rate;
}
//==============================================================================
int voc_hal_paganini_set_sample_rate(u32 u32SampleRate, u16 bck_mode, u32 mute_enable)
{
	...
	g_voc_setting.sample_rate = u32SampleRate;
...
{
	return g_voc_setting.bit_width;
}
//==============================================================================
int voc_hal_paganini_set_bit_width(uint8_t u8BitWidth)
{
	...
	g_voc_setting.bit_width = u8BitWidth;

	if (g_voc_setting.mic_input_mux >= VOC_MIC_INPUT_I2S_RX_MASTER)
		INSREG16(g_voc_reg.paganini + OFFSET8(VOC_REG_0X60), VOICE_REG_WR_BIT_MODE, sel);
...
//==============================================================================
void voc_hal_paganini_set_mic_i2s_common(void)
{
	//[ 7: 0] REG_DBG_PPCS//[    8] REG_BYP_PPCS
	INSREG8(g_voc_reg.vrec       + OFFSET8(VOC_REG_0XA1), VOC_REG_0X01, VOC_REG_0X01);
	//REG_ENABLE_CLK_SRC_A1_256FSi
	INSREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0X30), VOC_REG_0X01, VOC_REG_0X01);
	//REG_ENABLE_CLK_CODEC_I2S_RX_BCK
	INSREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0X30), VOC_REG_0X10, VOC_REG_0X10);
	//REG_ENABLE_CLK_NF_SYNTH_REF
	INSREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0X30), VOC_REG_0X40, VOC_REG_0X40);
	//REG_CODEC_RX_EN_TIME_GEN
	INSREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0XEC), VOC_REG_0X02, VOC_REG_0X02);
	//REG_ENABLE_CODEC_I2S_RX_BCK_GENe
	INSREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0X61), VOC_REG_0X80, VOC_REG_0X80);
	//[6] REG_CODEC_I2S_RX_FMT//1=left justified
	INSREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0X60), VOC_REG_0X40, VOC_REG_0X00);
	//tdm ch number
	INSREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0X61), VOC_REG_0X01, VOC_REG_0X01);
	//[13] REG_TDM_MODE//mandy :non TDM mode
	INSREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0X61), VOC_REG_0X20, VOC_REG_0X00);
	//reg_ckg_nf_synth_ref[4:0]
	INSREG8(g_voc_reg.ckgen00_pm + OFFSET8(VOC_REG_0X16), VOC_REG_0X1F, VOC_REG_0X04);
	//reg_ckg_src_a1_256si[4:0]
	INSREG8(g_voc_reg.ckgen00_pm + OFFSET8(VOC_REG_0X1E), VOC_REG_0X1F, VOC_REG_0X04);
	//reg_ckg_codec_i2s_ms_bck_p[4:0]
	INSREG8(g_voc_reg.ckgen00_pm + OFFSET8(VOC_REG_0X10), VOC_REG_0X1F, VOC_REG_0X08);
	#if (ENABLE_I2S_RX_MASTER_2STAGE_SETTING == 0)
	//reg_ckg_codec_i2s_rx_bck[4:0]
	INSREG8(g_voc_reg.ckgen00_pm + OFFSET8(VOC_REG_0X0E), VOC_REG_0X1F, VOC_REG_0X0C);
	#endif
	//[7]  REG_MS_WS_FMT
	INSREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0X60), VOC_REG_0X80, VOC_REG_0X00);
	//REG_MS_WS_FMT 0:I2S 1:Left justify
	INSREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0X60), VOC_REG_0X80, VOC_REG_0X00);
	//16bit ,0x43260[9] = 0
	INSREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0X61), VOC_REG_0X02, VOC_REG_0X00);
	//PDM & TDM sel mux:
	INSREG8(g_voc_reg.vrec       + OFFSET8(VOC_REG_0X04), VOC_REG_0X04, VOC_REG_0X04);
	// TDM RX Master Mode
	#if (ENABLE_I2S_RX_MASTER_2STAGE_SETTING == 0)
	//REG_I2S_TDM_PAD_OEN
	INSREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0X23), VOC_REG_0X80, VOC_REG_0X00);
	//REG_CODEC_I2S_RX_MS_MODE
	INSREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0X61), VOC_REG_0X40, VOC_REG_0X40);
	#endif
	//REG_CODEC_I2S_RX_WS_INV
	INSREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0X60), VOC_REG_0X20, VOC_REG_0X00);
	//REG_WS_WDTH
	INSREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0X62), VOC_REG_0XE0, VOC_REG_0X00);
	//REG_WS_WDTH
	INSREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0X63), VOC_REG_0X03, VOC_REG_0X00);
	// dec2hex(round(384000/16*2^23/256))
	//reg_ckg_nf_synth_ref
	INSREG8(g_voc_reg.ckgen00_pm + OFFSET8(VOC_REG_0X16), VOC_REG_0X1C, VOC_REG_0X04);
	//REG_CODEC_RX_NF_SYNTH_VALUE  [15: 0]
	OUTREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0XE8), VOC_REG_0X00);
	//REG_CODEC_RX_NF_SYNTH_VALUE  [15: 0]
	OUTREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0XE9), VOC_REG_0X00);
	//REG_CODEC_RX_NF_SYNTH_VALUE  [31:16]);
	OUTREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0XEA), VOC_REG_0X79);
	//REG_CODEC_RX_NF_SYNTH_VALUE  [31:16]
	OUTREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0XEB), VOC_REG_0X69);

	//REG_CODEC_RX_NF_SYNTH_TRIG
	INSREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0XEC), VOC_REG_0X01, VOC_REG_0X00);
	//REG_CODEC_RX_NF_SYNTH_TRIG
	INSREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0XEC), VOC_REG_0X01, VOC_REG_0X01);
	//REG_CODEC_RX_NF_SYNTH_TRIG
	INSREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0XEC), VOC_REG_0X01, VOC_REG_0X00);
	//bit [0]   = 0); //xtal
	INSREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0X3C), VOC_REG_0X01, VOC_REG_0X00);
	//bit [10]  = 0); //x 2
	INSREG8(g_voc_reg.vrec       + OFFSET8(VOC_REG_0X0D), VOC_REG_0X04, VOC_REG_0X00);
	//bit [3:2] = 2b'10); //internal
	INSREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0X3C), VOC_REG_0X0C, VOC_REG_0X08);
}
//==============================================================================
void voc_hal_paganini_set_mic_i2s_master(void)
{
#if ENABLE_I2S_RX_MASTER_2STAGE_SETTING
	//reg_ckg_codec_i2s_rx_bck[4:0]
	INSREG8(g_voc_reg.ckgen00_pm + OFFSET8(VOC_REG_0X0E), VOC_REG_0X1F, VOC_REG_0X0C);
	//reg_codec_i2s_rx_ms_mode
	INSREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0X61), VOC_REG_0X40, VOC_REG_0X40);
	//reg_codec_i2s_tdm_pad_oen
	INSREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0X23), VOC_REG_0X80, VOC_REG_0X00);
#endif
}
//==============================================================================
void voc_hal_paganini_set_mic_i2s_slave(void)
{
	//reg_ckg_codec_i2s_rx_bck[4:0]
	INSREG8(g_voc_reg.ckgen00_pm + OFFSET8(VOC_REG_0X0E), VOC_REG_0X1C, VOC_REG_0X04);
	//reg_codec_i2s_rx_ms_mode
	INSREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0X61), VOC_REG_0X40, VOC_REG_0X00);
	//reg_codec_i2s_tdm_pad_oen
	INSREG8(g_voc_reg.paganini   + OFFSET8(VOC_REG_0X23), VOC_REG_0X80, VOC_REG_0X80);
}
//==============================================================================
...
//==============================================================================
int voc_hal_paganini_set_bck_drive(u16 bck_drive)
{
	if (g_voc_reg.pm_ioctl != 0)
		INSREG16(g_voc_reg.pm_ioctl + OFFSET8(VOC_REG_0X80), VOC_REG_0X03, bck_drive);
...
//==============================================================================
static void voc_hal_paganini_set_recording_source(uint32_t value)
{
	switch (value) {
	case VOC_MIC_INPUT_PDM:
		INSREG8(g_voc_reg.vrec + OFFSET8(VOC_REG_0X04), BIT4, 0);
		break;

	case VOC_MIC_INPUT_I2S_RX_MASTER:
	case VOC_MIC_INPUT_I2S_RX_SLAVE:
	case VOC_MIC_INPUT_I2S_TDM_RX_MASTER:
	case VOC_MIC_INPUT_I2S_TDM_RX_SLAVE:
		INSREG8(g_voc_reg.vrec + OFFSET8(VOC_REG_0X04), BIT4, BIT4);
		break;
	}
}
//==============================================================================
void voc_hal_paganini_set_mic_input_mux(uint32_t value)
{
	if (value >= VOC_MIC_INPUT_MAX)
		return;

	voc_hal_paganini_set_recording_source(value);

	switch (value) {
	case VOC_MIC_INPUT_PDM:
		break;
	case VOC_MIC_INPUT_I2S_RX_MASTER:
		voc_hal_paganini_set_mic_i2s_common();
		voc_hal_paganini_set_mic_i2s_master();
		break;
	case VOC_MIC_INPUT_I2S_RX_SLAVE:
		voc_hal_paganini_set_mic_i2s_common();
		voc_hal_paganini_set_mic_i2s_slave();
		break;
	case VOC_MIC_INPUT_I2S_TDM_RX_MASTER:
	case VOC_MIC_INPUT_I2S_TDM_RX_SLAVE:
		voc_err("Not support I2S_TDM yet.\n");
	default:
		break;
	}

	g_voc_setting.mic_input_mux = value;
	g_voc_setting.pad_init = 0;

	//voc_debug("pad[11020]:0x%x\n",INREG16(g_voc_reg.pm_pmux|OFFSET8(VOC_REG_0X20)));
	//voc_debug("pad[11033]:0x%x\n",INREG8(g_voc_reg.pm_pmux|OFFSET8(VOC_REG_0X33)));
}
//==============================================================================
...
//==============================================================================
static void voc_hal_paganini_setting_init(void)
{
	memset(&g_voc_setting, 0, sizeof(struct mtk_voice_setting_t));
	g_voc_setting.mic_numbers = VOC_CHANNEL_4;
	g_voc_setting.bit_width   = VOC_BIT_WIDTH_16;
	g_voc_setting.sample_rate = VOC_SAMPLE_RATE_16K;
}
//==============================================================================
static int voc_hal_paganini_dts_init(void)
{
	...
	g_voc_reg.pm_ioctl	   = (phys_addr_t)of_iomap(np, VOC_VAL_19); //0x115
	of_node_put(np);
