...
#define VOICE_STANDBY_STATUS_MASK  (0xFF00)
#define VOICE_STANDBY_STATUS_SHIFT (8)

#define VOICE_IMI_ISOINTB_ON      (0xFFFF)
#define VOICE_IMI_ISOINTB_OFF     (0x0000)

#define VOICE_SUSPEND_TIMEOUT     (5000UL)
#define VOICE_RESUME_TIME         (1000UL)
#define VOICE_DUMP_TIME           (200)
#define VOICE_WAIT_SUSPEND_TIME   (10)
...
static int _mtk_mailbox_pm_imi_status(struct mtk_voc_mailbox *voc_mailbox, int status)
{
	...
	if (status) {
		...
		//delay 1us after sleep bank apply
		udelay(1);
	}
...
static int __mtk_mailbox_resume(struct mtk_voc_mailbox *voc_mailbox)
{
	...
	udelay(1); //wait to trigger interrupt to coprocessor
	CLRREG16((voc_mailbox->cpu_int)|(OFFSET(VOC_REG_0X02)), BIT2);
	usleep_range(VOICE_RESUME_TIME, VOICE_RESUME_TIME + 1);
	return 0;
}
...
static inline int __mtk_mailbox_rx_handler(struct mtk_voc_mailbox *voc_mailbox)
{
	...
	case VOICE_CMD_TIMESTAMP_NOTIFY: {
		...
		switch (stage) {
		case VOICE_TS_SYNC: {
			#if ENABLE_SYNC_BY_TIMER2
			voc_mailbox->voc->sync_time = voc_common_get_sync_time(ts);
			voc_dev_dbg(voc_mailbox->dev, "Sync time =,%lld\n",
				voc_mailbox->voc->sync_time);
			#endif
			ktime_get_ts64(&current_time);
			voc_mailbox->voc->ptp_time[PTP_TIME_INDEX_0] = ts*1000LL;//us -> ns
		...
		case VOICE_TS_DELAY_RESP: {
			...
			voc_mailbox->voc->ptp_time[PTP_TIME_INDEX_3] = ts*1000LL;//us->ns
			...
			diffcp = (voc_mailbox->voc->ptp_time[PTP_TIME_INDEX_3]
					- voc_mailbox->voc->ptp_time[PTP_TIME_INDEX_2]);
			#if (ENABLE_SYNC_BY_TIMER2 == 0)
			voc_mailbox->voc->sync_time = (diffpc - diffcp) >> 1;//average
			#endif
...
static void mtk_voc_mailbox_callback(struct mbox_client *client, void *mssg)
{
	...
	case MBOX_CMD_ACK:
		voc_dev_dbg(voc_mailbox->dev, "Got OK ack\n");
		voc_mailbox->ack_status = 0;
		complete(&voc_mailbox->ack);
		break;
	case MBOX_CMD_BAD_ACK:
		voc_dev_err(voc_mailbox->dev, "Got BAD ack\n");
		complete(&voc_mailbox->ack);
		break;
	case MBOX_CMD_MSG:
		...
		struct voice_mbox_msg *mssg, int need_ack)
{
	...
	if (ret < 0) {
		voc_dev_err(voc_mailbox->dev, "(%d) mailbox send failed, ret=%d\n", mssg->cmd, ret);
...
int voc_drv_mailbox_dma_start_channel(void)
{
	...
	ret = g_voc_mailbox->txn(g_voc_mailbox, &open_msg, VOICE_MBOX_NEED_ACK);
	if (ret) {
		voc_dev_err(g_voc_mailbox->dev, "%s failed, ret=%d\n", __func__, ret);
		goto voc_drv_mailbox_dma_start_channel_failed;
	}

	g_voc_mailbox->running_status = 1;
voc_drv_mailbox_dma_start_channel_failed:
	return ret;
}
...
int voc_drv_mailbox_dma_stop_channel(void)
{
	...
	if (ret) {
		voc_dev_err(g_voc_mailbox->dev, "%s failed, ret=%d\n", __func__, ret);
		goto voc_drv_mailbox_dma_stop_channel_failed;
	}

	g_voc_mailbox->running_status = 0;
voc_drv_mailbox_dma_stop_channel_failed:
	return ret;
}
				...
				uint32_t size)
{
	...
	src_addr_H = (uint32_t)((uint64_t)snd_model_paddr >> U32_BIT_NUM);
...
int voc_drv_mailbox_power_status(int32_t status)
{
	...
	if (status == VOC_POWER_MODE_SHUTDOWN) {
		while (suspend_cnt++ < VOICE_SUSPEND_TIMEOUT) {
			suspend_val = INREG16((g_voc_mailbox->vad)|(OFFSET(VOC_REG_0X1C)));
			suspend_val = (suspend_val & VOICE_STANDBY_STATUS_MASK);
			suspend_val = (suspend_val >> VOICE_STANDBY_STATUS_SHIFT);
			if (suspend_val == VOICE_COPROCESSOR_SUSPEND)
				break;
			if (suspend_cnt % VOICE_DUMP_TIME == 0) {
				voc_dev_warn(g_voc_mailbox->dev, "%s shutdown(%x,%d,%x,%x)\n",
					__func__,
					INREG16((g_voc_mailbox->coin_cpu)|(OFFSET(VOC_REG_0X4A))),
					INREG16((g_voc_mailbox->coin_cpu)|(OFFSET(VOC_REG_0X4C))),
					INREG16((g_voc_mailbox->coin_cpu)|(OFFSET(VOC_REG_0X4E))),
					INREG16((g_voc_mailbox->pm_sleep)|(OFFSET(VOC_REG_0XA0))));
			}
			udelay(VOICE_WAIT_SUSPEND_TIME);
		}

		suspend_cnt = 0;
		while (!(INREG16((g_voc_mailbox->coin_cpu)|(OFFSET(VOC_REG_0X20))) & BIT0)) {
			udelay(1);
			if (suspend_cnt++ > VOICE_SUSPEND_TIMEOUT)
				break;
		}

		if (suspend_cnt > VOICE_SUSPEND_TIMEOUT) {
			voc_dev_err(g_voc_mailbox->dev, "cm4 entry wfi timeout\n");
			return 0;
		}
		...
		while (suspend_cnt++ < VOICE_SUSPEND_TIMEOUT) {
			...
			if (suspend_val == VOICE_COPROCESSOR_SUSPEND)
				break;
			if (suspend_cnt % VOICE_DUMP_TIME == 0) {
				voc_dev_warn(g_voc_mailbox->dev, "%s suspend(%x,%d,%x,%x)\n",
					__func__,
					INREG16((g_voc_mailbox->coin_cpu)|(OFFSET(VOC_REG_0X4A))),
					INREG16((g_voc_mailbox->coin_cpu)|(OFFSET(VOC_REG_0X4C))),
					INREG16((g_voc_mailbox->coin_cpu)|(OFFSET(VOC_REG_0X4E))),
		...
		if (suspend_cnt > VOICE_SUSPEND_TIMEOUT) {
			voc_dev_err(g_voc_mailbox->dev, "cm4 power suspend timeout\n");
			return 0;
		}
...
int voc_drv_mailbox_notify_status(int32_t status)
{
	...
	if (ret)
		goto voc_drv_mailbox_notify_status_failed;

	if (status == 0)
		__mtk_mailbox_suspend(g_voc_mailbox);

	g_voc_mailbox->resume_status = status;
voc_drv_mailbox_notify_status_failed:
	return ret;
}
...
//==============================================================================
static int voc_drv_mailbox_get_handler(uint8_t cmd, void *params)
{
	UNUSED(cmd);
	UNUSED(params);
	return 0;
...
//==============================================================================
static int voc_drv_mailbox_set_handler(uint8_t cmd, void *params)
{
	UNUSED(cmd);
	UNUSED(params);
	return 0;
...
void voc_drv_mailbox_init(void)
{
	...
	vad = (phys_addr_t)of_iomap(np, VOC_VAL_13); //0x430
	coin_cpu = (phys_addr_t)of_iomap(np, VOC_VAL_15); //0x439
	pm_imi = (phys_addr_t)of_iomap(np, VOC_VAL_18); //0x438
	of_node_put(np);
	if (pm_sleep == 0 || pm_imi == 0 || pm_misc == 0 ||
		cpu_int == 0 || coin_cpu == 0 || vad == 0) {
...
static struct voc_communication_operater voc_mailbox_op = {
	...
	.get = voc_drv_mailbox_get_handler,
	.set = voc_drv_mailbox_set_handler,
};
