...
#include <linux/pinctrl/consumer.h> /* pinctrl API header*/
#include <sound/soc.h>
...
#if ENABLE_FIXED_CARD_ID
static int index[SNDRV_CARDS] = {VOC_FIXED_CARD_ID, [1 ... (SNDRV_CARDS-1)] = -1 };
#else
...
static void voc_card_shutdown(struct platform_device *pdev)
{
	...
	if (card_reg[VOC_REG_VQ_ONOFF] == VOC_WAKEUP_MODE_OFF ||
		card_reg[VOC_REG_VQ_ONOFF] == VOC_WAKEUP_MODE_NORMAL) {
		voc_communication_power_status(VOC_POWER_MODE_SHUTDOWN);
	}
}
...
static int voc_audio_dapm_suspend(struct device *dev)
{
	...
	if (card_reg[VOC_REG_VQ_ONOFF] == VOC_WAKEUP_MODE_OFF ||
		card_reg[VOC_REG_VQ_ONOFF] == VOC_WAKEUP_MODE_NORMAL) {
		voc_communication_power_status(VOC_POWER_MODE_SUSPEND);
	}
	return 0;
}
...
static int voc_audio_dapm_resume(struct device *dev)
{
	...
	if (card_reg[VOC_REG_VQ_ONOFF] == VOC_WAKEUP_MODE_OFF ||
		card_reg[VOC_REG_VQ_ONOFF] == VOC_WAKEUP_MODE_NORMAL) {
		voc_communication_power_status(VOC_POWER_MODE_RESUME);
	}
...
///sys/devices/platform/voc-card.0/mtk_dbg/force_mute
static ssize_t force_mute_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, MAX_LEN, "%u\n", g_pcm_muting);
}

static ssize_t force_mute_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t count)
{
	struct snd_card *card = dev_get_drvdata(dev);
	...
	if (voc) {
		ret = voc_communication_dmic_mute(g_pcm_muting | card_reg[VOC_REG_MUTE_SWITCH]);
		if (ret != 0)
			return ret;
	}
...
static DEVICE_ATTR_RW(force_mute);
				...
				char *buf)
{
	return scnprintf(buf, MAX_LEN, "Debug Help:\n"
				...
				"- force_mute <RW>: force mute microphone.\n"
				"                  Enable: 1; Disable: 0\n"
			...
			void __user *dst, unsigned long count)
{
	...
	if (voc->sysfs_fake_capture) {
		if (count > substream->dma_buffer.bytes)
			return -ENOMEM;
		zeroBuf = (unsigned char *)substream->dma_buffer.area;
	...
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
		...
		if (voc->sysfs_fake_capture) {
			if (!(card_reg[VOC_REG_MUTE_SWITCH] || g_pcm_muting)) {
			...
			struct snd_pcm_audio_tstamp_report *audio_tstamp_report)
{
	...
	audio_time = (pcm_data->pcm_time * VOICE_NS_PER_US //us->ns
				+ voc->sync_time); //ns
...
static int voc_card_dai_init(void)
{
	...
	if (np) {
		vbdma_base = of_iomap(np, VOC_DTS_ID_VBDMA);
...
static int voc_card_pinctrl_init(struct mtk_snd_voc *voc, char *pinctrl_name)
{
	int ret = 0;
	struct device_node *np;
	...
	if ((voc == NULL) || (voc->dev == NULL)) {
		voc_err("[%s]voc->dev is NULL\n", __func__);
		return -ENODEV;
	}

	np = voc->dev->of_node;
	voc->dev->of_node = of_find_compatible_node(NULL, NULL, "mediatek,mtk-mic-dma");
	if (!voc->dev->of_node) {
		ret = -EFAULT;
		goto exit2;
	}

	pinctrl = devm_pinctrl_get(voc->dev); /* get pinctrl handle */
	if (IS_ERR(pinctrl)) {
		voc_info("[%s] Can't find pinctrl.\n", __func__);
		ret = PTR_ERR(pinctrl);
		goto exit1;
	}
	/* retrieves a state handle from a pinctrl handle */
	default_state = pinctrl_lookup_state(pinctrl, pinctrl_name);
	if (IS_ERR(default_state)) {
		voc_info("[%s] Can't find pinctrl_name.\n", __func__);
...
static int voc_card_dma_init(struct platform_device *pdev, struct mtk_snd_voc *voc)
{
	...
	ret = of_property_read_u32(np, "mic-input-mux", &voc->mic_input_mux);
	if ((ret < 0) || (voc->mic_input_mux >= VOC_MIC_INPUT_MAX)) {
		voc_warn("[%s] Can't find input_mux from dts, use default setting.\n", __func__);
		voc->mic_input_mux = 0;
	}
...
static int voc_card_get_memory_info(u32 *addr)
{
	...
	if (p != NULL) {
		*addr = be32_to_cpup(p);
		...
		voc_info("can not find cpu_emi0_base info\n");
		of_node_put(target_memory_np);
		return -EINVAL;
	}
	return 0;
...
static void voc_card_mic_input_mux_init(struct mtk_snd_voc *voc)
{
	u32 param;
	...
	param = voc->mic_input_mux;
	switch (param) {
	case VOC_MIC_INPUT_PDM:
		 voc_card_pinctrl_init(voc, VOICE_PINCTRL_NAME_PDM);
		break;
	case VOC_MIC_INPUT_I2S_RX_MASTER:
	case VOC_MIC_INPUT_I2S_RX_SLAVE:
		 voc_card_pinctrl_init(voc, VOICE_PINCTRL_NAME_I2S);
		break;
	case VOC_MIC_INPUT_I2S_TDM_RX_MASTER:
	case VOC_MIC_INPUT_I2S_TDM_RX_SLAVE:
		 voc_card_pinctrl_init(voc, VOICE_PINCTRL_NAME_I2S_TDM);
	default:
		break;
	}
	voc_communication_set(VOC_CMD_SET_MIC_INPUT_MUX, &param);
}
...
static int voc_card_machine_init(struct platform_device *pdev,  struct mtk_snd_voc *voc)
{
	...
	ret = voc_card_get_memory_info(&miu_base);
	if (ret < 0) {
		voc_err("Request miu_base property failed, err = %d!!\n", ret);
		return ret;
	}
...
static int voc_card_module_init(struct platform_device *pdev, struct mtk_snd_voc *voc)
{
	...
	voc_card_mic_input_mux_init(voc);
				...
				unsigned int value)
{
	...
	case VOC_REG_MUTE_SWITCH: {
		voc_info("%s: mute switch reg: x%x, val = 0x%x (force mute:%d)\n", __func__,
				reg, value, g_pcm_muting);

		ret = voc_communication_dmic_mute(g_pcm_muting | value);
		break;
	}
	default:
...
static int voc_card_efuse_check(void)
{
	#define EFUSE_DTS_INDEX 17
	int ret = 0;
	u16 sw_config;
	struct device_node *np;
	...
	efuse_base = of_iomap(np, EFUSE_DTS_INDEX);
	of_node_put(np);

	if (!efuse_base)
		return ret;

	sw_config = readw(efuse_base + VOICE_SW_CONFIG_OFFSET);
	iounmap(efuse_base);

	if (sw_config & VOICE_SW_CONFIG_CM4_DISABLE)
		ret = -ENODEV;
...
static int voc_card_probe(struct platform_device *pdev)
{
	...
	ret = snprintf(card->longname, sizeof(card->longname), "vocsndcard %i", dev + 1);
	if (ret < 0) {
		voc_err("[%s][%d]Fail to call snprintf():%d\n"
			, __func__, __LINE__, ret);
	}
