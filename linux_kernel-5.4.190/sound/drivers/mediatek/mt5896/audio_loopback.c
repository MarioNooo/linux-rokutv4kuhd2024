...
#define CHANNEL_LAYOUT_2_0_0   (2)
#define CHANNEL_LAYOUT_5_1_2   (18)
#define CHANNEL_LAYOUT_MAX     (32)
...
#define DIGITAL_DELAY_NS_UNIT  (80000000)
#define TIMER_UNIT             (1)
...
static const char * const pcm_channel_layout_text[] = {
	"1.1.0", "1.0.0", "2.0.0", "2.1.0",
	"2.0.2", "2.1.2", "2.2.0", "3.0.0",
	"3.1.0", "3.0.2", "3.1.2", "4.0.0",
	"4.1.0", "4.0.2", "4.1.2", "5.0.0",
	"5.1.0", "5.0.2", "5.1.2", "2.0.4",
	"2.1.4", "3.0.4", "3.1.4", "3.2.0",
	"3.4.0", "3.2.2", "3.2.4", "4.0.4",
	"4.1.4", "5.0.4", "5.1.4", "7.1.0"};
...
SOC_ENUM_SINGLE(LOOPBACK_REG_PCM_CAPTURE_SOURCE, 0, LOOPBACK_FROM_MAX, pcm_capture_source_text);

static const struct soc_enum pcm_channel_layout_enum =
SOC_ENUM_SINGLE(LOOPBACK_REG_PCM_CHANNEL_LAYOUT, 0, CHANNEL_LAYOUT_MAX, pcm_channel_layout_text);

static const unsigned int pcm_capture_source_mapping[LOOPBACK_FROM_MAX] = {
	MIX_DSP_AEC_PATH_SEL_SPK_DELAY_SE,
	MIX_DSP_AEC_PATH_SEL_ADC2,
	MIX_DSP_AEC_PATH_SEL_ADC1,
	MIX_DSP_AEC_PATH_SEL_I2S_RX,
	MIX_DSP_AEC_PATH_SEL_I2S_RX_4ch,
	MIX_DSP_AEC_PATH_SEL_I2S_RX_8ch,
	MIX_DSP_AEC_PATH_SEL_ENC_FRONT_END_8ch};

//PCM related
static const struct snd_pcm_hardware audio_loopback_pcm_hardware = {
	.info = SNDRV_PCM_INFO_INTERLEAVED | SNDRV_PCM_INFO_BLOCK_TRANSFER,
	.formats = SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_S32_LE,
	.rates = SNDRV_PCM_RATE_16000 | SNDRV_PCM_RATE_32000 | SNDRV_PCM_RATE_48000,
	.rate_min = RATE_16000,
	.rate_max = RATE_48000,
	.channels_min = CH_2,
	.channels_max = CH_14,
...
static void audio_loopback_aud_set_loopback(int source)
{
	if (source == MIX_DSP_AEC_PATH_SEL_ADC2) {
		audio_loopback_write_reg_mask(REG.ADC_INPUT_CFG, MIX_DSP_ADC2_PATH_MASK,
			MIX_DSP_ADC_AUMIC_PATH);
	} else if (source == MIX_DSP_AEC_PATH_SEL_ADC1) {
		audio_loopback_write_reg_mask(REG.ADC_INPUT_CFG, MIX_DSP_ADC1_PATH_MASK,
			MIX_DSP_ADC_AUMIC_PATH << MIX_DSP_ADC1_PATH_SHIFT);
	}

	audio_loopback_write_reg_mask(REG.DSP_DMA_CTRL, MIX_DSP_AEC_MASK,
		source << MIX_DSP_AEC_FROM_ADC2_EN_BIT);
}
...
static void audio_loopback_capture_source_update(struct mtk_snd_loopback *loopback)
{
	if (loopback == NULL) {
		audio_err("[%s]Invalid loopback\n", __func__);
		return;
	...
	case LOOPBACK_FROM_I2S_RX_2CH:
		card_reg[LOOPBACK_REG_PCM_SLAVE_CHANNELS] = CH_2;
		card_reg[LOOPBACK_REG_PCM_CHANNEL_LAYOUT] = loopback->i2s_input_layout;
		break;
	case LOOPBACK_FROM_I2S_RX_4CH:
		card_reg[LOOPBACK_REG_PCM_SLAVE_CHANNELS] = CH_4;
		card_reg[LOOPBACK_REG_PCM_CHANNEL_LAYOUT] = loopback->i2s_input_layout;
		break;
	case LOOPBACK_FROM_I2S_RX_8CH:
		card_reg[LOOPBACK_REG_PCM_SLAVE_CHANNELS] = CH_8;
		card_reg[LOOPBACK_REG_PCM_CHANNEL_LAYOUT] = loopback->i2s_input_layout;
		break;
	case LOOPBACK_FROM_DIGITAL_OUT_8CH:
		card_reg[LOOPBACK_REG_PCM_SLAVE_CHANNELS] = CH_8;
		card_reg[LOOPBACK_REG_PCM_CHANNEL_LAYOUT] = CHANNEL_LAYOUT_5_1_2;
		card_reg[LOOPBACK_REG_PCM_SLAVE_ACTIVE_ONOFF] = 1;
		break;
	default:
		card_reg[LOOPBACK_REG_PCM_SLAVE_CHANNELS] = CH_2;
		card_reg[LOOPBACK_REG_PCM_CHANNEL_LAYOUT] = CHANNEL_LAYOUT_2_0_0;
	}
}
				...
				unsigned int value)
{
	...
	case LOOPBACK_REG_PCM_CAPTURE_SOURCE: {
		audio_info("%s: pcm capture source reg: x%x, val = 0x%x\n",
			__func__, reg, value);
		if (value >= LOOPBACK_FROM_MAX) {
			ret = -1;
			break;
		}
		loopback->dsp_capture_source = value;
		audio_loopback_capture_source_update(loopback);

		substream = loopback->dev_runtime.substream;
	...
	case LOOPBACK_REG_PCM_CHANNEL_LAYOUT: {
		audio_info("%s: pcm channel layout reg: x%x, val = 0x%x\n",
			__func__, reg, value);
		if (value >= CHANNEL_LAYOUT_MAX) {
			ret = -1;
			break;
		}

		card_reg[LOOPBACK_REG_PCM_CHANNEL_LAYOUT] = value;

		substream = loopback->dev_runtime.substream;
		if (loopback->loopback_pcm_running && substream != NULL) {
			memset(&elem_id, 0, sizeof(elem_id));
			elem_id.numid = LOOPBACK_REG_PCM_CHANNEL_LAYOUT;
			substream = loopback->dev_runtime.substream;
			kctl = snd_ctl_find_id(substream->pcm->card, &elem_id);
			if (kctl != NULL)
				snd_ctl_notify(loopback->card,
...
static const struct snd_kcontrol_new audio_loopback_snd_controls[] = {
	...
	SOC_ENUM_EXT("PCM Capture Source", pcm_capture_source_enum,
			audio_loopback_card_get_enum, audio_loopback_card_put_enum),
	SOC_ENUM_EXT("PCM Channel Layout", pcm_channel_layout_enum,
			audio_loopback_card_get_enum, audio_loopback_card_put_enum),
};
						...
						int sample_rate, int channels)
{
	...
	if (!loopback->loopback_aec_bypass && (sample_rate == AEC_RATE ||
		loopback->loopback_pcm_sw_running || channels <= loopback->aec_max_channel))
						...
						int sample_rate, int channels, int bits)
{
	...
	for (i = 0 ; i < loopback->dsp_channels_num && bits == BITS_16 &&
			...
			unsigned int size)
{
	...
	if (!loopback->dsp_mixing_mode) {
		...
		if (loopback->loopback_pcm_sw_running == 1) {
	...
	if (loopback->loopback_aec_bypass) {
		...
		if (loopback->dsp_pb_dma_buf.r_buf.buffer_level > 0) {
			...
			if (loopback->loopback_pcm_running &&
				(loopback->loopback_pcm_sw_running &&
				loopback->loopback_pcm_sw_running_stopped == 0))
...
static unsigned int audio_loopback_get_capture_dsp_offset(struct mtk_snd_loopback *loopback)
{
	...
	if (loopback->loopback_pcm_sw_running == 1) {
		if (loopback->loopback_pcm_sw_running_stopped == 1) {
...
static unsigned int audio_loopback_get_capture_aec_offset(struct mtk_snd_loopback *loopback)
{
	unsigned char *addr = loopback->aec_dma_buf.r_buf.addr;
	unsigned char *w_ptr = loopback->aec_dma_buf.r_buf.w_ptr;
	unsigned int write_level = 0;

	//get dma write level
	audio_loopback_write_reg_mask_byte(REG.AEC_DMA_CTRL, BIT5, BIT5);
	write_level = audio_loopback_read_reg(REG.AEC_DMA_WPTR);
	write_level = (write_level > FIFO_ALIGN_SIZE) ? write_level - FIFO_ALIGN_SIZE : ZERO;
	write_level = write_level * BYTES_IN_MIU_LINE;
	audio_loopback_write_reg_mask_byte(REG.AEC_DMA_CTRL, BIT5, ZERO);

	if ((write_level > loopback->aec_dma_buf.r_buf.size) ||
		(write_level < loopback->aec_dma_buf.r_buf.buffer_level))
		return loopback->aec_dma_buf.r_buf.size + 1; //overrun

	//offset is data size instead of position, it will increase if not read
	//After read, it will decrease
	write_level = write_level - loopback->aec_dma_buf.r_buf.buffer_level;
	return (w_ptr - addr + write_level) % loopback->aec_dma_buf.r_buf.size;
}
...
static int audio_loopback_dma_init(struct mtk_snd_loopback *loopback)
{
	...
	if (!loopback->dsp_mixing_mode) {
		if (loopback->loopback_aec_bypass && loopback->loopback_pcm_sw_running) {
	...
	if (!loopback->loopback_pcm_sw_running && !loopback->dsp_mixing_mode) {
		if (!loopback->loopback_aec_enabled) {
	...
	if (loopback->dsp_mixing_mode) {
		...
		dsp_channel = pcm_capture_source_mapping[loopback->dsp_capture_source];
		if (loopback->slt_test_config)
			dsp_channel = MIX_DSP_AEC_PATH_SEL_DBG_SINE_TONE;
		audio_loopback_aud_set_loopback(dsp_channel);
	} else if (!loopback->loopback_pcm_sw_running) {
		audio_loopback_write_reg_mask_byte(REG.DSP_DMA_CTRL,
			DSP_CHANNEL_MASK, dsp_channel);

		card_reg[LOOPBACK_REG_PCM_CAPTURE_SOURCE] = (dsp_channel == DSP_CHANNEL_8) ?
			LOOPBACK_FROM_I2S_RX_8CH : LOOPBACK_FROM_I2S_RX_2CH;
	}
...
static int audio_loopback_dma_start(struct mtk_snd_loopback *loopback)
{
	...
	if (loopback->dsp_mixing_mode) {
		if (loopback->dsp_capture_source >= LOOPBACK_FROM_I2S_RX_2CH &&
	...
	timer_enable = (loopback->loopback_aec_enabled || loopback->loopback_pcm_sw_running);
	...
	if (loopback->dsp_mixing_mode &&
		loopback->dsp_capture_source == LOOPBACK_FROM_DIGITAL_OUT_8CH)
		timespec_add_ns(&_stAecTimeStamp, DIGITAL_DELAY_NS_UNIT);
...
static int audio_loopback_dma_stop(struct mtk_snd_loopback *loopback)
{
	...
	xrun_trigger = loopback->dsp_dma_buf.xrun_trigger | loopback->aec_dma_buf.xrun_trigger;
	if (xrun_trigger == 0) {
		spin_lock_irqsave(&(dev_runtime->spin_lock), flags);
		dev_runtime->monitor.monitor_status = CMD_STOP;
		spin_unlock_irqrestore(&(dev_runtime->spin_lock), flags);
	...
	loopback->aec_dma_buf.r_buf.buffer_level = 0;
	 /* reset written size for DSP */
	loopback->aec_dma_buf.copy_size = 0;
	loopback->aec_dma_buf.xrun_trigger = 0;
	...
	/* reset written size for DSP */
	loopback->dsp_dma_buf.copy_size = 0;
	loopback->dsp_dma_buf.xrun_trigger = 0;
	loopback->dsp_dma_buf.status = CMD_STOP;
				...
				void *buffer, unsigned int bytes)
{
	...
	tmp_bytes = bytes;
	if (loopback->loopback_pcm_sw_running == 0)
	...
	else if (loopback->loopback_pcm_sw_running == 1 &&
			loopback->loopback_pcm_sw_running_stopped == 1)
	...
	if (loopback->loopback_aec_enabled) {
		...
		loopback->aec_dma_buf.r_buf.buffer_level -=
				(tmp_size > loopback->aec_dma_buf.r_buf.buffer_level) ?
				loopback->aec_dma_buf.r_buf.buffer_level : tmp_size;
...
static int audio_loopback_pcm_open(struct snd_pcm_substream *substream)
{
	...
	if (playback == 1) {
		...
		if (loopback->loopback_pcm_playback_running) {
			...
			loopback->loopback_pcm_sw_running = 1;
			loopback->loopback_pcm_sw_running_stopped = 0;
		} else {
			if (!loopback->dsp_mixing_mode) {
			...
			loopback->loopback_pcm_sw_running = 0;
			loopback->loopback_pcm_sw_running_stopped = 0;
		}
					...
					struct snd_pcm_hw_params *params)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if (loopback->loopback_aec_enabled) {
			if (!loopback->loopback_pcm_sw_running && dsp_support)
				loopback->loopback_aec_enabled = 0;
		} else if (loopback->loopback_pcm_sw_running) {
...
static snd_pcm_uframes_t audio_loopback_pcm_pointer(struct snd_pcm_substream *substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if (loopback->loopback_aec_enabled) {
			new_w_ptr_offset = audio_loopback_get_capture_aec_offset(loopback);
			if (new_w_ptr_offset > loopback->aec_dma_buf.r_buf.size) {
				offset_bytes = DMA_XRUN;
				loopback->aec_dma_buf.xrun_trigger = 1;
				mutex_unlock(&loopback->loopback_pcm_lock);
				goto _audio_loopback_pcm_pointer_update_pos;
			}

			new_w_ptr = loopback->aec_dma_buf.r_buf.addr + new_w_ptr_offset;
			offset_bytes = new_w_ptr - loopback->aec_dma_buf.r_buf.w_ptr;
			if (offset_bytes < 0)
				offset_bytes += loopback->aec_dma_buf.r_buf.size;

			temp_size = bytes_to_frames(runtime, offset_bytes);
			period_size = INTERVAL_UNIT *
				(loopback->aec_dma_buf.sample_rate/RATE_16000);

			if (temp_size < period_size) {
				offset_bytes = 0;
				mutex_unlock(&loopback->loopback_pcm_lock);
				goto _audio_loopback_pcm_pointer_update_pos;
			}

			offset_bytes = frames_to_bytes(runtime, period_size);
			...
			loopback->aec_dma_buf.r_buf.buffer_level += offset_bytes;
...
_audio_loopback_pcm_pointer_update_pos:
	if (offset_bytes < 0) {
		audio_err("%s: stream = %s xrun\n", __func__,
			(substream->stream ==
			SNDRV_PCM_STREAM_PLAYBACK ? "PLAYBACK" : "CAPTURE"));

		new_hw_ptr_pos = SNDRV_PCM_POS_XRUN;
	} else {
		offset_pcm_samples = bytes_to_frames(runtime, offset_bytes);
		new_hw_ptr = hw_ptr + offset_pcm_samples;
	...
	struct platform_device *pdev, struct mtk_snd_loopback *loopback)
{
	...
	if (mixing_mode) {
		REG.ADC_INPUT_CFG = audio_loopback_get_dts_u32(loop_np, "reg_mix_adc_input_cfg");
...
static int audio_loopback_get_memory_info(u32 *addr)
{
	...
	if (p != NULL) {
		*addr = be32_to_cpup(p);
		...
		audio_info("can not find cpu_emi0_base info\n");
		of_node_put(target_memory_np);
		return -EINVAL;
	}
	return 0;
...
static int audio_loopback_parse_memory(struct mtk_snd_loopback *loopback)
{
	...
	ret = audio_loopback_get_memory_info(&memory_bus_base);
	loopback->aec_mem_info.memory_bus_base = memory_bus_base;

	if (!mixing_mode) {
		loopback->dsp_mem_info.memory_bus_base    = memory_bus_base;
		loopback->dsp_pb_mem_info.memory_bus_base = memory_bus_base;
	}
	if (ret) {
...
static int audio_loopback_parse_capture_source(
	struct mtk_snd_loopback *loopback, const char *source)
{
	int capture_source = LOOPBACK_FROM_I2S_RX_2CH;

	if (loopback == NULL)
		return capture_source;

	if (source != NULL) {
		if (source[CAP_SOURCE_IDX0] == 'S' && source[CAP_SOURCE_IDX1] == 'W')
			capture_source = LOOPBACK_FROM_SPEAKER;
		else if (source[CAP_SOURCE_IDX0] == 'A' && source[CAP_SOURCE_IDX1] == 'D' &&
			source[CAP_SOURCE_IDX2] == 'C' && source[CAP_SOURCE_IDX3] == '2')
			capture_source = LOOPBACK_FROM_ADC2;
		else if (source[CAP_SOURCE_IDX0] == 'A' && source[CAP_SOURCE_IDX1] == 'D' &&
			source[CAP_SOURCE_IDX2] == 'C' && source[CAP_SOURCE_IDX3] == '1')
			capture_source = LOOPBACK_FROM_ADC1;
	}

	if (capture_source == LOOPBACK_FROM_I2S_RX_2CH) {
		switch (loopback->i2s_input_channel) {
		case CH_2:
			capture_source = LOOPBACK_FROM_I2S_RX_2CH;
			break;
		case CH_4:
			capture_source = LOOPBACK_FROM_I2S_RX_4CH;
			break;
		case CH_8:
			capture_source = LOOPBACK_FROM_I2S_RX_8CH;
			break;
		default:
			audio_warn("[%s] channel is not match, use default(%d)!\n",
				__func__, loopback->i2s_input_channel);
	...
	struct platform_device *pdev, struct mtk_snd_loopback *loopback)
{
	struct device_node *loop_np, *dsp_np;
	struct device_node *soc_np;
	struct device_node *voc_np, *voc_loop_np;
	const char *cap_source = NULL;
	int mixing_mode = 0;
	...
	dsp_np = of_find_node_by_name(NULL, "mtk-sound-capture-platform");
	soc_np = of_find_node_by_name(NULL, "mtk-sound-soc");
	voc_np = of_find_node_by_name(NULL, "voc-audio");
	if (!loop_np || (!mixing_mode && (!dsp_np | !soc_np)))
		return -ENODEV;
	...
	ret |= of_property_read_u32(loop_np, "i2s_input_layout", &loopback->i2s_input_layout);
	ret |= of_property_read_u32(loop_np, "i2s_out_clk", &loopback->i2s_out_clk);
	if (ret)
		return -EINVAL;

	if (voc_np) {
		voc_loop_np = of_get_child_by_name(voc_np, "loopback");
		if (voc_loop_np)
			ret = of_property_read_string(voc_loop_np, "source", &cap_source);
	}

	//update capture source
	loopback->dsp_capture_source = audio_loopback_parse_capture_source(loopback, cap_source);
	...
	struct platform_device *pdev, struct mtk_snd_loopback *loopback)
{
	...
	// Timer setup
	timer_setup(&(loopback->dev_play_runtime.timer), audio_loopback_pb_timer_monitor, 0);
	timer_setup(&(loopback->dev_runtime.timer), audio_loopback_timer_monitor, 0);
...
static int audio_loopback_card_remove(struct platform_device *pdev)
{
	...
	//Stop timer
	del_timer_sync(&(loopback->dev_runtime.timer));
	del_timer_sync(&(loopback->dev_play_runtime.timer));
