...
static inline int __mtk_rpmsg_rx_handler(struct mtk_voc_rpmsg *voc_rpmsg)
{
	...
	case VOICE_CMD_TIMESTAMP_NOTIFY: {
		...
		case VOICE_TS_SYNC: {
			...
			voc_rpmsg->voc->sync_time = voc_common_get_sync_time(ts);
			voc_dev_dbg(voc_rpmsg->dev, "Sync time =, %lld\n",
				voc_rpmsg->voc->sync_time);
			#endif
			ktime_get_ts64(&current_time);
			voc_rpmsg->voc->ptp_time[PTP_TIME_INDEX_0] = ts*1000LL;//us -> ns
			voc_rpmsg->voc->ptp_time[PTP_TIME_INDEX_1] =
				(current_time.tv_sec*1000000000LL + current_time.tv_nsec);

			voc_dev_dbg(voc_rpmsg->dev,
				"TS: %llu us, Time = [(%lld)nsec, (%lld)sec, (%ld)nsec]\n",
		...
		case VOICE_TS_DELAY_RESP: {
			...
			voc_rpmsg->voc->ptp_time[PTP_TIME_INDEX_3] = ts*1000LL;//us->ns
			voc_dev_dbg(voc_rpmsg->dev, "TS: %llu us\n", ts);
			...
			voc_rpmsg->voc->sync_time = (diffpc - diffcp) >> 1; //average
			#endif
...
static int voc_drv_rpmsg_get_handler(uint8_t cmd, void *params)
{
	UNUSED(cmd);
	UNUSED(params);
	return 0;
...
static int voc_drv_rpmsg_set_handler(uint8_t cmd, void *params)
{
	UNUSED(cmd);
	UNUSED(params);
	return 0;
...
static struct voc_communication_operater voc_rpmsg_op = {
	...
	.get = voc_drv_rpmsg_get_handler,
	.set = voc_drv_rpmsg_set_handler,
};
