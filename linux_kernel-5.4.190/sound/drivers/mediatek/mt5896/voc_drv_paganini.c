...
static uint64_t voc_drv_paganini_ktime_to_us(struct timespec64 *ktime)
{
	uint64_t value;

	value = ktime->tv_sec * VOICE_US_PER_SECOND;
	value +=  ktime->tv_nsec / VOICE_NS_PER_US;
	return value;
}
//==============================================================================
						...
						uint64_t *time_interval)
{
	...
	uint32_t sample_per_ms = voc_hal_paganini_get_sample_rate()/VOICE_MS_PER_SECOND;
	uint32_t buf_size      = frame_count * bytes_of_bit_width * channels;


	uint32_t underrun_size = (CONFIG_PCM_SAMPLE_RATE / VOICE_MS_PER_SECOND) *
							 bytes_of_bit_width *
							 channels *
							 VOICE_UNDERRUN_MS;
	...
	do {
		if (g_paganini.dma_buf_wr_enable == 0)
			...
			*time_interval = voc_drv_paganini_ktime_to_us(
			...
			usleep_range(VOICE_US_PER_MS, VOICE_US_PER_MS + 1);
...
int voc_drv_paganini_pcm_handler_thread(void *param)
{
	...
	do {
		...
		frame_count = (voc_hal_paganini_get_sample_rate() / VOICE_MS_PER_SECOND) *
						VOC_PERIOD_MS;
		...
		g_paganini.time_interval += VOC_PERIOD_MS * VOICE_US_PER_MS;
...
int voc_drv_paganini_dma_start_channel(void)
{
	...
	overrun_thres = (CONFIG_PCM_SAMPLE_RATE *
					sample_byte *
					mic_num *
					VOICE_OVERRUN_MS) / VOICE_MS_PER_SECOND;
	...
	if (!g_paganini.dma_buf_wr_enable) {
		...
		ktime_get_ts64(&g_paganini.timestamp);
		g_paganini.time_interval = voc_drv_paganini_ktime_to_us(
				&g_paganini.timestamp);
...
int voc_drv_paganini_dmic_switch(bool en)
{
	...
	if (en)
		voc_hal_paganini_set_bck_drive(g_paganini.bck_drive);

	ret = voc_hal_paganini_dmic_switch(en, g_paganini.bck_mode, g_paganini.mute_enable);
...
{
	if (status == VOC_POWER_MODE_RESUME)
...
//==============================================================================
static int voc_drv_paganini_get_handler(uint8_t cmd, void *params)
{
	UNUSED(cmd);
	UNUSED(params);
	return 0;
...
//==============================================================================
static int voc_drv_paganini_set_handler(uint8_t cmd, void *params)
{
	unsigned int value;

	switch (cmd) {
	case VOC_CMD_SET_MIC_INPUT_MUX:
		value = *(unsigned int *)params;
		voc_hal_paganini_set_mic_input_mux(value);
		break;

	default:
		voc_err("%s:command 0x%x is not support\n", __func__, cmd);
		return 0;
	}
	return 0;
}
...
static irqreturn_t voc_drv_paganini_irq_handler(int irq, void *p)
{
	...
	if (voc_hal_paganini_get_vdma_timer_interrupt_status())	{
		...
		timespec64_add_ns(&g_paganini.timestamp, VOC_PERIOD_MS*VOICE_NS_PER_MS);
		//ktime_get_ts64(&g_paganini.timestamp);
	}
...
static int voc_drv_paganini_dts_init(void)
{
	...
	if ((ret >= 0) && (u32Tmp <= VOC_BCK_MODE_MAX))
		g_paganini.bck_mode = (u16)u32Tmp;
	else
		g_paganini.bck_mode = VOC_DEFAULT_BCK_MODE;

	ret = of_property_read_u32(np, "bck_drive", &u32Tmp);

	if ((ret >= 0) && (u32Tmp <= VOC_BCK_DRIVE_MAX))
		g_paganini.bck_drive = (u16)u32Tmp;
	else
		g_paganini.bck_drive = VOC_DEFAULT_BCK_DRIVE;
...
static struct voc_communication_operater voc_paganini_op = {
	...
	.get = voc_drv_paganini_get_handler,
	.set = voc_drv_paganini_set_handler,
};
